...
The nonce must be 32-64 lowercase hexadecimal characters. It is parsed in memory from `AUTOMATIC_INGESTION_BOUNDED_APPROVAL`; it is not a workflow input. The approval value and raw nonce must never be placed in logs, summaries, artifacts, database rows, PR text, or comments.
