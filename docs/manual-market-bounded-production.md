...
The nonce must be 32-64 lowercase hexadecimal characters. The approval value and raw nonce must never be placed in logs, summaries, artifacts, database rows, PR text, or comments.
...
The Run uploads `market-bounded-manual-result-<run_id>`. Review the manual preflight, approval claim, fresh candidate audit, bounded plan, persistence preview, bounded result, Production count delta, rollback result, and secret scan before accepting the Run. A successful result requires the exact main and policy identities, a fresh plan, complete candidate identity checks, exact allowed deltas, zero forbidden table deltas, and zero secret findings.
