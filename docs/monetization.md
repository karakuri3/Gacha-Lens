...
- Rakuten API listings preserve the provider-issued `affiliateUrl` when `RAKUTEN_AFFILIATE_ID` is configured. A released variant uses it only when the current listing is linked, active, non-review-required, single-item data from the Rakuten API.
