...
- Rakuten API listings keep the ordinary `itemUrl` as the listing and candidate identity, and preserve the provider-issued `affiliateUrl` as separate sanitized provenance when `RAKUTEN_AFFILIATE_ID` is configured. Automatic bounded persistence carries that provenance without changing the durable listing ID. A released variant uses it only when the current listing is linked, active, non-review-required, single-item data from the Rakuten API.
