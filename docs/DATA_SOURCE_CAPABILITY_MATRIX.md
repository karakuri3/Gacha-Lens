...
| Mercari Shops broad market-intelligence capability | `unavailable` | Public API exists, but queries are explicitly scoped to the authenticated shop, e.g. “your Shop's Products” | Own-shop orders include completed/order data, but this is **not** broad C2C completed-market history | Product, preorder, shop/order administration capabilities exist inside the seller scope | Not a public marketplace-demand feed | Not evaluated for Gacha Lens | Product/shop/order IDs inside the authenticated seller scope | Personal API Access Token identifies the shop; docs expose `products` for your shop and `shop` that you own. The seller/store API exists, but the broad market-intelligence capability Gacha Lens needs is unavailable through that scope | **Low for broad market DATA / high semantic risk if misused** |
