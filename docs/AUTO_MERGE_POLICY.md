...
For merge decisions, this file is explicitly referenced by `AGENTS.md` and overrides older lower-precedence Agent OS text that says every merge must be human-performed. A merge whose only Production consequence is the repository's existing reviewed Cloudflare Production application release may also proceed without a repeated human acknowledgement only when `docs/PRODUCTION_RELEASE_POLICY.md` passes in full. This does not weaken database, secrets, destructive-action, paid-operation, workflow-dispatch, authentication-boundary, or major-product-decision boundaries.

Routine Vercel Git builds are intentionally skipped after the Cloudflare Production cutover under `docs/VERCEL_COST_CONTROL.md`; Vercel deployment status is not a standing merge/release gate while that documented skip remains active.

## Default

Use a pull request. Never direct-push autonomous work to `main`.

An Agent may move an eligible Draft PR to ready state and merge it to `main` without asking the human again only when the complete Auto-Merge Gate below passes. If merging triggers the existing reviewed Cloudflare Production application release, the Standing Production Release Gate must also pass.

Default merge method: squash unless repository constraints require another non-destructive method.

## Auto-Merge Gate

Every item must be true and evidenced in the PR, CI, or repository state:

- the task is bounded, safe, reversible, and non-Production before merge; any Production consequence of merge is limited to an eligible normal Cloudflare release under `docs/PRODUCTION_RELEASE_POLICY.md`
...
- exact-head Cloudflare Preview/version/runtime/cache/security evidence required by `docs/PRODUCTION_RELEASE_POLICY.md` for the diff passes, or is correctly `N/A` for a change with no application/runtime consequence
...
- Production promotion/gate changes or direct Production writes outside the eligible normal Cloudflare release
- Production database migration, schema change, backfill, cleanup, seed, reset, or data mutation
- GitHub Actions `workflow_dispatch`
- Repository, Cloudflare, Vercel, Supabase, or other Secrets / Variables changes
...
4. If merge triggers Cloudflare Production, also run the complete Standing Production Release Gate, including exact-head Cloudflare evidence applicable to the diff.
...
8. If authorized by the Production Release Policy, allow and observe only the existing reviewed Cloudflare Production application release triggered by the merge. Do not manually dispatch/promote a release by implication.
9. Record the merged PR, merge SHA, Cloudflare release disposition, and exact-head release evidence in the task result.
