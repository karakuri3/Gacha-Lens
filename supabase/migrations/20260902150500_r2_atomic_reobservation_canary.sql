-- R2 first-history canary: four known listings, one atomic transaction.
-- Repository-only prerequisite. Applying this migration to Production remains explicit-approval bound.

create or replace function public.apply_market_reobservation_r2_canary_v1(p_batch jsonb)
returns jsonb
language plpgsql
security invoker
set search_path = ''
...
begin
  if p_batch is null
    ...
    raise exception using errcode = '22023', message = 'r2_invalid_batch_size';
  end if;
  ...
  -- exactly-one-prior-observation precondition while this tiny canary commits.
  ...
  loop
    if jsonb_typeof(v_item) <> 'object' then
      raise exception using errcode = '22023', message = 'r2_invalid_entry';
    end if;
    ...
    if (v_provider = 'rakuten_ichiba' and v_source <> 'rakuten')
      or (v_provider = 'yahoo_shopping' and v_source <> 'yahoo_shopping') then
      raise exception using errcode = '22023', message = 'r2_provider_source_mismatch';
    end if;
    ...
    if v_shared_observation_key is null then
      ...
      raise exception using errcode = '22023', message = 'r2_observation_key_mismatch';
    end if;
    ...
    exception when invalid_text_representation or datetime_field_overflow or numeric_value_out_of_range then
      raise exception using errcode = '22023', message = 'r2_invalid_scalar';
    end;
    ...
    if not found then
      raise exception using errcode = 'P0001', message = 'r2_listing_missing';
    end if;
    ...
    if v_listing.listing_type is distinct from 'single'
      ...
      raise exception using errcode = 'P0001', message = 'r2_listing_identity_or_scope_changed';
    end if;
    ...
    if exists (
      ...
      raise exception using errcode = 'P0001', message = 'r2_unresolved_import_issue';
    end if;
    ...
    if v_prior_observation_count <> 1 then
      raise exception using errcode = 'P0001', message = 'r2_prior_observation_count_changed';
    end if;

    if exists (
      select 1 from public.market_listing_observations o where o.id = v_observation_id
    ) then
      raise exception using errcode = 'P0001', message = 'r2_observation_id_collision';
    end if;
    ...
    if v_observation_id <> v_expected_observation_id then
      raise exception using errcode = 'P0001', message = 'r2_observation_id_mismatch';
    end if;
  ...
  if cardinality(v_listing_ids) <> 4 or cardinality(v_observation_ids) <> 4 then
    raise exception using errcode = 'P0001', message = 'r2_atomic_apply_incomplete';
  end if;
  ...
  if exists (
    ...
    raise exception using errcode = 'P0001', message = 'r2_postwrite_observation_count_mismatch';
  end if;
...
revoke execute on function public.apply_market_reobservation_r2_canary_v1(jsonb) from public;
revoke execute on function public.apply_market_reobservation_r2_canary_v1(jsonb) from anon;
revoke execute on function public.apply_market_reobservation_r2_canary_v1(jsonb) from authenticated;
