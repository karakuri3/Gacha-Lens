-- R2 v2 first-history canary: four frozen Yahoo Shopping listings, one atomic transaction.
-- Repository-only prerequisite. Production application and execution remain separately explicit-approval bound.

create or replace function public.apply_market_reobservation_r2_canary_v2(p_batch jsonb)
returns jsonb
language plpgsql
security invoker
set search_path = ''
...
declare
  ...
  v_prior_observation_count integer;
  ...
  v_frozen_observation_key constant text := 'reobs-v1:r2-20260902-02';
begin
  if p_batch is null
    or jsonb_typeof(p_batch) <> 'array'
    or jsonb_array_length(p_batch) <> 4 then
    raise exception using errcode = '22023', message = 'r2_v2_invalid_batch_size';
  end if;
  ...
  if (
    ...
    raise exception using errcode = '22023', message = 'r2_v2_duplicate_batch_identity';
  end if;
  ...
  if v_received_listing_ids is distinct from v_frozen_listing_ids then
    raise exception using errcode = '22023', message = 'r2_v2_frozen_cohort_mismatch';
  end if;
  ...
  loop
    if jsonb_typeof(v_item) <> 'object' then
      raise exception using errcode = '22023', message = 'r2_v2_invalid_entry';
    end if;
    ...
    if v_listing_id = ''
      ...
      or v_provider <> 'yahoo_shopping'
      or v_source <> 'yahoo_shopping'
      or v_native_id = ''
      or v_public_url = ''
      or v_variant_id = ''
      or v_series_id = ''
      ...
      raise exception using errcode = '22023', message = 'r2_v2_invalid_entry_contract';
    end if;
    ...
    if v_shared_observation_key is null then
      ...
      raise exception using errcode = '22023', message = 'r2_v2_observation_key_mismatch';
    end if;
    ...
    begin
      ...
      if v_observed_at is null or v_expected_last_observed_at is null then
        raise exception using errcode = '22023', message = 'r2_v2_invalid_timestamp';
      end if;
      if coalesce(v_item->>'price', '') !~ '^[1-9][0-9]*$'
        or coalesce(v_item->>'expected_price', '') !~ '^[1-9][0-9]*$' then
        raise exception using errcode = '22023', message = 'r2_v2_invalid_price';
      end if;
      v_price := (v_item->>'price')::integer;
      v_expected_price := (v_item->>'expected_price')::integer;
    exception when invalid_text_representation or datetime_field_overflow or numeric_value_out_of_range then
      raise exception using errcode = '22023', message = 'r2_v2_invalid_scalar';
    end;
    ...
    if not found then
      raise exception using errcode = 'P0001', message = 'r2_v2_listing_missing';
    end if;
    ...
    if v_listing.listing_type is distinct from 'single'
      ...
      or v_listing.source is distinct from 'yahoo_shopping'
      or v_listing.source_type is distinct from 'marketplace'
      or v_listing.source_url is distinct from v_public_url
      or coalesce(v_listing.raw->>'provider', '') is distinct from 'yahoo_shopping'
      ...
      raise exception using errcode = 'P0001', message = 'r2_v2_listing_identity_or_scope_changed';
    end if;
    ...
    if v_listing.price is distinct from v_expected_price
      ...
      raise exception using errcode = 'P0001', message = 'r2_v2_listing_snapshot_changed';
    end if;

    if v_listing.last_observed_at is null or v_observed_at <= v_listing.last_observed_at then
      raise exception using errcode = 'P0001', message = 'r2_v2_observation_time_not_newer';
    end if;
    ...
    if exists (
      ...
      raise exception using errcode = 'P0001', message = 'r2_v2_unresolved_import_issue';
    end if;

    select count(*)::integer
    into v_prior_observation_count
    from public.market_listing_observations o
    where o.listing_id = v_listing_id;

    if v_prior_observation_count <> 1 then
      raise exception using errcode = 'P0001', message = 'r2_v2_prior_observation_count_changed';
    end if;

    if exists (
      select 1 from public.market_listing_observations o where o.id = v_observation_id
    ) then
      raise exception using errcode = 'P0001', message = 'r2_v2_observation_id_collision';
    end if;
    ...
    if v_observation_id <> v_expected_observation_id then
      raise exception using errcode = 'P0001', message = 'r2_v2_observation_id_mismatch';
    end if;
  ...
  if cardinality(v_listing_ids) <> 4 or cardinality(v_observation_ids) <> 4 then
    raise exception using errcode = 'P0001', message = 'r2_v2_atomic_apply_incomplete';
  end if;

  if exists (
    select 1
    from unnest(v_listing_ids) as target(listing_id)
    where (select count(*) from public.market_listing_observations o where o.listing_id = target.listing_id) <> 2
  ) then
    raise exception using errcode = 'P0001', message = 'r2_v2_postwrite_observation_count_mismatch';
  end if;
...
revoke execute on function public.apply_market_reobservation_r2_canary_v2(jsonb) from public;
revoke execute on function public.apply_market_reobservation_r2_canary_v2(jsonb) from anon;
revoke execute on function public.apply_market_reobservation_r2_canary_v2(jsonb) from authenticated;
grant execute on function public.apply_market_reobservation_r2_canary_v2(jsonb) to service_role;

comment on function public.apply_market_reobservation_r2_canary_v2(jsonb) is
  'R2 v2 Yahoo-only atomic four-listing re-observation canary for Issue #187. Exact frozen cohort/key; service_role only; Production application/execution separately approval-bound.';
