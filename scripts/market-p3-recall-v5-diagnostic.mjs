...
import { buildRecallV5Comparison, buildRecallV5Decision, buildRecallV5PreAuditMetrics, buildRecallV5VariantArm, runRecallV5ArmsSequentially } from "../lib/domain/market-p3-recall-v5-diagnostic.js";
...
async function runArm({ name, planner, targets, series, catalog }) {
  ...
  const summary = { ...candidateSummary, request_diagnostics: diagnostics, no_result_variants: Math.max(0, targets.length - candidateSummary.variants_with_results), listing_upserts: 0, observations_created: 0, ingestion_runs_written: 0 };
  const preAuditMetrics = buildRecallV5PreAuditMetrics({ candidateSummary: summary, requestDiagnostics: diagnostics, selectedVariantCount: targets.length });
  let audit;
  try {
    audit = buildSanitizedMarketCandidateAudit({ records: safety.records, queryPlan: queries, catalog, runContext: { mode: "dry-run", source_scope: "planner-apis", run_id: process.env.GITHUB_RUN_ID, head_sha: process.env.GITHUB_SHA, event_name: "workflow_dispatch", bounded_candidate_evidence: true }, summary });
  } catch (error) {
    error.diagnostic_arm = { name, status: "blocked", pre_audit_metrics: preAuditMetrics, request_diagnostics: diagnostics, failure: { reason: "candidate_audit_validation_failed" } };
    throw error;
  }
