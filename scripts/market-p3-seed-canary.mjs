...
if (options.persist !== "true") {
  ...
  try {
    ...
    const result = buildP3SeedCanaryResult({ target, candidate, outcome, before, after, status: "succeeded" });
    ...
    const status = error?.bounded_result?.rollback?.attempted ? error.bounded_result.rollback.verified ? "rolled-back" : "rollback-failed" : "blocked";
    const result = buildP3SeedCanaryResult({ target, candidate, error, before, after, status });
    fs.writeFileSync(path.join(output, "market-p3-seed-canary-result.json"), `${JSON.stringify(result, null, 2)}\n`);
    throw error;
  }
}
