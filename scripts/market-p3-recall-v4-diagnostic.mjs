...
try {
  ...
  Object.assign(results, await runRecallV4ArmsSequentially(arms, async (name, planner) => {
    return runArm({ name, planner, targets, series, catalog });
  }, results));
  ...
  write({ schema_version: 1, status: "blocked", workflow: { run_id: safe(process.env.GITHUB_RUN_ID, 40), head_sha: safeHead(process.env.GITHUB_SHA) }, sample_variant_ids: SAMPLE_IDS, production_counts_before: before, production_counts_after: after, database_writes: 0, zero_delta_verified: before && after ? JSON.stringify(before) === JSON.stringify(after) : false, arms: results, failure: { reason: "p3_recall_v4_diagnostic_failed", ...(error.diagnostic_failure ?? {}) } });
  throw error;
}
