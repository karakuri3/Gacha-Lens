...
import {
  ...
  MARKET_REOBSERVATION_R2_BATCH_SIZE,
  MARKET_REOBSERVATION_R2_RPC,
...
export const R2_OBSERVATION_KEY = "reobs-v1:r2-20260902-01";
export const R2_FROZEN_LISTING_IDS = Object.freeze([
...
export async function runMarketReobservationR2Canary(options = {}) {
  if (options.loadEnv !== false) loadOptionalEnvFile();
  const mode = options.mode ?? "dry-run";
  const headSha = clean(options.headSha ?? process.env.GITHUB_SHA).toLowerCase();
  const expectedMainSha = clean(options.expectedMainSha ?? process.env.R2_EXPECTED_MAIN_SHA).toLowerCase();
  if (!HEAD_SHA.test(headSha) || expectedMainSha !== headSha) throw new Error("R2 runner must be bound to exact current main.");
  ...
  const preflight = await loadR2PreflightEvidence({ listings, readRows, readCount });
  const cohortDigest = buildMarketReobservationR2CohortDigest({ headSha, observationKey: R2_OBSERVATION_KEY, listings });
  const invocation = validateMarketReobservationR2Invocation({
    mode,
    head_sha: headSha,
  ...
  if (!observedAt) throw new Error("R2 observed_at is invalid.");
  ...
  for (const listing of listings) {
    ...
    if (attempts < 1 || attempts > 3) throw new Error("R2 provider attempt count exceeded the per-listing contract.");
    totalAttempts += attempts;
    if (totalAttempts > 12) throw new Error("R2 provider attempt budget exceeded 12 HTTP attempts.");
    ...
    providerEvidence.push(sanitizeProviderEvidence(listing.id, read, plan));
    if (!SUCCESS_OUTCOMES.has(plan.outcome) || !plan.writes?.observation_insert || !plan.writes?.listing_update) {
      throw new Error(`R2 all-or-nothing provider preflight failed for ${listing.id}: ${plan.outcome || "provider_error"}.`);
    }
    plans.push(plan);
  }

  if (plans.length !== MARKET_REOBSERVATION_R2_BATCH_SIZE) throw new Error("R2 provider preflight did not produce four successful plans.");
  const batch = buildMarketReobservationR2RpcBatch({ listings, plans, observationKey: R2_OBSERVATION_KEY });
  const rpcCall = options.rpcCall ?? invokeR2Rpc;
  const rpcResult = validateMarketReobservationR2RpcResult(await rpcCall(batch, options));
  const postwrite = await verifyPostwrite({
  ...
  return {
    schema_version: 1,
    kind: "market_reobservation_r2_canary_result",
...
export async function loadFrozenListings(readRows = fetchRows) {
  const rows = await readRows("market_listings", {
    select: LISTING_SELECT,
    pageSize: MARKET_REOBSERVATION_R2_BATCH_SIZE,
    params: { id: inFilter(R2_FROZEN_LISTING_IDS), order: "id.asc" },
    operationName: "market_reobservation_r2.market_listings",
  });
  const byId = new Map((rows ?? []).map((row) => [row.id, row]));
  if (byId.size !== MARKET_REOBSERVATION_R2_BATCH_SIZE || R2_FROZEN_LISTING_IDS.some((id) => !byId.has(id))) {
    throw new Error("R2 frozen listing cohort is incomplete.");
  }
  return R2_FROZEN_LISTING_IDS.map((id) => byId.get(id));
}

export async function loadR2PreflightEvidence({ listings, readRows = fetchRows, readCount = fetchRowCount } = {}) {
  ...
  const [targetObservations, collidingObservations, unresolvedIssues, allObservations, marketListings, observations, completedSold] = await Promise.all([
    readRows("market_listing_observations", {
      ...
      operationName: "market_reobservation_r2.target_observations",
    }),
    readRows("market_listing_observations", {
      select: "id,listing_id",
      pageSize: 4,
      params: { id: inFilter(observationIds), order: "id.asc" },
      operationName: "market_reobservation_r2.id_collisions",
    }),
    readRows("import_issues", {
      ...
      operationName: "market_reobservation_r2.import_issues",
    }),
    readRows("market_listing_observations", {
      select: "id,listing_id,status",
      operationName: "market_reobservation_r2.all_observations",
    }),
  ...
  if (listings.some((listing) => targetCounts.get(listing.id) !== 1)
    ...
    throw new Error("R2 Production preflight no longer matches the frozen one-observation cohort.");
  }
...
export async function verifyPostwrite({ listingsBefore, batch, preflight, readRows = fetchRows, readCount = fetchRowCount } = {}) {
  ...
  const [listingsAfter, insertedObservations, targetObservations, allObservations, marketListings, observations, completedSold] = await Promise.all([
    readRows("market_listings", {
      ...
      operationName: "market_reobservation_r2.post_listings",
    }),
    readRows("market_listing_observations", {
      select: "id,listing_id,variant_id,series_id,price,status,source,observed_at,raw,created_at",
      pageSize: 4,
      params: { id: inFilter(observationIds), order: "id.asc" },
      operationName: "market_reobservation_r2.post_observations",
    }),
    readRows("market_listing_observations", {
      select: "id,listing_id",
      pageSize: 20,
      params: { listing_id: inFilter(listingIds), order: "id.asc" },
      operationName: "market_reobservation_r2.post_target_counts",
    }),
    readRows("market_listing_observations", {
      select: "id,listing_id,status",
      operationName: "market_reobservation_r2.post_all_observations",
    }),
    readCount("market_listings"),
    readCount("market_listing_observations"),
    readCount("market_listings", { status: "eq.sold" }),
  ...
  if (afterById.size !== 4 || insertedObservations.length !== 4) throw new Error("R2 postwrite rows are incomplete.");
  ...
  for (const id of listingIds) {
    ...
    if (!before || !after || !expected
      ...
      || after.sold_at !== null) {
      throw new Error(`R2 protected listing verification failed for ${id}.`);
    }
  }
  ...
  for (const [key, expectedDelta] of Object.entries(expectedDeltas)) {
    if (afterCounts[key] - preflight.counts[key] !== expectedDelta) throw new Error(`R2 unexpected Production delta for ${key}.`);
  }
  if (listingIds.some((id) => targetCounts.get(id) !== 2)) throw new Error("R2 target observation history did not become exactly two rows each.");
  if (new Set(insertedObservations.map((row) => row.id)).size !== 4
    || observationIds.some((id) => !insertedObservations.some((row) => row.id === id))) {
    throw new Error("R2 deterministic observation rows are missing after commit.");
  }

  return { verified: true, before: preflight.counts, after: afterCounts, deltas: expectedDeltas };
}

export async function invokeR2Rpc(batch, options = {}) {
  if (!Array.isArray(batch) || batch.length !== MARKET_REOBSERVATION_R2_BATCH_SIZE) throw new Error("R2 RPC call requires exactly four entries.");
  ...
  if (!supabaseUrl || !serviceRoleKey) throw new Error("R2 RPC requires Supabase URL and service-role key.");
  const url = new URL(`/rest/v1/rpc/${MARKET_REOBSERVATION_R2_RPC}`, supabaseUrl);
  ...
  if (!response.ok) throw new Error(`R2 atomic RPC failed with HTTP ${response.status}.`);
  const value = await response.json();
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("R2 atomic RPC response is invalid.");
  return value;
}
...
function buildPreparationArtifact({ headSha, cohortDigest, listings, preflight }) {
  ...
  return {
    schema_version: 1,
    kind: "market_reobservation_r2_preflight",
    mode: "dry-run",
    head_sha: headSha,
    cohort_digest: cohortDigest,
    observation_key: R2_OBSERVATION_KEY,
    ...
    required_approval: expectedMarketReobservationR2Approval({ headSha, cohortDigest }),
    provider_requests: 0,
...
async function enforceProviderPacing(provider, lastStartedAtByProvider, options = {}) {
  ...
  if (!minimum) throw new Error("R2 provider pacing contract is unavailable.");
...
async function main() {
  ...
  const artifact = await runMarketReobservationR2Canary(args);
  console.log(JSON.stringify(artifact, null, 2));
}

if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  main().catch((error) => {
    console.error(`market re-observation R2 canary failed closed: ${error?.message || "unknown_error"}`);
    process.exitCode = 1;
  });
}
