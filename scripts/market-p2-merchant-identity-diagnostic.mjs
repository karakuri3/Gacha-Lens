...
const output = path.resolve(options["output-dir"] || "market-p2-merchant-identity-diagnostic");
...
try {
  ...
  const plan = planPriorityTwoDistinctEvidenceQueries(catalog, data.coverageRows, {
    ...
    limit,
    rotationKey: `priority-2-merchant-identity:${process.env.GITHUB_RUN_ID}`,
  });
  ...
  const diagnostic = buildPriorityTwoDistinctEvidenceDiagnostic({
    audit,
    queryPlan: plan.queries,
    existingListings,
  ...
  writeBlocked({ before, after, reason: "priority_2_merchant_identity_diagnostic_failed" });
  throw error;
}
...
function assertInvocation(value) {
  if (process.env.GITHUB_EVENT_NAME !== "workflow_dispatch" || process.env.GITHUB_REF !== "refs/heads/main"
    ...
    throw new Error("Priority 2 merchant identity diagnostic invocation is not bound to current main.");
  }
}
...
function writeDiagnostic(value) {
  fs.writeFileSync(path.join(output, "market-p2-merchant-identity-diagnostic.json"), `${JSON.stringify(value, null, 2)}\n`);
  fs.writeFileSync(path.join(output, "market-p2-merchant-identity-diagnostic.md"), renderPriorityTwoDistinctEvidenceDiagnosticMarkdown(value));
}
...
function writeBlocked({ before, after, reason }) {
  ...
  fs.writeFileSync(path.join(output, "market-p2-merchant-identity-diagnostic.json"), `${JSON.stringify(value, null, 2)}\n`);
  fs.writeFileSync(path.join(output, "market-p2-merchant-identity-diagnostic.md"), `# Priority 2 Merchant Identity Diagnostic\n\n- Status: blocked\n- Database writes: 0\n- Reason: ${reason}\n`);
}
