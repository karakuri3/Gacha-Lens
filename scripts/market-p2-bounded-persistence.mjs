...
import { buildPriorityTwoDistinctEvidenceDiagnostic, PRIORITY_TWO_MERCHANT_IDENTITY_KIND, renderPriorityTwoDistinctEvidenceDiagnosticMarkdown } from "../lib/domain/market-p2-distinct-evidence-diagnostic.js";
...
import {
  ...
  renderMarketP2BoundedArtifactMarkdown,
...
export async function executeMarketP2Bounded({ options = parseOptions(process.argv.slice(2)), output_dir = null } = {}) {
  const output = path.resolve(output_dir || options["output-dir"] || "market-p2-bounded-persistence");
  ...
  let outcome = null;
  try {
    invocation = validateMarketP2BoundedInvocation({
      ...
      approval: process.env.P2_BOUNDED_APPROVAL,
    });
    ...
    const plan = planPriorityTwoDistinctEvidenceQueries(catalog, data.coverageRows, {
      ...
      rotationKey: `priority-2-bounded:${workflow.run_id}`,
    });
    ...
    audit = buildSanitizedMarketCandidateAudit({
      ...
      summary: { ...summary, request_diagnostics: requestDiagnostics, no_result_variants: Math.max(0, plan.selected.length - summary.variants_with_results), listing_upserts: 0, observations_created: 0, ingestion_runs_written: 0, manual_diagnostic: buildPriorityTwoMerchantIdentityReadOnlyDiagnostic() },
    });
    if (audit.result.report_complete !== true || Number(audit.result.truncated_count) !== 0) throw new Error("P2 bounded candidate audit is incomplete.");
    const unchangedCounts = await snapshotCounts();
    if (canonical(before) !== canonical(unchangedCounts)) throw new Error("P2 bounded retrieval changed Production data.");
    diagnostic = buildPriorityTwoDistinctEvidenceDiagnostic({
      audit,
      queryPlan: plan.queries,
      existingListings,
    ...
    selection = selectMarketP2BoundedCandidates({ audit, diagnostic, existingListings, limit: 2 });
    if (invocation.mode === "canary-write") {
      if (!selection.selected.length || selection.selected[0].candidate.candidate_key !== invocation.candidate_key) throw new Error("P2 bounded approved candidate is not the deterministic first eligible target.");
    ...
    writeTerminal(output, buildMarketP2BoundedArtifact({ workflow, mode: invocation.mode, status: invocation.mode === "dry-run" ? "dry-run" : "ready", write_authorized: invocation.write_authorized, selection, before, after: unchangedCounts }));
    ...
    const rows = buildMarketP2BoundedRows({ selected: selection.selected, workflow });
    ...
    assertMarketP2BoundedPrewrite({ rows, selected: selection.selected, existingActiveListings: freshExisting, ...conflicts });
    const store = createStore();
    outcome = await persistMarketP2Bounded({ rows, selected: selection.selected, existingActiveListings: freshExisting, store, beforeCounts: await store.fetchCounts() });
    after = await snapshotCounts();
    writeTerminal(output, buildMarketP2BoundedArtifact({ workflow, mode: invocation.mode, status: "succeeded", write_authorized: true, selection, before, after, outcome }));
    ...
    const rollback = error?.p2_bounded_rollback ?? error?.bounded_result?.rollback;
    const status = rollback?.attempted ? rollback.verified ? "rolled-back" : "rollback-failed" : "blocked";
    writeTerminal(output, buildMarketP2BoundedArtifact({ workflow, mode: invocation?.mode ?? options.mode, status, write_authorized: false, selection, before, after, outcome, rollback, reason_code: "priority_2_bounded_persistence_failed" }));
    throw error;
  }
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) await executeMarketP2Bounded();
...
function assertPlan(plan, limit) {
  ...
  if (!selected.length || selected.length > limit || queries.length !== selected.length
    ...
    || queries.some((entry) => entry.query_profile !== "priority_2_distinct_exact_diagnostic" || entry.fallback_queries?.length > 2)) {
    throw new Error("P2 bounded selection contract is invalid.");
  }
}
...
function createStore() {
  return {
    ...
    fetchP2PrewriteConflicts: loadPrewriteConflicts,
    upsertRows: (table, rows, writeOptions) => upsertRows(table, rows, { ...writeOptions, label: "market-p2-bounded-persistence", allowSchemaFallback: false }),
...
function fetchIn(table, column, values, select) {
  ...
  return fetchRows(table, { select, pageSize: 100, params: { [column]: `in.(${inValues(entries)})`, order: "id.asc" }, operationName: `priority_2_bounded.${table}.${column}` });
}
...
function writeReadOnlyArtifacts(output, audit, diagnostic) {
  ...
  fs.writeFileSync(path.join(output, "market-p2-storefront-diagnostic.json"), `${JSON.stringify(diagnostic, null, 2)}\n`);
  fs.writeFileSync(path.join(output, "market-p2-storefront-diagnostic.md"), renderPriorityTwoDistinctEvidenceDiagnosticMarkdown(diagnostic));
}

function writeTerminal(output, artifact) {
  fs.writeFileSync(path.join(output, "market-p2-bounded-result.json"), `${JSON.stringify(artifact, null, 2)}\n`);
  fs.writeFileSync(path.join(output, "market-p2-bounded-result.md"), renderMarketP2BoundedArtifactMarkdown(artifact));
}
