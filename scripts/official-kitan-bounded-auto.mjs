...
if (command === "gate") gate(); else if (command === "prepare") prepare(); else if (command === "execute") await execute(); else if (command === "finalize") finalize(); else if (command === "scan") scan(); else if (command === "verify") verify(); else throw new Error("Expected gate, prepare, execute, finalize, scan, or verify command.");
...
async function execute() {
  ...
  let after;
  try {
    prepared = readJson(resultFile());
    const report = loadAudit();
    const authorization = authorizeOfficialKitanBoundedAuto({ report, prepared, auditRunId: workflowIdentity().run_id, auditDigest: prepared.audit_digest, headSha: workflowIdentity().head_sha, originMainSha: args["origin-main-sha"], applyDigest: prepared.plan?.selected_apply_contract_digest });
    ...
    before = await adapter.captureCounts();
    bounded = await executeOfficialKitanBoundedAutoTransaction({ adapter, authorization, workflow: workflowIdentity() });
    after = await adapter.captureCounts();
    const result = finalizeOfficialKitanBoundedAutoTransaction({ prepared, bounded, before, after });
    writeResult(result);
    if (result.final_verdict !== "OFFICIAL_KITAN_BOUNDED_AUTO_COMMITTED") throw coded(result.reason_code || result.final_verdict);
  } catch (error) {
    if (bounded) writeResult(finalizeOfficialKitanBoundedAutoTransaction({ prepared, bounded, before, after, postflightReason: error?.reason_code || "official_kitan_bounded_auto_postflight_failed" }));
    else writeBlocked(error?.reason_code || "official_kitan_bounded_auto_execution_failed");
    throw error;
  }
