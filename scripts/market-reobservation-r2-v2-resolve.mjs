...
import {
  ...
  loadR2V2FrozenListings,
} from "./market-reobservation-r2-v2-canary.mjs";

const OBSERVATION_SELECT = "id,listing_id,variant_id,series_id,price,status,source,observed_at,raw,created_at";

export async function resolveMarketReobservationR2V2Commit(options = {}) {
  ...
  for (const listing of listings) {
    ...
    if (!identity.complete
      ...
      return result("inconsistent", listings, observationIds, {
        reason: "frozen_yahoo_listing_identity_invalid",
  ...
  const [r2Observations, targetObservations] = await Promise.all([
    readRows("market_listing_observations", {
      ...
      operationName: "market_reobservation_r2_v2.resolve_ids",
    }),
    readRows("market_listing_observations", {
      select: OBSERVATION_SELECT,
      pageSize: 20,
      params: { listing_id: inFilter(R2_V2_FROZEN_LISTING_IDS), order: "id.asc" },
      operationName: "market_reobservation_r2_v2.resolve_targets",
    }),
  ]);
  ...
  if (rows.length === 0) {
    const allStillOne = R2_V2_FROZEN_LISTING_IDS.every((id) => targetCounts.get(id) === 1);
    return result(allStillOne ? "not_committed" : "inconsistent", listings, observationIds, {
      reason: allStillOne ? "no_r2_v2_observation_ids_and_one_prior_each" : "no_r2_v2_ids_but_target_history_changed",
  ...
  if (rows.length !== 4 || new Set(rows.map((row) => row.id)).size !== 4) {
    return result("inconsistent", listings, observationIds, {
      reason: "partial_or_duplicate_r2_v2_observation_set",
  ...
  for (const listing of listings) {
    ...
    if (!row
      ...
      || raw?.observation_key !== R2_V2_OBSERVATION_KEY
  ...
  if (observationIds.some((id) => !rows.some((row) => row.id === id))) {
    return result("inconsistent", listings, observationIds, {
      reason: "deterministic_r2_v2_observation_id_mismatch",
      target_observation_counts: countsObject(targetCounts),
      write_retry_authorized: false,
    });
  }
  ...
  return result("committed", listings, observationIds, {
    reason: "all_four_r2_v2_deterministic_observations_verified",
    target_observation_counts: countsObject(targetCounts),
    write_retry_authorized: false,
  });
}
...
function result(state, listings, observationIds, extra = {}) {
  return {
    schema_version: 2,
    kind: "market_reobservation_r2_v2_commit_resolution",
    state,
...
function countsObject(counts) {
  return Object.fromEntries(R2_V2_FROZEN_LISTING_IDS.map((id) => [id, counts.get(id) ?? 0]));
}
...
async function main() {
  const artifact = await resolveMarketReobservationR2V2Commit();
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  main().catch((error) => {
    console.error(`market re-observation R2 v2 commit resolution failed closed: ${error?.message || "unknown_error"}`);
    process.exitCode = 1;
  });
}
