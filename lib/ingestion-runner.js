...
export async function runIngestionTask(taskName, options = {}) {
  ...
  const strictDurableLog = ["scheduled_write", "manual_full_write"].includes(process.env.INGESTION_EXECUTION_TYPE);
  const metadata = ingestionRunMetadata(taskName);
  await recordRun({
    id: runId,
    task: taskName,
    status: "running",
  ...
  try {
    ...
    await recordRun({
      ...
      summary: strictDurableLog ? metadata : { ...metadata, result: result.parsedOutput ?? {} },
      error_message: null,
    }, { required: strictDurableLog });
    ...
    try {
      ...
      summary: strictDurableLog ? metadata : { ...metadata, result: error.summary ?? {} },
      error_message: strictDurableLog ? "Ingestion task failed." : String(error.message || error).slice(0, 2000),
      }, { required: strictDurableLog });
    } catch (logError) {
      error.durableRunLogFailure = true;
      error.message = `${error.message}; final durable run log failed`;
      error.cause = logError;
    }
    throw error;
  }
}
...
async function recordRun(row, options = {}) {
  try {
    return await upsertIngestionRun(row, options);
  } catch (error) {
...
function ingestionRunMetadata(task) {
  const allowed = {
    github_run_id: process.env.GITHUB_RUN_ID || null,
    github_run_attempt: process.env.GITHUB_RUN_ATTEMPT || null,
    github_event_name: process.env.GITHUB_EVENT_NAME || null,
    github_head_sha: process.env.GITHUB_SHA || null,
    execution_type: process.env.INGESTION_EXECUTION_TYPE || null,
    mode: process.env.INGESTION_MODE || null,
    task,
    schedule: process.env.INGESTION_SCHEDULE || null,
    preflight_reason_code: process.env.INGESTION_PREFLIGHT_REASON_CODE || null,
  };
