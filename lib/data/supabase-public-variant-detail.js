...
const RELATED_SERIES_PREFILTER_LIMIT = 250;
...
export async function fetchSupabaseScopedRelatedCatalog(supabaseClient, slug, options = {}) {
  ...
  if (parent.franchise) {
    const candidateSeriesIds = await fetchRelatedFranchiseSeriesIds(supabaseClient, parent.franchise);
    if (candidateSeriesIds === null) candidates = candidates.eq("parent.franchise", parent.franchise);
    else if (candidateSeriesIds.length) candidates = candidates.in("series_id", candidateSeriesIds);
...
async function fetchRelatedFranchiseSeriesIds(client, franchise) {
  const result = await client
    .from(TABLE_MAP.series)
    .select("id")
    .eq("franchise", franchise)
    .limit(RELATED_SERIES_PREFILTER_LIMIT + 1);
  if (result.error) {
    throw new Error(`Supabase scoped related series fetch failed: ${result.error.message}`);
  }

  const ids = uniqueValues((result.data ?? []).map((row) => row.id));
  return ids.length > RELATED_SERIES_PREFILTER_LIMIT ? null : ids;
}
