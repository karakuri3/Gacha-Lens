...
const MARKET_LISTING_PUBLIC_SELECT = "id,variant_id,matched_variant_id,series_id,title,listing_type,market_review_type,classification_reason,classification_confidence,price,status,source,source_type,source_url,listed_at,sold_at,last_observed_at,confidence,review_required,created_at,updated_at";
const MARKET_LISTING_SET_SELECT = `${MARKET_LISTING_PUBLIC_SELECT},raw`;

// Public detail/restock rendering does not consume persisted raw payloads. Keep
// the fields used by normalization/scoring, while verified series-level market
// sets retain raw.market_safety through MARKET_LISTING_SET_SELECT.
const TABLE_SELECTS = {
  ...
  restockEvents: "id,variant_id,matched_variant_id,series_id,source_type,source_weight,event_type,event_label,classification_reason,classification_keywords,text,region,shop_name,source_url,reported_at,confidence,review_required,created_at,updated_at",
...
export async function fetchSupabaseScopedRelatedCatalog(supabaseClient, slug, options = {}) {
  ...
  const candidateSeriesIds = await fetchRelatedSeriesIds(supabaseClient, target.series_id, parent);
  let candidateResult = { data: [], error: null };

  if (candidateSeriesIds.length) {
    const candidates = applyPublicVariantFilter(
      supabaseClient.from(TABLE_MAP.variants).select(relationSelect)
    )
      .neq("id", target.id)
      .in("series_id", candidateSeriesIds);

    candidateResult = await candidates
...
async function fetchRelatedSeriesIds(client, targetSeriesId, parent) {
  let scope = null;
  if (parent.franchise) scope = ["franchise", parent.franchise];
  else if (parent.brand) scope = ["brand", parent.brand];
  else if (parent.category) scope = ["category", parent.category];

  if (!scope) return uniqueValues([targetSeriesId]);

  const [column, value] = scope;
  const result = await client
    .from(TABLE_MAP.series)
    .select("id")
    .eq(column, value);
  if (result.error) {
    throw new Error(`Supabase scoped related series fetch failed: ${result.error.message}`);
  }
  return uniqueValues((result.data ?? []).map((row) => row.id));
}
...
async function fetchSignalsForVariantScope(client, variantIds, seriesIds) {
  ...
  const [
    marketByVariant,
    marketByMatchedVariant,
    seriesSetListings,
    xByVariant,
    xByMatchedVariant,
    restockByVariant,
    restockByMatchedVariant,
    stockByVariant,
    stockByMatchedVariant,
  ] = await Promise.all([
    fetchRowsForColumn(client, TABLE_MAP.marketListings, MARKET_LISTING_PUBLIC_SELECT, "variant_id", ids),
    fetchRowsForColumn(client, TABLE_MAP.marketListings, MARKET_LISTING_PUBLIC_SELECT, "matched_variant_id", ids),
    ...
    fetchRowsForColumn(client, TABLE_MAP.stockReports, TABLE_SELECTS.stockReports, "matched_variant_id", ids),
  ]);
  ...
  return {
    ...
    stockReports: dedupeRows([...stockByVariant, ...stockByMatchedVariant]),
  };
}
...
async function fetchSeriesSetListings(client, seriesIds) {
  ...
  return fetchPaged(() => client
    ...
    .select(MARKET_LISTING_SET_SELECT)
