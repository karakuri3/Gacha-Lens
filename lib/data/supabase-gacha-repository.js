...
const TABLE_SELECTS = {
  ...
  marketListings: "id,variant_id,matched_variant_id,series_id,title,listing_type,market_review_type,classification_reason,classification_confidence,price,status,source,source_type,source_url,listed_at,sold_at,confidence,review_required,created_at,updated_at",
...
export async function fetchSupabaseCatalogVariant(supabaseClient, slug) {
  ...
  const signals = await fetchSignalsForCatalog(supabaseClient, catalog);
  return { ...catalog, ...signals, importIssues: [] };
}
...
async function fetchRowsIn(client, tableName, select, column, values) {
  ...
  for (const valueBatch of chunkValues(values, 100)) {
    const { data, error } = await client.from(tableName).select(select).in(column, valueBatch);
    if (error) {
      console.warn(`[supabase-gacha] ${tableName} catalog signals unavailable: ${error.message}`);
      return rows;
    }
    rows.push(...(data ?? []));
  }
