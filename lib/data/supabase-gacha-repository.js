...
export async function fetchSupabaseCatalogPage(supabaseClient, options = {}) {
  ...
  if (options.release === "released" || options.filter === "released") query = query.eq("released", true);
  if (options.release === "upcoming" || options.filter === "upcoming") query = query.eq("released", false);
...
export async function fetchSupabaseParentSeriesCatalogPage(supabaseClient, options = {}) {
  ...
  if (options.release === "released" || options.filter === "released") query = query.eq("is_released", true);
  if (options.release === "upcoming" || options.filter === "upcoming") query = query.eq("is_released", false);
...
export async function fetchSupabaseCategoryCatalog(supabaseClient) {
  ...
  const categories = await Promise.all(names.map(async (name) => {
    const [itemCount, upcomingCount, imageResult] = await Promise.all([
      countPublicVariantsByCategory(supabaseClient, name),
      countPublicVariantsByCategory(supabaseClient, name, false),
...
export async function fetchSupabaseUpcomingMonths(supabaseClient) {
  if (!supabaseClient) return [];
...
export async function fetchSupabaseParentSeriesCategoryCatalog(supabaseClient) {
  if (!supabaseClient) return [];
  const rows = await fetchTable(supabaseClient, TABLE_MAP.series, "id,category,image_url,is_released");
  return collectParentSeriesCategoryCatalog(rows);
}

export function collectParentSeriesCategoryCatalog(rows = []) {
  ...
  for (const row of rows) {
    ...
    if (!row.is_released) group.upcoming_count += 1;
...
export async function fetchSupabaseUpcomingParentSeriesMonths(supabaseClient) {
  if (!supabaseClient) return [];
...
async function countPublicVariantsByCategory(client, category, released) {
  let query = applyPublicVariantFilter(
    client.from(TABLE_MAP.variants).select("id,parent:series!inner(category)", { count: "exact", head: true })
  ).eq("parent.category", category);
  if (typeof released === "boolean") query = query.eq("released", released);
  const { count, error } = await query;
