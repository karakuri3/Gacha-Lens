...
export async function fetchSupabaseCatalogPage(supabaseClient, options = {}) {
  ...
  query = query.neq("variant_type", "provisional");
...
export async function fetchSupabaseCatalogCounts(supabaseClient) {
  ...
  const [series, all, released, upcoming] = await Promise.all([
    ...
    countRows(supabaseClient, TABLE_MAP.variants, [{ column: "variant_type", operator: "neq", value: "provisional" }]),
    countRows(supabaseClient, TABLE_MAP.variants, [{ column: "variant_type", operator: "neq", value: "provisional" }, { column: "released", value: true }]),
    countRows(supabaseClient, TABLE_MAP.variants, [{ column: "variant_type", operator: "neq", value: "provisional" }, { column: "released", value: false }]),
  ]);
...
export async function fetchSupabaseCategoryCatalog(supabaseClient) {
  ...
  const result = await fetchAllRows(supabaseClient, TABLE_MAP.variants, select);
