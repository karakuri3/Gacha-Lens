...
export async function fetchBoundedSeriesObserverRows(supabaseClient, options = {}) {
  ...
  const cutoff = observerCutoff(options.today);
  const rows = [];
  ...
  for (let offset = 0; offset <= maxRows; offset += pageSize) {
    ...
    if (cutoff) {
      query = query.or(
        `release_date.gte.${cutoff.date},and(release_date.is.null,release_month.gte.${cutoff.month})`
      );
    }
...
function observerCutoff(value) {
  const today = String(value || "").trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(today)) return null;
  const parsed = new Date(`${today}T00:00:00.000Z`);
  if (!Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0, 10) !== today) return null;
  parsed.setUTCDate(parsed.getUTCDate() - 180);
