...
export function normalizeRecordShape(records = {}, { now = new Date() } = {}) {
  const stored = normalizeStoredRecordShape(records);
  const series = stored.series.map((record) => ({
    ...record,
    is_released: effectiveReleaseState(record, { now }),
  }));
  const seriesById = new Map(series.map((record) => [String(record.id || ""), record]));
  const variants = stored.variants.map((record) => ({
    ...record,
    released: effectiveReleaseState(record, {
      parent: seriesById.get(String(record.series_id || "")) ?? null,
      now,
    }),
  }));
...
export function normalizeStoredRecordShape(records = {}) {
  return {
    ...
    ingestionRuns: asArray(records.ingestionRuns),
    communityReports: asArray(records.communityReports),
  };
}
...
export function mergeGachaRecords(...recordSets) {
  return recordSets.reduce(
    (merged, records) => {
      const normalized = normalizeStoredRecordShape(records);
      return {
        ...
        marketObservations: mergeById(merged.marketObservations, normalized.marketObservations),
        ...
        ingestionRuns: mergeById(merged.ingestionRuns, normalized.ingestionRuns),
        communityReports: mergeById(merged.communityReports, normalized.communityReports),
      };
    },
    { ...EMPTY_GACHA_RECORDS }
  );
