...
function getSuggestedAction(issue) {
  if (issue.issue_type === "community_report_pending") return "内容と確認URLを確認し、承認または却下する";
...
function getIssueGroup(issue) {
  ...
  if (tableName === "community_reports") return "Community submissions";
  return "Other";
}

function getIssuePriority(issue) {
  if (issue.table_name === "community_reports") return "medium";
