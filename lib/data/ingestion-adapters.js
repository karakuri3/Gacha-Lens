...
import { mergeGachaRecords, normalizeStoredRecordShape } from "./gacha-repository";
...
export function ingestOfficialSchedule(rawSchedule = []) {
  return ingestOfficialRows(rawSchedule, "official_schedule");
}

export function ingestOfficialProducts(rawProducts = []) {
  return ingestOfficialRows(rawProducts, "official_product");
}

function ingestOfficialRows(rows = [], tableName) {
  const series = [];
  const variants = [];
  const importIssues = [];
  ...
  for (const raw of asArray(rows)) {
    ...
    if (!normalizedSeries.id || !normalizedSeries.name) {
      importIssues.push(createImportIssue(tableName, raw, IMPORT_ISSUE_TYPES.INVALID_RECORD));
      continue;
    }
    ...
    const rawVariants = asArray(raw.variants || raw.items || raw.lineup || raw.line_up);
    if (!rawVariants.length) {
      importIssues.push(createImportIssue(tableName, raw, IMPORT_ISSUE_TYPES.MISSING_VARIANTS, seriesRecord.id));
    }

    for (const variantRaw of rawVariants) {
      const variantRecord = normalizeOfficialVariant(variantRaw, seriesRecord);
      variants.push(variantRecord);
      if (variantRecord.review_required) {
        importIssues.push(createImportIssue("variants", variantRaw, IMPORT_ISSUE_TYPES.INVALID_RECORD, variantRecord.id));
      }
    }
  }
...
export function ingestMarketListings(rawListings = [], catalog = {}) {
  const normalizedCatalog = normalizeStoredRecordShape(catalog);
...
function createImportIssue(tableName, raw, issueType, parentId = "") {
  return {
    ...
    source_url: raw.official_url || raw.url || raw.product_url || "",
    raw,
    resolved: false,
    note: issueType === IMPORT_ISSUE_TYPES.MISSING_VARIANTS
      ? "公式データに variants/lineup が未入力です。単品マスタを確認してください。"
      : "投入前に形式確認が必要です。",
  };
}
