...
export function buildOpsHealthReport(dataModel = {}, nowInput = new Date()) {
  ...
  const communityReports = dataModel.communityReports ?? [];
  ...
  const pipelines = [
    ...
    communityPipeline({ communityReports }),
  ];
...
function communityPipeline({ communityReports }) {
  const pending = communityReports.filter((report) => report.status === "pending").length;
  const approved = communityReports.filter((report) => report.status === "approved").length;
  return {
    key: "community",
    label: "Community reports",
    status: pending > 30 ? "warning" : "ok",
    summary: pending ? "price and stock reports are waiting for review" : "community review queue is clear",
    metrics: [metric("reports", communityReports.length), metric("pending", pending), metric("approved", approved)],
  };
}
