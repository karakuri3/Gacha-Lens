...
export const PRIORITY_TWO_MERCHANT_IDENTITY_KIND = "priority_2_merchant_identity_read_only";

export function buildPriorityTwoDistinctEvidenceDiagnostic({ audit = {}, queryPlan = [], existingListings = [], candidateStorefronts = new Map(), before = null, after = null, kind = PRIORITY_TWO_DISTINCT_EVIDENCE_KIND } = {}) {
...
export function validatePriorityTwoDistinctEvidenceDiagnostic(value) {
  if (!value || value.schema_version !== 1 || ![PRIORITY_TWO_DISTINCT_EVIDENCE_KIND, PRIORITY_TWO_MERCHANT_IDENTITY_KIND].includes(value.kind)
...
export function renderPriorityTwoDistinctEvidenceDiagnosticMarkdown(value) {
  ...
  const merchantDiagnostic = value.kind === PRIORITY_TWO_MERCHANT_IDENTITY_KIND;
  const lines = [
    merchantDiagnostic ? "# Priority 2 Merchant Identity Diagnostic" : "# Priority 2 Distinct Evidence Diagnostic",
    "",
    "- Read-only: true",
