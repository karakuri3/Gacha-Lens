...
export function buildSeriesCompleteSetPreview(listing = {}, assessment = {}, catalog = {}) {
  ...
  return {
    ...
    source_listing_id: text(listing.raw?.source_listing_id || listing.raw?.itemCode || listing.raw?.code || listing.id),
...
export function evaluateSeriesCompleteSetCandidates({ records = [], queryPlan = [], catalog = {} } = {}) {
  ...
  for (const record of records) {
    ...
    const query = resolveCanonicalPlannerQuery(record, queryByText);
...
function resolveCanonicalPlannerQuery(record, queryByText) {
  const rawQuery = record?.raw?.query;
  const queryText = typeof rawQuery === "string"
    ? rawQuery
    : rawQuery?.query || record?.raw?.keyword;
  return queryByText.get(normalize(queryText));
}
