...
export const RAKUTEN_AFFILIATE_PROVENANCE_CONTRACT = "item_search_20260701_item_code_join";
...
export function getRakutenAffiliateDestination(item = {}) {
  ...
  const destination = sanitizeRakutenAffiliateProvenance({
    ...
    sourceDocumentation: payload.source_documentation,
  });
  return {
...
export function isSafeRakutenAffiliateListing(listing = {}) {
  ...
  const payload = marketplacePayload(listing);
  if (String(payload.provider || "").toLowerCase() !== "rakuten_ichiba") return false;
  if (!String(payload.itemCode || payload.source_listing_id || "").trim()) return false;
...
export function sanitizeRakutenAffiliateProvenance({
  ...
  if (affiliateUrlContract !== RAKUTEN_AFFILIATE_PROVENANCE_CONTRACT) return null;
