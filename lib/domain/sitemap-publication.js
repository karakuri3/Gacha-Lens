...
export function buildSitemapIndexXml(paths = [], { siteUrl } = {}) {
  const origin = validHttpsOrigin(siteUrl);
  if (!origin) throw new Error("Sitemap index requires an absolute HTTPS site URL.");
  const values = [...new Set((Array.isArray(paths) ? paths : []).map(text).filter(Boolean))];
  if (values.length > MAX_OBSERVER_SITEMAP_URLS) {
    throw new Error(`Sitemap index exceeds ${MAX_OBSERVER_SITEMAP_URLS} entries`);
  }
  const sitemaps = values.map((path) => {
    if (!path.startsWith("/")) throw new Error("Sitemap index paths must be absolute-path references.");
