...
export async function runRecallV4ArmsSequentially(arms, execute) {
  const results = {};
  for (const [name, planner] of arms) results[name] = await execute(name, planner);
  return results;
}
...
export function buildRecallV4Decision(arms, comparison, zeroDeltaVerified) {
  ...
  return { retrieval_delta_v3_vs_v2: retrievalV3, retrieval_delta_v4_vs_v3: retrievalV4, accepted_unique_delta_v3_vs_v2: acceptedV3, accepted_unique_delta_v4_vs_v3: acceptedV4, v4_retrieval_win_count: comparison.flatMap((entry) => entry.v4_only_records).length, v4_accepted_win_count: comparison.flatMap((entry) => entry.v4_only_accepted_records).length, v4_retrieval_but_rejected_count: rejected.length, top_v4_rejection_reasons: reasons, provider_errors: providerErrors, decision_label: label };
}
