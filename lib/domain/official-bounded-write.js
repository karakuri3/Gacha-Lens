...
export async function executeOfficialBoundedTransaction({ adapter, authorization, workflow = {} } = {}) {
  ...
  try {
    ...
    for (const operation of operations) {
      ...
      assertPrecondition(operation, current);
    ...
    for (const operation of operations) {
      const current = await adapter.readRow(operation.table, operation.id, { lock: true });
      assertExpectedRow(operation, current, beforeRows.get(operationKey(operation)));
    }
    assertWriteCounts(operations, writeCounts);
    ...
    return buildOfficialBoundedResult({
      ...
      beforeDigests,
      afterDigests: {},
      writeCounts: { series: 0, variants: 0, restock_events: 0 },
      databaseWrites: committed ? Object.values(writeCounts).reduce((sum, value) => sum + value, 0) : 0,
      transactionState: committed ? "committed_post_verify_failed" : (rollbackVerified ? "rolled_back" : "rollback_failed"),
      rollbackAttempted: began && !committed,
      rollbackVerified,
      finalVerdict: began && !committed && rollbackVerified
...
export function buildOfficialBoundedReadyResult({ workflow = {}, authorization } = {}) {
  ...
  return validateOfficialBoundedResult({
    ...
    after_digests: {},
    operations: { series: 0, variants: 0, restock_events: 0 },
    planned_operations: summarizeOperations(candidateOperations(authorization.candidate)),
    committed_operations: emptyOperationCounts(),
    database_writes: 0,
    deletes: 0,
    cleanup_operations: 0,
...
function candidateOperations(candidate) {
  const contract = candidate.apply_contract;
  const operations = [contract.series, ...asArray(contract.variants), ...(contract.restock_event ? [contract.restock_event] : [])];
  if (operations.length > 1 + OFFICIAL_BOUNDED_CANARY_LIMITS.max_variants + OFFICIAL_BOUNDED_CANARY_LIMITS.max_restock_events) {
    throw boundedError("official_bounded_operation_cap_exceeded");
  }
  return operations;
}

function assertPrecondition(operation, current) {
...
function assertExpectedRow(operation, current, before) {
...
function assertWriteCounts(operations, actual) {
...
function buildOfficialBoundedResult({
  ...
  return validateOfficialBoundedResult({
    ...
    committed_operations: transactionState.startsWith("committed")
      ? summarizeOperations(candidateOperations(authorization.candidate), { writesOnly: true })
