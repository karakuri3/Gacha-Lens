...
export function validateManualMarketAuditReport(report, options = {}) {
  ...
  if (options.expectedPriority != null && selected.some((entry) => Number(entry.priority) !== Number(options.expectedPriority))) {
    throw new Error("Manual market audit selection priority does not match the fixed contract.");
  }
