...
import { sanitizeCanaryQueryReplay } from "./market-approved-query-replay.js";
...
export function validateApprovedMarketAudit(report, options = {}) {
  ...
  if (isNonAuthoritativeManualMarketAudit(report)) {
    throw new Error("Diagnostic-only manual market audits cannot authorize canary or write operations.");
  }
...
export function assertApprovedCanaryCandidatesMatch(approved, current, candidateKeys) {
  const diagnostics = buildCanaryAuditMismatchDiagnostics(approved, current, candidateKeys);
  try {
    validateMarketCandidateAudit(approved);
    validateMarketCandidateAudit(current);
    if (approved.schema_version !== current.schema_version) {
      throw new Error("Current market audit schema does not match the approved audit.");
    }
    if (approved.mode !== "dry-run" || current.mode !== "dry-run") {
      throw new Error("Canary matching requires dry-run audits.");
    }
    if (approved.source_scope !== "planner-apis" || current.source_scope !== "planner-apis") {
      throw new Error("Canary matching requires planner-apis audits.");
    }
    assertAuditSelectionValid(approved);
    assertAuditSelectionValid(current);
    if (canonical(approved.selection) !== canonical(current.selection)) {
      throw new Error("Current market audit selection does not match the approved audit.");
    }
    assertCompleteZeroWriteAudit(approved, "Approved");
    assertCompleteZeroWriteAudit(current, "Current");

    const requestedKeys = parseCanaryCandidateKeys(candidateKeys);
    const approvedByKey = indexAuditCandidates(approved);
    const currentByKey = indexAuditCandidates(current);
    for (const key of requestedKeys) {
      const approvedCandidate = approvedByKey.get(key);
      const currentCandidate = currentByKey.get(key);
      if (!approvedCandidate || !currentCandidate) {
        throw new Error(`Requested candidate ${key} is missing from a canary audit.`);
      }
      assertCanaryCandidateEligible(approvedCandidate, key, "approved");
      assertCanaryCandidateEligible(currentCandidate, key, "current");
      if (canonical(comparableCanaryCandidate(approvedCandidate)) !== canonical(comparableCanaryCandidate(currentCandidate))) {
        throw new Error(`Requested candidate ${key} does not match the approved audit.`);
      }
    }
    return true;
  } catch (error) {
    if (error && typeof error === "object" && !error.canaryAuditMismatch) {
      error.canaryAuditMismatch = diagnostics;
    }
    throw error;
  }
}

export function buildCanaryAuditMismatchDiagnostics(approved, current, candidateKeys) {
  try {
    const approvedCandidates = safeAuditCandidates(approved);
    const currentCandidates = safeAuditCandidates(current);
    const approvedByKey = new Map(approvedCandidates.map((candidate) => [candidate.candidate_key, candidate]));
    const currentByKey = new Map(currentCandidates.map((candidate) => [candidate.candidate_key, candidate]));
    const approvedKeys = sortedCandidateKeys(approvedCandidates.map((candidate) => candidate.candidate_key));
    const currentKeys = sortedCandidateKeys(currentCandidates.map((candidate) => candidate.candidate_key));
    const requestedKeys = sortedCandidateKeys(
      (Array.isArray(candidateKeys) ? candidateKeys : String(candidateKeys ?? "").split(",")),
    );
    const requested = new Set(requestedKeys);
    const missingRequested = requestedKeys.filter((key) => !approvedByKey.has(key) || !currentByKey.has(key));
    const changedRequested = requestedKeys.filter((key) => {
      const approvedCandidate = approvedByKey.get(key);
      const currentCandidate = currentByKey.get(key);
      return approvedCandidate
        && currentCandidate
        && canonical(comparableCanaryCandidate(approvedCandidate)) !== canonical(comparableCanaryCandidate(currentCandidate));
    });
    return {
      selection_changed: canonical(approved?.selection) !== canonical(current?.selection),
      approved_candidate_keys: approvedKeys,
      current_candidate_keys: currentKeys,
      requested_candidate_keys: requestedKeys,
      missing_requested_keys: missingRequested,
      changed_requested_keys: changedRequested,
      unrelated_added_keys: currentKeys.filter((key) => !approvedByKey.has(key) && !requested.has(key)),
      unrelated_removed_keys: approvedKeys.filter((key) => !currentByKey.has(key) && !requested.has(key)),
    };
  } catch {
    return emptyCanaryAuditMismatch();
  }
}
...
export function buildSanitizedCanaryFailureResult(input = {}) {
  ...
  const auditMismatch = sanitizeCanaryAuditMismatch(input.auditMismatch);
  const queryReplay = sanitizeCanaryQueryReplay(input.queryReplay);
  return {
    schema_version: 1,
    ...
    ...(auditMismatch ? { audit_mismatch: auditMismatch } : {}),
    ...(queryReplay ? { query_replay: queryReplay } : {}),
  };
}
...
export function renderMarketCanaryResultMarkdown(result = {}) {
  ...
  const auditMismatch = sanitizeCanaryAuditMismatch(result.audit_mismatch);
  const queryReplay = sanitizeCanaryQueryReplay(result.query_replay);
  const lines = [
    ...
    ...(auditMismatch ? [
      "## Audit mismatch",
      "",
      `- Selection changed: ${auditMismatch.selection_changed}`,
      `- Approved candidate keys: ${renderCandidateKeys(auditMismatch.approved_candidate_keys)}`,
      `- Current candidate keys: ${renderCandidateKeys(auditMismatch.current_candidate_keys)}`,
      `- Requested candidate keys: ${renderCandidateKeys(auditMismatch.requested_candidate_keys)}`,
      `- Missing requested keys: ${renderCandidateKeys(auditMismatch.missing_requested_keys)}`,
      `- Changed requested keys: ${renderCandidateKeys(auditMismatch.changed_requested_keys)}`,
      `- Unrelated added keys: ${renderCandidateKeys(auditMismatch.unrelated_added_keys)}`,
      `- Unrelated removed keys: ${renderCandidateKeys(auditMismatch.unrelated_removed_keys)}`,
      "",
    ] : []),
    ...(queryReplay ? [
      "## Query replay",
      "",
      `- Source: ${queryReplay.source}`,
      `- Approved selected count: ${queryReplay.approved_selected_count}`,
      `- Replayed query count: ${queryReplay.replayed_query_count}`,
      `- Catalog identity match: ${queryReplay.catalog_identity_match}`,
      `- Query safety match: ${queryReplay.query_safety_match}`,
      "",
    ] : []),
...
function comparableCanaryCandidate(candidate) {
  return {
    candidate_key: candidate?.candidate_key,
    source: {
      provider: candidate?.source?.provider,
      listing_id: candidate?.source?.listing_id,
      public_url: candidate?.source?.public_url,
    },
    listing: {
      title: candidate?.listing?.title,
      price: candidate?.listing?.price,
      status: candidate?.listing?.status,
      listing_type: candidate?.listing?.listing_type,
    },
    target: {
      variant_id: candidate?.target?.variant_id,
      variant_slug: candidate?.target?.variant_slug,
      variant_name: candidate?.target?.variant_name,
      series_id: candidate?.target?.series_id,
      series_slug: candidate?.target?.series_slug,
      series_name: candidate?.target?.series_name,
      search_query: candidate?.target?.search_query,
    },
...
function assertAuditSelectionValid(report) {
  ...
  if (
    ...
    throw new Error("Market audit selection totals do not match.");
  }
}

function assertCompleteZeroWriteAudit(report, label) {
  if (report.result?.report_complete !== true) throw new Error(`${label} market audit is incomplete.`);
  if (Number(report.result?.truncated_count) !== 0) throw new Error(`${label} market audit is truncated.`);
  const candidates = safeAuditCandidates(report);
  const acceptedCount = candidates.filter((candidate) => candidate.assessment?.accepted === true).length;
  const reviewCount = candidates.filter((candidate) => candidate.assessment?.review_required === true).length;
  if (
    Number(report.result?.candidate_count) !== candidates.length
    || Number(report.result?.accepted_count) !== acceptedCount
    || Number(report.result?.review_count) !== reviewCount
    || acceptedCount + reviewCount !== candidates.length
  ) {
    throw new Error(`${label} market audit candidate totals do not match.`);
  }
  if (Object.values(report.database_writes ?? {}).some((value) => Number(value) !== 0)) {
    throw new Error(`${label} market audit must declare zero database writes.`);
  }
}

function indexAuditCandidates(report) {
  const candidates = safeAuditCandidates(report);
  const byKey = new Map();
  for (const candidate of candidates) {
    const key = requiredText(candidate?.candidate_key);
    if (!CANDIDATE_KEY.test(key) || byKey.has(key)) {
      throw new Error("Market audit candidate keys must be unique 16-character lowercase hex.");
    }
...
function assertCanaryCandidateEligible(candidate, key, label) {
  if (
    candidate.assessment?.accepted !== true
    || candidate.assessment?.review_required !== false
    || candidate.assessment?.reason !== ACCEPTED_REASON
    ...
    throw new Error(`Requested candidate ${key} is not accepted in the ${label} audit.`);
  }
}

function safeAuditCandidates(report) {
  return Array.isArray(report?.candidates) ? report.candidates.filter(isPlainObject) : [];
}

function sortedCandidateKeys(values) {
  return [...new Set(values.map(requiredText).filter((key) => CANDIDATE_KEY.test(key)))]
    .sort((left, right) => left.localeCompare(right, "en"));
}

function emptyCanaryAuditMismatch() {
  return {
    ...
    changed_requested_keys: [],
    unrelated_added_keys: [],
    unrelated_removed_keys: [],
  };
}

function sanitizeCanaryAuditMismatch(value) {
  if (!isPlainObject(value)) return null;
  const clean = emptyCanaryAuditMismatch();
  clean.selection_changed = value.selection_changed === true;
  for (const key of Object.keys(clean).filter((key) => key.endsWith("_keys"))) {
    clean[key] = sortedCandidateKeys(Array.isArray(value[key]) ? value[key] : []);
  }
  return clean;
}
