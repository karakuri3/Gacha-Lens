...
export function buildMarketDepthCollectorDryRun({ selection = {}, rows = null, retrieval = {}, generatedAt = new Date() } = {}) {
  ...
  return {
    ...
    projected_writes: sanitizeProjectedWrites(projected),
...
function assertSelectionIntegrity(selection) {
  ...
  const listingIds = selection.selected.map(candidateIdentity).map((identity) => identity?.listing_id ?? null);
...
function sanitizeProjectedWrites(value) {
  return {
