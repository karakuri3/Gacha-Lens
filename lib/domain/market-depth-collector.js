...
export function selectMarketDepthCandidates(candidates = [], options = {}) {
  ...
  if (!targetVariantId) throw new Error("Depth Collector requires an explicit targetVariantId.");
  ...
  return {
    ...
    target_series_id: targetSeriesId || selected[0]?.target?.series_id || null,
...
function evaluateCandidate(candidate, context) {
  ...
  if (context.targetSeriesId && text(candidate?.target?.series_id, 180) !== context.targetSeriesId) {
