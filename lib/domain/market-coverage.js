...
import { effectiveReleaseState, releaseDateAtJstStart } from "./release-state.js";
...
export function classifyVariantMarketCoverage({
  ...
  const released = effectiveReleaseState(variant, { parent: parentSeries, now: nowDate });
...
export function selectMarketCollectionTargets(rows = [], options = {}) {
  ...
  const excludedVariantIds = normalizeStringSet(options.excludedVariantIds);
  const maxVariantsPerSeries = normalizeSeriesCap(options.maxVariantsPerSeries);
  const constrainedSelection = excludedVariantIds.size > 0 || Number.isFinite(maxVariantsPerSeries);
  ...
  const rotationBucket = normalizedRotationBucket(options.rotationKey) || dayBucket;
  candidates.sort((a, b) =>
    a.priority - b.priority
    || attemptTime(a.lastCollectionAttemptAt) - attemptTime(b.lastCollectionAttemptAt)
    || rotationRank(a.variantId, rotationBucket) - rotationRank(b.variantId, rotationBucket)
    || a.variantId.localeCompare(b.variantId, "ja")
  );

  const dedupedCandidates = dedupeBy(candidates, (row) => row.variantId);
  const selected = [];
  const selectedVariantIds = new Set();
  const selectedSeriesCounts = new Map();
  let skippedExcludedVariants = 0;
  let skippedSeriesCap = 0;
  for (const row of dedupedCandidates) {
    if (selectedVariantIds.has(row.variantId)) continue;
    selectedVariantIds.add(row.variantId);
    if (excludedVariantIds.has(row.variantId)) {
      skippedExcludedVariants += 1;
      continue;
    }
    const seriesKey = row.seriesId || `missing:${row.variantId}`;
    const seriesCount = selectedSeriesCounts.get(seriesKey) ?? 0;
    if (seriesCount >= maxVariantsPerSeries) {
      skippedSeriesCap += 1;
      continue;
    }
    selected.push(row);
    selectedSeriesCounts.set(seriesKey, seriesCount + 1);
  ...
  if (constrainedSelection) {
    Object.assign(summary, {
      skipped_excluded_variants: skippedExcludedVariants,
      skipped_series_cap: skippedSeriesCap,
      distinct_series_selected: selectedSeriesCounts.size,
      max_variants_per_series: Number.isFinite(maxVariantsPerSeries) ? maxVariantsPerSeries : null,
    });
  }
...
function releaseDateValue(variant, parentSeries) {
  return releaseDateAtJstStart(variant.release_date ?? parentSeries?.release_date);
}
...
function normalizedRotationBucket(value) {
  return String(value ?? "").normalize("NFKC").trim().slice(0, 160);
}
...
function normalizeSeriesCap(value) {
  if (value == null || value === Infinity) return Infinity;
  const number = Number(value);
  return Number.isInteger(number) && number >= 1 ? Math.min(200, number) : Infinity;
}
