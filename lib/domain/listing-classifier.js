...
const TYPE_RULES = [
  {
    ...
    confidence: 0.88,
    keywords: ["シークレット", "シクレ", "secret", "sec", "隠し", "隠れ", "レア枠"],
  },
  {
  ...
  },
  {
    ...
    confidence: 0.72,
    keywords: ["2種", "3種", "4種", "2点", "3点", "4点", "二種", "三種", "四種", "一部", "セット", "よりどり"],
  },
  {
...
const TITLE_ALIASES = [
  ...
  { aliases: ["パンどろぼうへんしん", "へんしんだ"], names: ["パンどろぼう（へんしんだ！）"] },
...
export function resolveVariantsFromListing(rawListing = {}, catalog = {}, options = {}) {
  ...
  if (explicitId) {
    ...
    const additional = variants.filter((variant) => {
      ...
      if (!options.allowFranchiseVariant && isFranchiseNameVariant(variant, catalog)) return false;
...
function shouldAllowFranchiseVariantForSingle(value = "", catalog = {}) {
  if (hasSingleKeyword(value)) return true;
  return hasFranchiseVariantMentionOutsideSeries(value, catalog);
}

function hasFranchiseVariantMentionOutsideSeries(value = "", catalog = {}) {
  const title = normalize(expandAliases(value));
  if (!title) return false;

  return (catalog.variants ?? []).some((variant) => {
    if (!isFranchiseNameVariant(variant, catalog)) return false;

    const variantTerms = getVariantTerms(variant, catalog, { allowFranchiseVariant: true }).filter(Boolean);
    if (!variantTerms.some((term) => title.includes(term))) return false;

    const parent = (catalog.series ?? []).find((series) => series.id === variant.series_id);
    if (!parent) return false;

    const reducedTitle = [parent.name, parent.slug]
      .filter(Boolean)
      .map(normalize)
      .sort((a, b) => b.length - a.length)
      .reduce((current, term) => (term ? current.replaceAll(term, "") : current), title);

    return variantTerms.some((term) => term && reducedTitle.includes(term));
  });
}

function getSeriesTerms(series) {
  return [series.name, series.slug, series.franchise].filter(Boolean).map(normalize).filter(Boolean);
}

function isFranchiseNameVariant(variant, catalog = {}) {
  const parent = (catalog.series ?? []).find((series) => series.id === variant.series_id);
  return parent && normalize(parent.franchise) === normalize(variant.name || variant.variant_name);
}
