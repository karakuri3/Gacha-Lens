...
export function classifyListingTitleDetailed(title = "") {
  ...
  const completeMatch = normalized.match(/全\d+種|全部揃(?:って)?(?:ます|います)?|フルコンプ/);
  if (completeMatch) return classification(LISTING_TYPES.COMPLETE_SET, "full_set_pattern", { matched_keywords: [completeMatch[0]] }, 0.94);
  const numberedSetMatch = normalized.match(/(?:ノーマル)?\d+種セット|\d+点セット/);
  if (numberedSetMatch) return classification(LISTING_TYPES.PARTIAL_SET, "numbered_set_pattern", { matched_keywords: [numberedSetMatch[0]] }, 0.86);

  for (const rule of TYPE_RULES) {
    const keyword = rule.keywords.find((entry) => normalized.includes(normalize(entry)));
    if (rule.type === LISTING_TYPES.RARE_SINGLE && /レア(?:は)?(?:含まれ|含み)ません/.test(normalized)) continue;
