...
export function decodeCategoryDiscoveryParam(value) {
  // Preserve the raw App Router value. Runtime-specific percent-encoding differences are
  // handled by categoryDiscoveryLookupCandidates so a literal percent escape can win first.
...
export function categoryDiscoveryHref(name) {
  ...
  return segment ? `/categories/${segment}` : "/categories";
}
