...
export function assessMarketCandidate(listing = {}, query = {}, catalog = {}) {
  ...
  const variantTerms = [targetVariant.name].map(normalize).filter((term) => term.length >= 2);
  ...
  const auditChecks = {
    variantEvidencePresent: matchedIds.includes(targetVariant.id) && variantTerms.some((term) => normalizedTitle.includes(term)),
...
function classifyStrictCandidate(listing, { catalog, parentSeries }) {
  ...
  const scopedVariants = (catalog.variants ?? []).filter((variant) =>
    variant.id
    && variant.name
    && variant.series_id === parentSeries.id
    && String(variant.variant_type || "").toLowerCase() !== "provisional"
  );
  const titleMatches = scopedVariants.flatMap((variant) =>
    findTermOccurrences(title, normalize(variant.name)).map((occurrence) => ({
      ...occurrence,
      variantId: variant.id,
    }))
  );
  const effectiveMatches = titleMatches.filter((match) => !titleMatches.some((candidate) =>
    candidate.term.length > match.term.length
    && candidate.start <= match.start
    && candidate.end >= match.end
  ));
  const matchedVariantIds = [...new Set(effectiveMatches.map((match) => match.variantId))];
...
function findTermOccurrences(title, term) {
  if (term.length < 2) return [];
  const occurrences = [];
  let start = title.indexOf(term);
  while (start >= 0) {
    occurrences.push({ term, start, end: start + term.length });
    start = title.indexOf(term, start + 1);
  }
  return occurrences;
}
