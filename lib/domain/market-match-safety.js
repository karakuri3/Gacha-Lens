...
export function assessMarketCandidate(listing = {}, query = {}, catalog = {}) {
  ...
  if (
    auditChecks.explicitLabelOtherVariantMatch
    && (auditChecks.explicitLabelTargetMatch || targetTextEvidence)
  ) {
  ...
  if (auditChecks.explicitLabelPresent && !auditChecks.explicitLabelTargetMatch) {
    return review("target_variant_not_confirmed", classification, auditChecks);
  }
...
function classifyStrictCandidate(listing, { catalog, parentSeries }) {
  ...
  const titleMatches = scopedVariants.flatMap((variant) =>
    findVariantNameOccurrences(listing.title || listing.name, variant.name).map((occurrence) => ({
      ...occurrence,
