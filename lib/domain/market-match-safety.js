...
export function buildFormalSeriesByFranchise(series = []) {
  const index = new Map();
  for (const parent of series ?? []) {
    if (!parent?.id || !parent.name) continue;
    const franchise = normalize(parent.franchise);
    if (franchise.length < 3) continue;
    const entries = index.get(franchise) ?? [];
    entries.push(parent);
    index.set(franchise, entries);
  }
  return index;
}

export function buildCatalogParentVariantIdentityKey(parentSeriesName, variantName) {
  const parent = normalize(parentSeriesName);
  const variant = normalize(variantName);
  return parent && variant ? `${parent}\u0000${variant}` : "";
}

export function buildFormalParentVariantSeriesIds({ variants = [], series = [], seriesById } = {}) {
  const parents = seriesById instanceof Map
    ? seriesById
    : new Map((series ?? []).filter((parent) => parent?.id).map((parent) => [parent.id, parent]));
  const index = new Map();
  for (const variant of variants ?? []) {
    if (!variant?.id || !variant.name || String(variant.variant_type || "").toLowerCase() === "provisional") continue;
    const seriesId = String(variant.series_id || "").trim();
    const parent = parents.get(seriesId);
    const key = buildCatalogParentVariantIdentityKey(parent?.name, variant.name);
    if (!seriesId || !key) continue;
    const seriesIds = index.get(key) ?? new Set();
    seriesIds.add(seriesId);
    index.set(key, seriesIds);
  }
  return index;
}

export function prepareMarketSafetyCatalog(catalog = {}) {
  const hasFormalVariants = catalog.formalVariantsBySeries instanceof Map;
  const hasFormalSeries = catalog.formalSeriesByFranchise instanceof Map;
  const hasFormalParentVariantIdentities = catalog.formalParentVariantSeriesIds instanceof Map;
  if (hasFormalVariants && hasFormalSeries && hasFormalParentVariantIdentities) return catalog;

  const series = catalog.seriesById instanceof Map
    ? [...catalog.seriesById.values()]
    : catalog.series ?? [];
  const formalVariantsBySeries = hasFormalVariants
    ? catalog.formalVariantsBySeries
    : buildFormalVariantsBySeries(catalog.variants ?? []);
  const formalVariants = [...formalVariantsBySeries.values()].flat();
  return {
    ...catalog,
    formalVariantsBySeries,
    formalSeriesByFranchise: hasFormalSeries
      ? catalog.formalSeriesByFranchise
      : buildFormalSeriesByFranchise(series),
    formalParentVariantSeriesIds: hasFormalParentVariantIdentities
      ? catalog.formalParentVariantSeriesIds
      : buildFormalParentVariantSeriesIds({ variants: formalVariants, series, seriesById: catalog.seriesById }),
  };
}
...
export function assessMarketCandidate(listing = {}, query = {}, catalog = {}) {
  ...
  if (/(?:予約|発売前|発売予定|pre[\s-]?order)/iu.test(String(listing.title || listing.name || ""))) {
    return review("preorder_listing", classification, { queryContextPresent: true });
  }
  ...
  const parentIdentity = analyzeParentSeriesIdentity({
    ...
    parentSeriesFranchise: parentSeries.franchise,
    targetVariantName: targetVariant.name,
    siblingVariantNames: scopedVariants
      .filter((variant) => variant.id !== targetVariant.id)
      .map((variant) => variant.name),
    siblingSeriesNames: (preparedCatalog.formalSeriesByFranchise?.get(normalize(parentSeries.franchise)) ?? [])
      .filter((series) => series.id !== parentSeries.id)
  ...
  const catalogIdentityKey = buildCatalogParentVariantIdentityKey(parentSeries.name, targetVariant.name);
  const catalogIdentitySeriesIds = preparedCatalog.formalParentVariantSeriesIds?.get(catalogIdentityKey);
  const auditChecks = {
    ...
    parentFranchiseEvidencePresent: parentIdentity.parentFranchiseEvidencePresent,
    parentSeriesExactEvidencePresent: parentIdentity.parentSeriesExactEvidencePresent,
    parentSeriesDiscriminatorRequired: parentIdentity.parentSeriesDiscriminatorRequired,
    parentSeriesDiscriminatorEvidencePresent: parentIdentity.parentSeriesDiscriminatorEvidencePresent,
    parentSeriesEvidencePresent: parentIdentity.parentSeriesEvidencePresent,
    catalogParentVariantIdentityAmbiguous: catalogIdentitySeriesIds instanceof Set
      && catalogIdentitySeriesIds.size > 1,
    ...
    parentSeriesEditionConflict: parentIdentity.parentSeriesEditionConflict,
  ...
  if (auditChecks.catalogParentVariantIdentityAmbiguous) {
    return review("catalog_parent_variant_identity_ambiguous", classification, auditChecks);
  }
...
export function summarizeFetchedMarketCandidates({ records = [], rawCount = 0, queryPlan = [], feedResults = [], catalog = {}, safetyResult } = {}) {
  ...
  for (const assessment of safety.assessments.filter((entry) => !entry.accepted)) {
    const reason = String(assessment.reason || "review_required").trim().toLowerCase();
    candidateRejectionReasons[reason] = (candidateRejectionReasons[reason] ?? 0) + 1;
  }
  return {
    ...
    ...safety.summary,
    review_required_count: safety.assessments.filter((entry) => entry.reviewRequired).length,
    variant_matches: safety.assessments.filter((entry) => entry.accepted && entry.variantId).length,
    candidate_rejection_reason_counts: Object.fromEntries(
      Object.entries(candidateRejectionReasons).sort(([left], [right]) => left.localeCompare(right)),
    ),
...
function attachSafetyAssessment(record, assessment) {
  const marketSafety = {
    ...
    checks: {
      ...
      parent_franchise_evidence_present: assessment.auditChecks?.parentFranchiseEvidencePresent === true,
      parent_series_exact_evidence_present: assessment.auditChecks?.parentSeriesExactEvidencePresent === true,
      parent_series_discriminator_required: assessment.auditChecks?.parentSeriesDiscriminatorRequired === true,
      parent_series_discriminator_evidence_present: assessment.auditChecks?.parentSeriesDiscriminatorEvidencePresent === true,
      parent_series_evidence_present: assessment.auditChecks?.parentSeriesEvidencePresent === true,
      catalog_parent_variant_identity_ambiguous: assessment.auditChecks?.catalogParentVariantIdentityAmbiguous === true,
