...
export function buildOfficialRereleaseEvent({ record, series, observedAt } = {}) {
  ...
  // For a new series the read-only audit passes the same resolved fetch record
  // as both `record` and `series` so the event can reuse its identity fields.
  // That object is not an existing catalog row. Treating it as one loses the
  // original year for month-precision rerelease evidence because the persisted
  // series shape stores release_month but has no standalone release year.
  const existingSeries = series === record ? null : series;
  const canonical = resolveCanonicalOfficialRelease(record, existingSeries);
