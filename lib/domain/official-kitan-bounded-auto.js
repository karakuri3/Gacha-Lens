...
export function prepareOfficialKitanBoundedAuto({ report, auditRunId, auditDigest, headSha, originMainSha, workflow = {} } = {}) {
  ...
  if (report.report_complete !== true || report.database?.writes !== 0 || (report.blockers || []).some((blocker) => blocker !== "kitan_no_safe_candidate")) throw coded("official_kitan_bounded_auto_audit_blocked");
  ...
  const plan = {
    ...
    conflict_excluded_count: classified.conflict_excluded_count,
...
export function authorizeOfficialKitanBoundedAuto({ prepared, auditRunId, auditDigest, headSha, originMainSha, applyDigest } = {}) {
  ...
  if (id(auditRunId) !== id(prepared.run_id) || text(auditDigest) !== text(prepared.audit_digest)) throw coded("official_kitan_bounded_auto_audit_binding_mismatch");
  if (text(applyDigest) !== text(prepared.plan.selected_apply_contract_digest)) throw coded("official_kitan_bounded_auto_apply_digest_mismatch");
  validateInsertOnly(prepared._candidate);
  return { ok: true, kind: "kitan_bounded_automatic", head_sha: head, audit_run_id: id(auditRunId), audit_digest: text(auditDigest), approval_digest: text(auditDigest), candidate: prepared._candidate };
}
...
function classifyCandidates(eligible, selected) {
  ...
  return {
    ...
    conflict_excluded_count: unique.filter((candidate) => candidate.source_count_conflict === true).length,
  };
}
