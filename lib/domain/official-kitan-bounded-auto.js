...
export function finalizeOfficialKitanBoundedAutoTerminalResult({ existing = null, workflow = {}, reasonCode } = {}) {
  if (isTerminalResult(existing)) return existing;
  const base = existing && typeof existing === "object"
    ? existing
    : buildOfficialKitanBoundedAutoDisabledResult({ workflow });
  const database = emptyDatabase();
  return {
    ...base,
    _candidate: undefined,
    automatic_gate_enabled: true,
    run_id: id(workflow.run_id) || base.run_id || null,
    head_sha: sha(workflow.head_sha) || base.head_sha || null,
    database,
    database_writes: 0,
    operations: { series: 0, variants: 0, restock_events: 0 },
    planned_operations: base.planned_operations || null,
    committed_operations: null,
    before_digests: {},
    after_digests: {},
    transaction: { state: "not_started", rollback_attempted: false, rollback_verified: false },
    reason_code: text(reasonCode) || "official_kitan_bounded_auto_terminal_failure",
    final_verdict: "OFFICIAL_KITAN_BOUNDED_AUTO_BLOCKED",
  };
}
...
function isTerminalResult(value) { return new Set(["OFFICIAL_KITAN_BOUNDED_AUTO_DISABLED", "OFFICIAL_KITAN_BOUNDED_AUTO_NOOP", "OFFICIAL_KITAN_BOUNDED_AUTO_COMMITTED", "OFFICIAL_KITAN_BOUNDED_AUTO_COMMIT_OUTCOME_UNKNOWN", "OFFICIAL_KITAN_BOUNDED_AUTO_COMMITTED_POST_VERIFY_FAILED", "OFFICIAL_KITAN_BOUNDED_AUTO_POSTFLIGHT_FAILED", "OFFICIAL_KITAN_BOUNDED_AUTO_BLOCKED", "OFFICIAL_BOUNDED_WRITE_ROLLED_BACK", "OFFICIAL_BOUNDED_WRITE_BLOCKED"]).has(value?.final_verdict); }
