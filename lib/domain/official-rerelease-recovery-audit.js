...
export function validateOfficialRereleaseRecoveryAudit(report) {
  ...
  for (const candidate of report.plan.candidates) {
    ...
    if (candidate.state === "eligible" && !["insert", "update", "none"].includes(candidate.operation)) {
      throw new Error("Official rerelease recovery operation is invalid.");
    }
    if (candidate.state === "blocked" && candidate.operation !== null) {
      throw new Error("Blocked rerelease recovery candidate has an operation.");
    }
  }
