...
export function validateOfficialRereleaseRecoveryAudit(report) {
  ...
  const computedDelta = Object.fromEntries(COUNT_KEYS.map((key) => [
    key,
    Number(report?.database?.after?.[key]) - Number(report?.database?.before?.[key]),
  ]));
  const databaseDeltaConsistent = databaseCountsComplete
    && COUNT_KEYS.every((key) => Number(report?.database?.delta?.[key]) === computedDelta[key]);
  const databaseStable = databaseDeltaConsistent
    && COUNT_KEYS.every((key) => computedDelta[key] === 0);
