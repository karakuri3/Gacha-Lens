...
const PRIORITY_TWO_DISTINCT_EVIDENCE_KIND = "priority_2_distinct_evidence_read_only";
const PRIORITY_TWO_MERCHANT_IDENTITY_KIND = "priority_2_merchant_identity_read_only";
const PRIORITY_ONE_BOUNDED_EVIDENCE_KIND = "priority_1_bounded_evidence_read_only";
...
export function isNonAuthoritativeManualMarketAudit(report = {}) {
  ...
  return diagnostic?.canary_eligible === false
    ...
    && [DIAGNOSTIC_KIND, PRIORITY_THREE_SEED_KIND, PRIORITY_TWO_DISTINCT_EVIDENCE_KIND, PRIORITY_TWO_MERCHANT_IDENTITY_KIND, PRIORITY_ONE_BOUNDED_EVIDENCE_KIND].includes(diagnostic.kind);
}
...
export function buildPriorityTwoDistinctEvidenceReadOnlyDiagnostic() {
  return {
    ...
    fallback_reason: "priority_2_distinct_evidence_read_only",
  };
}

export function buildPriorityTwoMerchantIdentityReadOnlyDiagnostic() {
  return {
    ...
    requested_priority: "2",
    effective_priority: "2",
    fallback_reason: "priority_2_merchant_identity_read_only",
  };
}

export function buildPriorityOneBoundedEvidenceReadOnlyDiagnostic() {
  return {
    ...
    fallback_reason: "priority_1_bounded_evidence_read_only",
  };
}
...
export function sanitizeManualMarketAuditDiagnostic(value) {
  ...
  if (
    value?.kind === PRIORITY_TWO_DISTINCT_EVIDENCE_KIND
    && value?.canary_eligible === false
    && value?.write_eligible === false
    && value?.requested_priority === "2"
    && value?.effective_priority === "2"
    && value?.fallback_reason === "priority_2_distinct_evidence_read_only"
  ) {
    return buildPriorityTwoDistinctEvidenceReadOnlyDiagnostic();
  }
  if (
    value?.kind === PRIORITY_TWO_MERCHANT_IDENTITY_KIND
    && value?.canary_eligible === false
    && value?.write_eligible === false
    && value?.requested_priority === "2"
    && value?.effective_priority === "2"
    && value?.fallback_reason === "priority_2_merchant_identity_read_only"
  ) {
    return buildPriorityTwoMerchantIdentityReadOnlyDiagnostic();
  }
  if (
    value?.kind === PRIORITY_ONE_BOUNDED_EVIDENCE_KIND
    && value?.canary_eligible === false
    && value?.write_eligible === false
    && value?.requested_priority === "1"
    && value?.effective_priority === "1"
    && value?.fallback_reason === "priority_1_bounded_evidence_read_only"
  ) {
    return buildPriorityOneBoundedEvidenceReadOnlyDiagnostic();
  }
  if (
