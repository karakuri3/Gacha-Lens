import { effectiveReleaseState } from "./release-state.js";
...
export function parseCatalogQuery(input = {}) {
  ...
  // Discovery defaults to the stable parent-series entity. Legacy signal views
  // remain variant-first unless callers explicitly opt into a series scope.
  const requestedScope = oneOf(input.scope, CATALOG_SCOPES, "");
  const scope = requestedScope || (LEGACY_MODES.has(legacyFilter) ? "variant" : "series");
...
export function buildCatalogHref(query = {}, changes = {}, options = {}) {
  ...
  // A retained legacy signal filter otherwise implies variant scope on parse.
  // Keep an explicit series override round-trippable instead of silently losing it.
  if (next.scope !== "series" || next.legacyMode) params.set("scope", next.scope);
...
export function buildCatalogCanonicalHref(query = {}) {
...
export function recordMatchesCatalogQuery(record = {}, query = {}, scope = "variant", now = new Date()) {
  ...
  const released = effectiveReleaseState(record, { now });
  if (parsed.release === "released" && !released) return false;
  if (parsed.release === "upcoming" && released) return false;
