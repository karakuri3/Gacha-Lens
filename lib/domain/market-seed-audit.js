...
export function buildPriorityThreeSeedQueryPlanArtifact(plan = {}) {
  ...
  for (const query of plan.queries) {
    ...
    entries.push({
      ...
      query_profile: text(query?.query_profile, 64),
...
export function renderPriorityThreeSeedQueryPlanMarkdown(artifact) {
  ...
  const lines = [
    ...
    "",
    "- Priority: 3",
    `- Query profile: ${artifact.query_profile}`,
    "- Read-only: true",
...
function assertSafeArtifact(artifact) {
  if (
    !artifact
    || artifact.kind !== "priority_3_seed_read_only_query_plan"
    || artifact.priority !== 3
    || artifact.query_profile !== QUERY_PROFILE
  ) {
  ...
  for (const entry of artifact.selected_variants) {
    if (
      ...
      || entry.priority !== 3
      || entry.query_profile !== QUERY_PROFILE
      || !entry.primary_query
    ) {
