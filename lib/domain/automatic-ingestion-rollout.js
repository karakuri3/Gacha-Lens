...
import { validateMarketBoundedCoverageSnapshot } from "./market-bounded-coverage.js";
...
export function buildAutomaticMarketRolloutPlan(input = {}) {
  ...
  const safeCandidates = candidates.filter((_, index) => evaluations[index].eligible);
  const eligibleKeys = safeCandidates.map((candidate) => candidate.candidate_key).sort();
  ...
  try {
    coverageSnapshot = validateMarketBoundedCoverageSnapshot(input.coverage_snapshot);
  } catch {
    throw fail("rollout_coverage_unavailable", "Durable market coverage is unavailable or ambiguous.");
  }
  let boundedSelection;
  try {
    boundedSelection = selectDeterministicMarketPersistenceCandidates({
      candidates: safeCandidates,
      selectedVariants: audit.selection.selected_variants,
      coverageSnapshot,
      capacity: limits.max_persistence_candidates,
    });
  } catch {
    throw fail("rollout_plan_incomplete", "Bounded persistence selection could not be derived.");
  }
  const selectedKeys = boundedSelection.selectedCandidateKeys;
  const listingWritesPlanned = Math.max(selectedKeys.length, nonnegative(input.planned_counts?.listings));
  const observationWritesPlanned = Math.max(selectedKeys.length, nonnegative(input.planned_counts?.observations));
  const reviewWritesPlanned = nonnegative(input.planned_counts?.review_required);
  const reviewRequiredCount = candidates.filter((candidate) => candidate.assessment?.review_required === true).length;
  const budgetChecks = {
    selected_variants_within_limit: selectedCount <= limits.max_selected_variants,
    total_candidates_within_limit: candidates.length <= limits.max_candidates,
    safe_candidates_within_limit: eligibleKeys.length <= limits.max_accepted_candidates,
    selected_candidates_within_limit: selectedKeys.length <= limits.max_persistence_candidates
      && selectedKeys.length <= MARKET_BOUNDED_PERSISTENCE_HARD_CAP,
  ...
  const plan = {
    ...
    safe_candidate_count: eligibleKeys.length,
    review_required_candidate_count: reviewRequiredCount,
    selected_for_persistence_count: selectedKeys.length,
    safe_not_selected_count: boundedSelection.safeNotSelectedCandidateKeys.length,
    selected_distinct_variant_count: boundedSelection.selectedDistinctVariantCount,
    selected_new_variant_count: boundedSelection.selectedNewVariantCount,
    selected_previously_persisted_candidate_count: boundedSelection.selectedPreviouslyPersistedCandidateCount,
    selected_candidate_keys: selectedKeys,
    safe_not_selected_candidate_keys: boundedSelection.safeNotSelectedCandidateKeys,
    safe_not_selected_candidates: boundedSelection.safeNotSelectedCandidateKeys.map((candidateKey) => ({
    ...
    coverage_snapshot: structuredClone(coverageSnapshot),
    coverage_snapshot_digest: coverageSnapshot.snapshot_digest,
    coverage_source_row_count: coverageSnapshot.source_row_count,
...
export function buildSanitizedRolloutReport(input = {}) {
  ...
  return {
    ...
    candidate_counts: {
      ...
      total: plan.total_candidate_count,
      safe: plan.safe_candidate_count,
      review_required: plan.review_required_candidate_count,
      selected_for_persistence: plan.selected_for_persistence_count,
      safe_not_selected: plan.safe_not_selected_count,
      selected_distinct_variants: plan.selected_distinct_variant_count,
      selected_new_variants: plan.selected_new_variant_count,
      selected_previously_persisted_candidates: plan.selected_previously_persisted_candidate_count,
    },
...
export function renderAutomaticMarketRolloutPlanMarkdown(plan) {
  return [
    ...
    `- Candidates: ${plan.candidate_count}`,
    `- Independently safe: ${plan.safe_candidate_count}`,
    `- Selected for persistence: ${plan.selected_for_persistence_count}`,
    `- Safe but not selected: ${plan.safe_not_selected_count}`,
    `- Selected distinct variants: ${plan.selected_distinct_variant_count}`,
    `- Newly covered variants: ${plan.selected_new_variant_count}`,
    `- Previously persisted candidates selected: ${plan.selected_previously_persisted_candidate_count}`,
    `- Coverage snapshot: ${plan.coverage_snapshot_digest}`,
    `- Excluded: ${plan.excluded_count}`,
...
export function renderAutomaticIngestionShadowReportMarkdown(report) {
  return [
    ...
    `- Independently safe candidates: ${report.candidate_counts.safe}`,
    `- Would write candidates: ${report.candidate_counts.selected_for_persistence}`,
    `- Safe but not selected: ${report.candidate_counts.safe_not_selected}`,
...
export function collectAutomaticIngestionSecretValues(env = {}) {
  return Object.entries(env)
    .filter(([name]) => /(?:KEY|TOKEN|SECRET|PASSWORD|APPLICATION_ID|AFFILIATE(?:_TRACKING)?_ID)$/i.test(name)
      && !PUBLIC_TRACKING_ENV_NAMES.has(name))
...
function validateMarketStage(stage, expected) {
  ...
  if (expected.bounded && (
    ...
    || stage.max_candidates !== 40
    || stage.max_accepted_candidates !== 20
    || stage.max_persistence_candidates !== 2
...
function sanitizeContract(contract) {
  ...
  const allowed = [
    ...
    "max_candidates", "max_accepted_candidates", "max_persistence_candidates", "max_listing_writes",
