...
export function getYahooAffiliateDestination(item = {}) {
  ...
  const destination = sanitizeYahooAffiliateProvenance({
    ...
    listingId: listing.raw?.code || listing.raw?.source_listing_id,
    publicUrl: listing.source_url,
    affiliateUrl: listing.raw?.affiliate_url,
    affiliateUrlSource: listing.raw?.affiliate_url_source,
    affiliateUrlContract: listing.raw?.affiliate_url_contract,
    sourceDocumentation: listing.raw?.affiliate_url_documentation || listing.raw?.source_documentation,
  });
...
export function isSafeYahooAffiliateListing(listing = {}) {
  ...
  if (String(listing.raw?.provider || "").toLowerCase() !== "yahoo_shopping") return false;

  const listingId = String(listing.raw?.code || listing.raw?.source_listing_id || "").trim();
