...
import { sanitizeManualMarketAuditDiagnostic } from "./manual-market-audit-diagnostic.js";
...
export function buildSanitizedMarketCandidateAudit({ records = [], queryPlan = [], catalog = {}, runContext = {}, summary = {} } = {}) {
  ...
  const boundedEvidence = runContext.bounded_candidate_evidence === true;
  ...
  const retrievalEffectiveness = requestDiagnostics
    ? buildMarketRetrievalEffectiveness({
      ...
      summary: {
        ...
        ...(boundedEvidence ? {
          review_required_count: visibleCandidates.filter((candidate) => candidate.assessment.review_required).length,
        } : {}),
      },
    })
    : null;
  ...
  const report = {
    ...
    ...(summary.manual_diagnostic ? {
      manual_diagnostic: sanitizeManualMarketAuditDiagnostic(summary.manual_diagnostic),
    } : {}),
    selection: {
...
export function validateMarketCandidateAudit(report) {
  ...
  if (report.candidate_evidence) validateCandidateEvidence(report.candidate_evidence, report);
  ...
  if (report.manual_diagnostic) sanitizeManualMarketAuditDiagnostic(report.manual_diagnostic);
...
function validateCandidateEvidence(value, report) {
  if (!value || value.scope !== "bounded") throw new Error("Candidate evidence scope is invalid.");
  const limit = number(value.candidate_limit, -1);
  const full = number(value.full_candidate_count, -1);
  const visible = number(value.visible_candidate_count, -1);
  const overflow = number(value.overflow_count, -1);
  if (limit < 1 || full < 0 || visible < 0 || overflow < 0 || visible !== report.candidates.length || full !== visible + overflow || overflow !== report.result.truncated_count) {
    throw new Error("Candidate evidence totals are invalid.");
  }
}
...
export function renderMarketCandidateAuditMarkdown(report) {
  ...
  const lines = [
    ...
    ...(report.manual_diagnostic ? [
      "## Manual diagnostic boundary",
      "",
      `- Kind: ${md(report.manual_diagnostic.kind)}`,
      `- Requested priority: ${md(report.manual_diagnostic.requested_priority)}`,
      `- Effective priority: ${md(report.manual_diagnostic.effective_priority)}`,
...
function buildCandidate(record, queryByText, variantById, seriesById) {
  ...
  return {
    ...
    checks: {
      ...
      parent_franchise_evidence_present: safety.checks?.parent_franchise_evidence_present === true,
      parent_series_exact_evidence_present: safety.checks?.parent_series_exact_evidence_present === true,
      parent_series_discriminator_required: safety.checks?.parent_series_discriminator_required === true,
      parent_series_discriminator_evidence_present: safety.checks?.parent_series_discriminator_evidence_present === true,
      parent_series_evidence_present: safety.checks?.parent_series_evidence_present === true,
      catalog_parent_variant_identity_ambiguous: safety.checks?.catalog_parent_variant_identity_ambiguous === true,
