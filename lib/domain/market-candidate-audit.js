...
export function buildSanitizedMarketCandidateAudit({ records = [], queryPlan = [], catalog = {}, runContext = {}, summary = {} } = {}) {
  ...
  const report = {
    ...
    ...(summary.request_diagnostics ? {
      request_diagnostics: sanitizeMarketRequestDiagnostics(summary.request_diagnostics),
    } : {}),
...
export function validateMarketCandidateAudit(report) {
  ...
  for (const candidate of report.candidates) {
    ...
    if (candidate.source?.affiliate_destination) {
      const sanitized = sanitizeRakutenAffiliateProvenance({
      ...
      if (!sanitized || JSON.stringify(sanitized) !== JSON.stringify(candidate.source.affiliate_destination)) {
        throw new Error("Audit report contains unsafe Rakuten affiliate provenance.");
      }
    }
  }
  return true;
...
function buildCandidate(record, queryByText, variantById, seriesById) {
  ...
  const affiliateDestination = sanitizeRakutenAffiliateProvenance({
    provider,
    publicUrl,
    ...
    sourceDocumentation: record.raw?.source_documentation,
  });
