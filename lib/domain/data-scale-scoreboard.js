    ...
    panels: {
      data: {
        catalog: {
          ...
          supported_source_count: data.sourceCapabilities === null ? unavailable() : metric(data.sourceCapabilities.length),
          source_capabilities: data.sourceCapabilities === null ? unavailable() : metric(sanitizeSourceCapabilities(data.sourceCapabilities)),
        },
...
function buildSignalPanel(data, socialAuthorized, now) {
  return {
    stock: data.stockReports === null ? unavailable() : signalWindow(data.stockReports, now, signalTimestamp),
    restock: data.restockEvents === null ? unavailable() : signalWindow(data.restockEvents, now, signalTimestamp),
