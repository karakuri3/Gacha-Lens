...
export const PRIORITY_ONE_BOUNDED_EVIDENCE_KIND = "priority_1_bounded_evidence_read_only";

export function buildPriorityOneDistinctEvidenceDiagnostic({ audit = {}, queryPlan = [], existingListings = [], candidateStorefronts = new Map(), before = null, after = null, kind = PRIORITY_ONE_BOUNDED_EVIDENCE_KIND } = {}) {
  assertAuditContract(audit, queryPlan, kind);
  if (!Array.isArray(existingListings)) throw new Error("Priority 1 existing listings are unavailable.");
  ...
  const variants = (audit.selection?.selected_variants ?? []).map((selected) => {
    ...
    const classified = accepted.map((candidate) => classifyAcceptedCandidate(candidate, existing, candidateStorefronts));
    ...
    return {
      ...
      priority: 1,
      released: queriesByVariant.get(variantId)?.released === true,
      variant_type: text(queriesByVariant.get(variantId)?.variant_type, 60),
  ...
  const result = {
    ...
    summary: {
      ...
      accepted_distinct_listing_count: distinct.length,
      distinct_storefront_count: new Set(distinct.map(storefrontIdentityKey).filter(Boolean)).size,
      distinct_safe_variant_count: variants.filter((entry) => entry.accepted_distinct_count > 0).length,
      independent_storefront_distinct_variant_count: variants.filter((entry) => entry.accepted_distinct.some((candidate) => candidate.independent_storefront_evidence === true)).length,
      independent_merchant_count: null,
      independent_merchant_distinct_variant_count: null,
      merchant_identity_status: "unknown",
    },
    request_diagnostics: audit.request_diagnostics ?? null,
    variants,
  };
  validatePriorityOneDistinctEvidenceDiagnostic(result);
  return result;
}

export function validatePriorityOneDistinctEvidenceDiagnostic(value) {
  if (!value || value.schema_version !== 1 || value.kind !== PRIORITY_ONE_BOUNDED_EVIDENCE_KIND
    || value.priority !== 1 || value.query_profile !== PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE
    ...
    throw new Error("Priority 1 distinct evidence diagnostic contract is invalid.");
  }
  ...
  if (ids.some((id) => !id) || new Set(ids).size !== ids.length || value.selection?.selected_variant_count !== ids.length) {
    throw new Error("Priority 1 distinct evidence selection is invalid.");
  }
  for (const entry of value.variants) {
    if (entry.priority !== 1 || entry.released !== true || !entry.variant_type || entry.variant_type === "provisional"
      || !Array.isArray(entry.existing_listing_identity) || entry.existing_listing_identity.length !== 2 || !Array.isArray(entry.queries_attempted)
      ...
      throw new Error("Priority 1 distinct evidence variant contract is invalid.");
    }
    ...
    if (entry.accepted_distinct_count !== distinct.length || entry.accepted_existing_count !== existing.length
      ...
      throw new Error("Priority 1 distinct evidence totals are invalid.");
    }
  ...
  if (value.summary?.accepted_distinct_count !== value.variants.reduce((count, entry) => count + entry.accepted_distinct_count, 0)
    || value.summary?.accepted_distinct_listing_count !== value.summary?.accepted_distinct_count
    || value.summary?.distinct_storefront_count !== new Set(value.variants.flatMap((entry) => entry.accepted_distinct).map(storefrontIdentityKey).filter(Boolean)).size
    || value.summary?.distinct_safe_variant_count !== value.variants.filter((entry) => entry.accepted_distinct_count > 0).length
    || value.summary?.independent_storefront_distinct_variant_count !== value.variants.filter((entry) => entry.accepted_distinct.some((candidate) => candidate.independent_storefront_evidence === true)).length
    || value.summary?.merchant_identity_status !== "unknown"
    || value.summary?.independent_merchant_count !== null
    || value.summary?.independent_merchant_distinct_variant_count !== null) {
    throw new Error("Priority 1 distinct evidence summary is invalid.");
  }
  inspect(value);
  return true;
}

export function renderPriorityOneDistinctEvidenceDiagnosticMarkdown(value) {
  validatePriorityOneDistinctEvidenceDiagnostic(value);
  const lines = [
    "# Priority 1 Bounded Storefront Evidence Diagnostic",
    "",
    "- Read-only: true",
    ...
    `- Distinct storefronts: ${value.summary.distinct_storefront_count}`,
    `- Independent storefront variants: ${value.summary.independent_storefront_distinct_variant_count}`,
    `- Merchant identity status: ${value.summary.merchant_identity_status}`,
    "",
    "## Per variant",
    "",
  ...
  lines.push(
      "## Distinct storefront evidence",
      "",
      "| Provider | Listing ID | Candidate key | Safe URL | Price | Storefront ID | Storefront name | Storefront source | Independent storefront | Merchant identity |",
      "|---|---|---|---|---:|---|---|---|---|---|",
      ...value.variants.flatMap((entry) => entry.accepted_distinct).map((candidate) => `| ${md(candidate.provider)} | ${md(candidate.source_listing_id)} | ${md(candidate.candidate_key)} | ${md(candidate.public_url)} | ${candidate.price} | ${md(candidate.storefront_id)} | ${md(candidate.storefront_name)} | ${md(candidate.storefront_identity_source)} | ${candidate.independent_storefront_evidence} | ${candidate.merchant_identity_status} |`),
      "",
    );
  return `${lines.join("\n")}\n`;
}

function classifyAcceptedCandidate(candidate, existing, candidateStorefronts) {
  const candidateIdentity = sanitizeCandidate(candidate, candidateStorefronts);
  const matches = existing.flatMap((identity) => identityMatches(candidateIdentity, identity));
  return {
    ...candidateIdentity,
    classification: matches.length ? "accepted_existing" : "accepted_distinct",
    existing_match_fields: [...new Set(matches)].sort(),
    independent_storefront_evidence: matches.length ? "unknown" : compareIndependentStorefrontEvidence(candidateIdentity, existing),
  };
}
...
function sanitizeExistingIdentity(listing = {}) {
  ...
  return {
    ...
    ...resolveMarketplaceStorefrontEvidence(listing),
  };
}

function sanitizeCandidate(candidate = {}, candidateStorefronts) {
  const storefront = candidateStorefronts instanceof Map ? candidateStorefronts.get(text(candidate.candidate_key, 64)) : null;
  return {
    ...
    listing_identity: text(candidate.candidate_key, 64),
    storefront_id: text(storefront?.storefront_id, 120) || null,
    storefront_name: text(storefront?.storefront_name, 180) || null,
    storefront_identity_source: text(storefront?.storefront_identity_source, 120) || null,
    merchant_identity: null,
    merchant_identity_status: "unknown",
    independent_storefront_evidence: "unknown",
...
function validateCandidate(candidate) {
  if (!CANDIDATE_KEY.test(candidate.candidate_key) || !candidate.provider || !candidate.source_listing_id
    ...
    || candidate.listing_identity !== candidate.candidate_key
    || candidate.merchant_identity !== null || candidate.merchant_identity_status !== "unknown"
    || candidate.independent_merchant_evidence !== "unknown"
    || ![true, false, "unknown"].includes(candidate.independent_storefront_evidence)
    || (candidate.storefront_id === null && (candidate.storefront_name !== null || candidate.storefront_identity_source !== null))
    || (candidate.storefront_id !== null && !candidate.storefront_identity_source)
    || !Array.isArray(candidate.existing_match_fields)) {
    throw new Error("Priority 1 distinct evidence candidate is invalid.");
  }
...
function assertAuditContract(audit, queryPlan, kind) {
  if (audit?.mode !== "dry-run" || audit?.source_scope !== "planner-apis" || audit?.manual_diagnostic?.kind !== kind
    ...
    throw new Error("Priority 1 distinct evidence audit is not a complete read-only diagnostic.");
  }
  const selected = audit.selection?.selected_variants ?? [];
  if (!selected.length || selected.length > 5 || selected.some((entry) => Number(entry.priority) !== 1)
    || !Array.isArray(queryPlan) || queryPlan.length !== selected.length
    || queryPlan.some((entry) => entry?.query_profile !== PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE
      || Number(entry?.priority) !== 1 || queryAttempts(entry).length > MARKET_MAX_QUERY_ATTEMPTS_PER_ROOT)) {
    throw new Error("Priority 1 distinct evidence audit selection is invalid.");
  }
}
...
function inspect(value) { if (Array.isArray(value)) return value.forEach(inspect); if (!value || typeof value !== "object") return; for (const [key, child] of Object.entries(value)) { if (FORBIDDEN.test(key)) throw new Error("Priority 1 distinct evidence artifact contains a forbidden field."); inspect(child); } }
