...
export function buildOfficialPhaseA3Plan(classification = {}, { allowEmpty = false } = {}) {
  const safeRecords = sortRecordsByUrl(asArray(classification.safeRecords));
  ...
  for (const record of safeRecords) {
    ...
    for (let variantIndex = 0; variantIndex < variants.length; variantIndex += 1) {
      ...
      let row = buildOfficialPhaseA3VariantRow(variant, record);
      ...
      if (variantIds.has(row.id) || variantSlugs.has(row.slug)) {
        row = disambiguateOfficialPhaseA3VariantIdentity(row, {
          seriesId,
          variantIndex,
          variantIds,
          variantSlugs,
        });
        identityDisambiguations += 1;
      }

      if (variantIds.has(row.id)) throw phaseA3Error("phase_a3_duplicate_variant_id_after_disambiguation");
      if (variantSlugs.has(row.slug)) throw phaseA3Error("phase_a3_duplicate_variant_slug_after_disambiguation");
  ...
  const writeContractDigest = buildOfficialPhaseA3WriteContractDigest({
    targets,
  ...
  return {
    ...
    write_contract_digest: writeContractDigest,
    target_series: targets.length,
    target_variants: variantRows.length,
    identity_disambiguations: identityDisambiguations,
    targets,
    variant_rows: variantRows,
  };
}
...
export function buildOfficialPhaseA3WriteContractDigest(plan = {}) {
  const targets = asArray(plan?.targets)
    .map((target) => ({
      series_id: text(target?.series_id),
      official_url: canonicalOfficialUrl(target?.official_url),
    }))
    .sort((left, right) => left.series_id.localeCompare(right.series_id, "en"));

  const variants = asArray(plan?.variant_rows)
    .map((row) => ({
      id: text(row?.id),
      slug: text(row?.slug),
      series_id: text(row?.series_id),
      name: text(row?.name),
      variant_type: text(row?.variant_type),
      rarity: text(row?.rarity),
      role: text(row?.role),
      brand: text(row?.brand),
      official_url: canonicalOfficialUrl(row?.official_url),
      axes: row?.axes && typeof row.axes === "object" ? row.axes : {},
      signals: row?.signals && typeof row.signals === "object" ? row.signals : {},
      tags: asArray(row?.tags).map(text).filter(Boolean),
      source_type: text(row?.source_type),
      review_required: Boolean(row?.review_required),
      image_scope: text(row?.raw?.image_scope || row?.raw?.raw?.image_scope),
    }))
    .sort((left, right) => left.id.localeCompare(right.id, "en"));

  return officialCanonicalDigest({
    schema_version: OFFICIAL_PHASE_A3_SCHEMA_VERSION,
    contract_version: 2,
    targets,
    variants,
...
export function disambiguateOfficialPhaseA3VariantIdentity(row, {
  seriesId,
  variantIndex,
  variantIds,
  variantSlugs,
} = {}) {
  const baseId = text(row?.id);
  const baseSlug = text(row?.slug);
  const normalizedSeriesId = text(seriesId || row?.series_id);
  const ordinal = Number.isInteger(variantIndex) && variantIndex >= 0 ? variantIndex + 1 : 1;

  for (let attempt = 0; attempt < 100; attempt += 1) {
    const suffix = "-variant-" + ordinal + (attempt ? "-" + (attempt + 1) : "");
    const nextId = appendIdentitySuffix(baseId || normalizedSeriesId || "variant", suffix);
    const nextSlug = appendIdentitySuffix(baseSlug || nextId, suffix);
    if (!variantIds?.has(nextId) && !variantSlugs?.has(nextSlug)) {
      return { ...row, id: nextId, slug: nextSlug };
    }
  }

  throw phaseA3Error("phase_a3_variant_identity_disambiguation_exhausted");
}
...
export function assertOfficialPhaseA3Expectation(snapshot, expected = {}) {
  ...
  const writeContractDigest = text(expected.write_contract_digest);
  if (writeContractDigest && snapshot?.plan?.write_contract_digest !== writeContractDigest) {
    throw phaseA3Error("phase_a3_write_contract_digest_mismatch");
  }

  for (const [scope, field, expectedValue] of [
    ["counts", "known_undetailed", expected.known_undetailed],
    ["counts", "safe_records", expected.safe_records],
    ["counts", "safe_variants", expected.safe_variants],
    ["counts", "rerelease_records", expected.rerelease_records],
    ["counts", "unresolved_records", expected.unresolved_records],
    ["scan", "held_shared_detailed_urls", expected.held_shared_detailed_urls],
    ["scan", "held_unsupported_provider_urls", expected.held_unsupported_provider_urls],
    ["plan", "identity_disambiguations", expected.identity_disambiguations],
  ]) {
    ...
    if (Number(snapshot?.[scope]?.[field]) !== parsed) throw phaseA3Error("phase_a3_expectation_mismatch");
  }
  return snapshot;
}
...
export function buildOfficialPhaseA3Snapshot({
  ...
  return {
    ...
    plan: {
      ...
      write_contract_digest: text(plan?.write_contract_digest),
...
export function validateOfficialPhaseA3Snapshot(report) {
  ...
  if (!/^sha256:[0-9a-f]{64}$/.test(text(report?.plan?.write_contract_digest))) {
    throw phaseA3Error("phase_a3_write_contract_digest_invalid");
  }
...
export function formatOfficialPhaseA3SnapshotMarkdown(report) {
  return [
    ...
    "- Write contract digest: " + report.plan.write_contract_digest,
    ...
    "- Identity disambiguations: " + report.plan.identity_disambiguations,
...
function appendIdentitySuffix(value, suffix) {
  const normalized = text(value) || "variant";
  const maxBaseLength = Math.max(1, 140 - suffix.length);
  return normalized.slice(0, maxBaseLength).replace(/-+$/g, "") + suffix;
}
