...
export function buildOfficialPhaseA3VariantRow(raw = {}, series = {}) {
  ...
  const seriesName = text(series?.name || series?.title || series?.product_name);
  ...
  return {
    ...
    image,
    released: resolveReleased(releaseDate, raw?.released ?? series?.is_released ?? series?.released),
    price,
