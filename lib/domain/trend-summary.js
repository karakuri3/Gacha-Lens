...
export function buildCirculationScore({ marketSummary = {}, availabilitySummary = {} }) {
  ...
  if (score >= 42) return { score, label: "流通信号あり" };
...
function buildTrendTags({ score, xIntentCounts, activeListings, soldListings, stockMovement, availabilitySummary, releasedRecently, rareMentions, marketSummary }) {
  ...
  if (xIntentCounts[X_INTENT_TAGS.ATTENTION] || xReactionsStrong(xIntentCounts)) tags.push("SNS反応強め");
...
function countRareMentions(xReactions = [], marketListings = []) {
  const rarePattern = /レア|シークレット|secret|rare|当たり|レアカラー/i;
  const xCount = xReactions.filter((reaction) => rarePattern.test(reaction.text || "")).length;
  const marketCount = marketListings.filter((listing) => rarePattern.test(`${listing.title || ""} ${listing.listing_type || ""}`)).length;
