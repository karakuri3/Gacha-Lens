...
export function assertP3SeedCanaryPrewrite({ target: targetInput, rows, variantListings = [], existingListings = [], existingObservations = [] } = {}) {
  const target = loadP3SeedCanaryTarget(targetInput);
  if (variantListings.length !== 0 || existingListings.length !== 0 || existingObservations.length !== 0) {
