...
export function buildP3SeedCanaryRows({ candidate, target: targetInput, workflow, observed_at = new Date() } = {}) {
  ...
  if (!source || !sourceUrl || !listingId || !Number.isFinite(Number(candidate.listing.price)) || Number(candidate.listing.price) <= 0) throw new Error("P3 seed canary marketplace identity or price is invalid.");
...
export function assertP3SeedCanaryPrewrite({ target: targetInput, rows, variantListings = [], sourceUrlRows = [], existingListings = [], existingObservations = [] } = {}) {
  const target = loadP3SeedCanaryTarget(targetInput);
  if (variantListings.length !== 0 || sourceUrlRows.length !== 0 || existingListings.length !== 0 || existingObservations.length !== 0) {
    throw new Error("P3 seed canary target already has market evidence.");
  }
  if (rows?.listingRows?.length !== 1 || rows?.observationRows?.length !== 1
    || rows.listingRows[0].variant_id !== target.variant_id
    || rows.listingRows[0].source_url !== target.public_url) throw new Error("P3 seed canary prewrite identity is invalid.");
  return true;
}

export function buildP3SeedCanaryResult({ target: targetInput, candidate = null, outcome = null, before = null, after = null, status = "blocked", error = null } = {}) {
  const target = loadP3SeedCanaryTarget(targetInput);
  const bounded = error?.bounded_result ?? outcome ?? {};
  return {
    schema_version: 1,
    status: ["succeeded", "blocked", "rolled-back", "rollback-failed"].includes(status) ? status : "blocked",
    target: { variant_id: target.variant_id, series_id: target.series_id, provider: target.provider, source_listing_id: target.source_listing_id, public_url: target.public_url },
    candidate_key: /^[0-9a-f]{16}$/.test(String(candidate?.candidate_key ?? "")) ? candidate.candidate_key : null,
    ...
    verification_diagnostic: bounded.verification_diagnostic ?? null,
    rollback: bounded.rollback ?? { attempted: false, verified: false, listings_deleted: 0, observations_deleted: 0, listings_restored: 0, observations_restored: 0 },
  };
}
...
function sanitizeOperations(value = {}) { return { listings: (value.listings ?? []).map((row) => ({ id: clean(row.id), operation: ["insert", "update", "unchanged"].includes(row.operation) ? row.operation : "unknown" })), observations: (value.observations ?? []).map((row) => ({ id: clean(row.id), operation: ["insert", "update", "unchanged"].includes(row.operation) ? row.operation : "unknown" })), durable_run: ["insert", "update", "unchanged"].includes(value.durable_run) ? value.durable_run : "unchanged" }; }
