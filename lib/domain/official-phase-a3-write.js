...
export function authorizeOfficialPhaseA3Write({
  ...
  return {
    ...
    expectation: {
      ...
      // Rerelease/unresolved/hold sub-buckets are read-only audit metadata.
      // They may move between full live rescans without changing the variants-only write set.
      // The stable write-contract digest plus the frozen total/safe counts still fail closed
      // on any identity, scope, or database drift that could affect Production writes.
    },
    database: {
