...
const APPROVAL_PATTERN = /^APPROVE_GACHA_PHASE_A3_WRITE:([0-9a-f]{40}):(\d+):(sha256:[0-9a-f]{64})$/;
...
export function buildOfficialPhaseA3WriteResult({
  ...
  const committedOrUnknown = ["committed", "committed_post_verify_failed", "commit_outcome_unknown"].includes(transactionState);
  const result = {
    ...
    database: {
      ...
      writes: committedOrUnknown ? nonNegativeInt(transaction?.database_writes) : 0,
    },
    transaction: {
...
export function formatOfficialPhaseA3WriteResultMarkdown(result) {
  return [
    ...
    "- Database writes: " + result.database.writes,
