...
import { PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE } from "../fetchers/market-p1-distinct-evidence-query-planner.js";
...
import { classifyMarketEvidence, MARKET_EVIDENCE_TIERS } from "./market-evidence.js";
...
export const MARKET_P1_BOUNDED_CONFIRMATION = "APPROVE_MARKET_P1_BOUNDED_CANARY_V1";
export const MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS = 5;
export const MARKET_P1_BOUNDED_MAX_WRITES = 1;
export const MARKET_P1_BOUNDED_CANARY_WRITES = 1;
export const MARKET_P1_BOUNDED_POLICY = Object.freeze({
  version: 1,
  priority: 1,
  release: "released",
  query_profile: PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE,
  max_selected_variants: MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS,
  max_persistence_candidates: MARKET_P1_BOUNDED_MAX_WRITES,
  canary_persistence_candidates: MARKET_P1_BOUNDED_CANARY_WRITES,
  one_variant_per_series: true,
  existing_active_single_count_before: 2,
  existing_active_single_count_after: 3,
  postwrite_market_evidence_tier: MARKET_EVIDENCE_TIERS.LISTING_GUIDE,
  price_ranking_requires_completed_count: 3,
  insert_only: true,
  merchant_identity_eligibility: false,
});
export const MARKET_P1_BOUNDED_POLICY_DIGEST = crypto.createHash("sha256").update(canonicalJson(MARKET_P1_BOUNDED_POLICY)).digest("hex");

export function expectedMarketP1BoundedApproval(headSha, candidateKey) {
  return `${MARKET_P1_BOUNDED_CONFIRMATION}:${MARKET_P1_BOUNDED_POLICY_DIGEST}:${headSha}:${candidateKey}`;
}

export function validateMarketP1BoundedInvocation(input = {}) {
  ...
  if (input.event_name !== "workflow_dispatch" || input.ref !== "refs/heads/main" || !ALLOWED_MODES.has(mode)
    || !Number.isInteger(limit) || limit < 1 || limit > MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS
    || !HEAD_SHA.test(headSha) || expectedMainSha !== headSha || originMainSha !== headSha) {
    throw new Error("P1 bounded invocation is not exactly bound to current main.");
  }
  if (mode === "dry-run") {
    if (candidateKey || text(input.approval, 300)) throw new Error("P1 bounded dry-run must not include write authorization.");
    return { mode, limit, write_authorized: false, candidate_key: null };
  }
  if (!CANDIDATE_KEY.test(candidateKey)
    || input.approval !== expectedMarketP1BoundedApproval(headSha, candidateKey)) {
    throw new Error("P1 bounded canary approval is invalid.");
  }
  return { mode, limit, write_authorized: true, candidate_key: candidateKey };
}

export function selectMarketP1BoundedCandidates({ audit = {}, diagnostic = {}, existingListings = [], limit = MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS } = {}) {
  const capacity = Number(limit);
  if (!Number.isInteger(capacity) || capacity < 1 || capacity > MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS) throw new Error("P1 bounded persistence limit is invalid.");
  if (diagnostic?.priority !== 1 || diagnostic?.query_profile !== PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE) throw new Error("P1 bounded diagnostic contract is invalid.");
  ...
  for (const variant of diagnostic.variants ?? []) {
    ...
    if (!variantId || !seriesId || Number(variant.priority) !== 1 || variant.released !== true
      || !variant.variant_type || variant.variant_type === "provisional" || existing.length !== 2
      || !hasDistinctKnownStorefronts(existing)) continue;
    for (const evidence of variant.accepted_distinct ?? []) {
      const candidate = auditCandidates.get(evidence.candidate_key);
      if (!isEligibleMarketP1BoundedCandidate(candidate, evidence)
        || compareCandidateWithExisting(candidate, evidence, existing) !== true) continue;
      ranked.push({
        candidate,
        evidence,
        existing_listings: existing,
        subject: { id: variantId, slug: variantId, name: text(variant.variant_name, 180), series_id: seriesId, variant_type: variant.variant_type, released: true },
        rank_class: existing.some((row) => normalizeProvider(evidence.provider) === normalizeProvider(row.source ?? row.raw?.provider)) ? "A" : "B",
      });
    }
  }
...
export function isEligibleMarketP1BoundedCandidate(candidate, evidence) {
...
export function buildMarketP1BoundedRows({ selected = [], workflow = {}, observed_at = new Date() } = {}) {
  if (!Array.isArray(selected) || selected.length !== MARKET_P1_BOUNDED_CANARY_WRITES) throw new Error("P1 bounded row selection is invalid.");
  ...
  if (!Number.isFinite(observed.getTime()) || !runId || !HEAD_SHA.test(headSha)) throw new Error("P1 bounded workflow identity is invalid.");
...
export function assertMarketP1BoundedPrewrite({ rows, selected = [], existingActiveListings = [], listingIdConflicts = [], observationIdConflicts = [], sourceIdentityConflicts = [], sourceUrlConflicts = [] } = {}) {
  const listingRows = rows?.listingRows ?? [];
  const observationRows = rows?.observationRows ?? [];
  if (!listingRows.length || listingRows.length > MARKET_P1_BOUNDED_MAX_WRITES || listingRows.length !== observationRows.length || selected.length !== listingRows.length) {
    throw new Error("P1 bounded prewrite batch is invalid.");
  }
  ...
  if (hasConflict || uniqueFields.some((entries) => entries.some((entry) => !entry) || new Set(entries).size !== entries.length)) throw new Error("P1 bounded prewrite identity conflict.");
  for (let index = 0; index < selected.length; index += 1) {
    ...
    const existingStorefronts = existing.map(resolveMarketplaceStorefrontEvidence);
    const independentlyVerified = compareIndependentStorefrontEvidence(candidateStorefront, existingStorefronts);
    if (existing.length !== 2 || entry.evidence?.independent_storefront_evidence !== true
      || !storefrontIdentityKey(candidateStorefront) || existingStorefronts.some((value) => !storefrontIdentityKey(value))
      || new Set(existingStorefronts.map(storefrontIdentityKey)).size !== 2
      || independentlyVerified !== true
      || existing.some((value) => listingIdentityConflicts(row, value))
      ...
      throw new Error("P1 bounded prewrite storefront or coverage state changed.");
    }
  }
  return true;
}

export function validateMarketP1BoundedPostwrite({ beforeCounts, afterCounts, selected = [], activeListingsAfter = [] } = {}) {
  const before = sanitizeCounts(beforeCounts);
  const after = sanitizeCounts(afterCounts);
  const count = selected.length;
  if (!before || !after || count !== MARKET_P1_BOUNDED_CANARY_WRITES) throw new Error("P1 bounded postwrite counts are unavailable.");
  const expected = { market_listings: count, market_listing_observations: count };
  for (const key of COUNT_KEYS) {
    const delta = after[key] - before[key];
    if (delta !== (expected[key] ?? 0)) throw new Error("P1 bounded Production delta is unexpected.");
  }
  const byVariant = groupEligibleExistingListings(activeListingsAfter);
  const entry = selected[0];
  const targetListings = (byVariant.get(entry.candidate.target.variant_id) ?? []);
  if (targetListings.length !== 3) throw new Error("P1 bounded target coverage did not move from two to three.");
  const evidence = classifyMarketEvidence({ subject: entry.subject, listings: activeListingsAfter, scope: "variant" });
  if (evidence.tier !== MARKET_EVIDENCE_TIERS.LISTING_GUIDE || evidence.activeCount !== 3
    || evidence.label !== "出品価格の目安" || evidence.primaryPrice === null
    || evidence.minimumPrice === null || evidence.maximumPrice === null
    || evidence.eligibleForPriceRanking !== (evidence.completedCount >= 3)) {
    throw new Error("P1 bounded listing guide verification failed.");
  }
  return {
    verified: true,
    deltas: Object.fromEntries(COUNT_KEYS.map((key) => [key, after[key] - before[key]])),
    target_active_listing_count_after: 3,
    market_evidence: sanitizePostwriteEvidence(evidence),
  };
}

export async function persistMarketP1Bounded({ rows, selected, store, beforeCounts } = {}) {
  if (typeof store?.fetchActiveEligibleListingsByVariantIds !== "function" || typeof store?.fetchP1PrewriteConflicts !== "function") {
    throw new Error("P1 bounded prewrite store is incomplete.");
  }
  const [freshExisting, conflicts] = await Promise.all([
    ...
    store.fetchP1PrewriteConflicts(rows),
  ]);
  assertMarketP1BoundedPrewrite({ rows, selected, existingActiveListings: freshExisting, ...conflicts });
  const outcome = await persistMarketBounded({
    listingRows: rows.listingRows,
    observationRows: rows.observationRows,
    store,
    persistencePolicy: MARKET_BOUNDED_PERSISTENCE_POLICIES.p1_distinct_v1,
  });
  try {
    ...
    return { ...outcome, postwrite: validateMarketP1BoundedPostwrite({ beforeCounts, afterCounts, selected, activeListingsAfter }) };
  } catch (error) {
    ...
    error.p1_bounded_rollback = rollback;
    throw error;
  }
}

export function buildMarketP1BoundedArtifact(input = {}) {
  ...
  const value = {
    schema_version: 1,
    kind: "priority_1_bounded_persistence",
    status,
    mode,
    priority: 1,
    query_profile: PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE,
    policy_digest: MARKET_P1_BOUNDED_POLICY_DIGEST,
    ...
    limits: { selected_variants: MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS, persistence_candidates: MARKET_P1_BOUNDED_MAX_WRITES, canary_candidates: MARKET_P1_BOUNDED_CANARY_WRITES },
    ...
    selected_candidates: selected.map((entry) => ({
      ...
      existing_listings: entry.existing_listings.map(sanitizeExistingListingForArtifact),
    })),
  ...
  validateMarketP1BoundedArtifact(value);
  return value;
}

export function validateMarketP1BoundedArtifact(value) {
  if (value?.schema_version !== 1 || value.kind !== "priority_1_bounded_persistence" || value.priority !== 1
    || value.query_profile !== PRIORITY_ONE_DISTINCT_EVIDENCE_QUERY_PROFILE || value.policy_digest !== MARKET_P1_BOUNDED_POLICY_DIGEST
    || !Array.isArray(value.selected_candidates) || value.selected_candidates.length > MARKET_P1_BOUNDED_MAX_SELECTED_VARIANTS
    || value.selected_candidates.some((candidate) => !CANDIDATE_KEY.test(candidate.candidate_key) || !candidate.variant_id || !candidate.series_id
      ...
      || !Array.isArray(candidate.existing_listings) || candidate.existing_listings.length !== 2
      || candidate.existing_listings.some((existing) => !existing.listing_id || !existing.source_listing_id
        || !existing.canonical_listing_identity || !existing.source_url || !existing.storefront_id || !existing.storefront_identity_source))) {
    throw new Error("P1 bounded artifact is invalid.");
  }
  ...
  if (value.write_eligible !== shouldBeWriteEligible
    ...
    || (value.status === "succeeded" && (!shouldBeWriteEligible || value.database_writes !== 2 || value.postwrite?.verified !== true
      || value.postwrite?.target_active_listing_count_after !== 3
      || value.postwrite?.market_evidence?.tier !== MARKET_EVIDENCE_TIERS.LISTING_GUIDE
      || value.postwrite?.market_evidence?.label !== "出品価格の目安"
      || value.postwrite?.market_evidence?.eligible_for_price_ranking !== (value.postwrite?.market_evidence?.completed_count >= 3)))
    ...
    throw new Error("P1 bounded artifact outcome is inconsistent.");
  }
  return true;
}

export function renderMarketP1BoundedArtifactMarkdown(value) {
  validateMarketP1BoundedArtifact(value);
  return `${[
    "# Priority 1 bounded persistence",
    "",
    `- Status: ${value.status}`,
    `- Mode: ${value.mode}`,
    ...
    `- Postwrite evidence tier: ${value.postwrite?.market_evidence?.tier ?? "not-run"}`,
    `- Eligible for price ranking: ${value.postwrite?.market_evidence?.eligible_for_price_ranking ?? false}`,
    "",
...
function buildRows(entry, { runId, headSha, observedAt }) {
  if (!isEligibleMarketP1BoundedCandidate(entry.candidate, entry.evidence)) throw new Error("P1 bounded candidate is not eligible.");
  ...
  if (!source || !sourceListingId || !sourceUrl || !listingId || !storefront.storefront_id) throw new Error("P1 bounded marketplace identity is incomplete.");
  const marker = { stage: "p1-bounded-persistence", workflow_run_id: runId, head_sha: headSha, policy_digest: MARKET_P1_BOUNDED_POLICY_DIGEST, candidate_key: candidate.candidate_key };
  ...
  const listing = {
    ...
    raw: {
      ...
      ...storefront,
      p1_bounded_persistence: marker,
    },
  };
  const identity = resolveBoundedMarketplaceIdentity(listing);
  if (!identity.complete || identity.derivedId !== listing.id || identity.sourceListingId !== sourceListingId || identity.publicUrl !== sourceUrl) throw new Error("P1 bounded marketplace identity is inconsistent.");
  const observation = {
    id: stableId("market-p1-bounded-observation", runId, candidate.candidate_key, listingId),
    ...
    raw: { p1_bounded_persistence: marker },
  };
...
function sanitizeExistingListingForArtifact(row = {}) {
  ...
  const identity = resolveBoundedMarketplaceIdentity(row);
  return {
    listing_id: text(row.id, 140),
    provider: normalizeProvider(row.source ?? row.raw?.provider),
    source_listing_id: text(identity.sourceListingId, 140),
    canonical_listing_identity: identity.complete ? buildMarketCandidateKey({ source: { provider: identity.provider, listing_id: identity.sourceListingId, public_url: identity.publicUrl } }) : null,
    ...
    active_eligible_listing_count_before: 2,
  };
}

function hasDistinctKnownStorefronts(existing = []) {
  const storefronts = existing.map(resolveMarketplaceStorefrontEvidence);
  const keys = storefronts.map(storefrontIdentityKey);
  return keys.every(Boolean) && new Set(keys).size === existing.length;
}

function compareCandidateWithExisting(candidate, evidence, existing = []) {
  if (evidence?.classification !== "accepted_distinct" || (evidence.existing_match_fields ?? []).length > 0) return false;
  const candidateStorefront = resolveSelectedCandidateStorefront({ evidence });
  const existingStorefronts = existing.map(resolveMarketplaceStorefrontEvidence);
  if (compareIndependentStorefrontEvidence(candidateStorefront, existingStorefronts) !== true) return false;
  const provider = normalizeProvider(evidence.provider);
  const sourceListingId = text(evidence.source_listing_id, 140);
  const publicUrl = sanitizeMarketPublicUrl(evidence.public_url);
  const canonicalPublicUrl = canonicalizeBoundedMarketplaceUrl(evidence.public_url);
  if (!provider || !sourceListingId || !publicUrl || !canonicalPublicUrl) return false;
  const candidateKey = buildMarketCandidateKey({ source: { provider, listing_id: sourceListingId, public_url: publicUrl } });
  if (candidateKey !== candidate?.candidate_key) return false;
  return existing.every((row) => {
    const identity = resolveBoundedMarketplaceIdentity(row);
    const existingKey = buildMarketCandidateKey({ source: { provider: identity.provider, listing_id: identity.sourceListingId, public_url: sanitizeMarketPublicUrl(row.source_url) } });
    return identity.complete
      && row.id !== sourceListingId
      && existingKey !== candidateKey
      && !(identity.provider === provider && identity.sourceListingId === sourceListingId)
      && identity.publicUrl !== canonicalPublicUrl;
  });
}

function listingIdentityConflicts(desired, existing) {
  const left = resolveBoundedMarketplaceIdentity(desired);
  const right = resolveBoundedMarketplaceIdentity(existing);
  if (!left.complete || !right.complete) return true;
  return desired.id === existing.id
    || (left.provider === right.provider && left.sourceListingId === right.sourceListingId)
    || left.publicUrl === right.publicUrl
    || left.derivedId === right.derivedId;
}

function sanitizePostwriteEvidence(value = {}) {
  return {
    tier: value.tier,
    label: value.label,
    active_count: Number(value.activeCount),
    completed_count: Number(value.completedCount),
    primary_price: Number(value.primaryPrice),
    minimum_price: Number(value.minimumPrice),
    maximum_price: Number(value.maximumPrice),
    eligible_for_price_ranking: value.eligibleForPriceRanking === true,
  };
}
