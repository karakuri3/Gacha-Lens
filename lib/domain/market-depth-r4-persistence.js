...
export function verifyMarketDepthR4Committed({
  ...
  for (const { candidate, observationId, listing, observation } of expected) {
    ...
    const sourceTime = frozen.source_r3_generated_at;
    if (!identity.complete || identity.derivedId !== candidate.listing_id
      ...
      || listing.classification_reason !== "variant_and_parent_evidence_confirmed"
      || Number(listing.classification_confidence) < MARKET_DEPTH_R4_CONFIDENCE_FLOOR
      || Number(listing.confidence) < MARKET_DEPTH_R4_CONFIDENCE_FLOOR
      || listing.status !== "active" || listing.sold_at !== null || Number(listing.price) !== candidate.price
      || listing.source !== source || listing.source_type !== "marketplace"
      || iso(listing.listed_at) !== sourceTime || iso(listing.last_observed_at) !== sourceTime
      || marker?.observation_key !== frozen.observation_key
      ...
      || iso(marker?.source_r3_generated_at) !== sourceTime
      ...
      || observation.source !== source || iso(observation.observed_at) !== sourceTime
      || observationMarker?.observation_key !== frozen.observation_key
      ...
      || observationMarker?.source_r3_run_id !== frozen.source_r3_run_id
      || observationMarker?.source_r3_main_sha !== frozen.source_r3_main_sha
      || observationMarker?.source_r3_artifact_digest !== frozen.source_r3_artifact_digest
      || iso(observationMarker?.source_r3_generated_at) !== sourceTime
