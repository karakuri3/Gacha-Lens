import {
  ...
  buildOfficialRestockWriteValues,
...
export const OFFICIAL_READ_ONLY_AUDIT_SCHEMA_VERSION = 3;
...
export function buildOfficialReadOnlyAudit({
  ...
  for (const record of formalRecords) {
    ...
    const seriesValues = buildOfficialSeriesWriteValues(resolvedRecord);
    const seriesOperation = byId
      ? (digestOfficialRow("series", byId) === digestOfficialRow("series", seriesValues) ? "none" : "update")
      : "insert";
    const seriesApply = buildOfficialApplyOperation({
      table: "series",
    ...
    for (const variant of asArray(resolvedRecord.variants)) {
      ...
      const values = buildOfficialVariantWriteValues(variant, resolvedRecord);
      const operation = existing
        ? (digestOfficialRow("variants", existing) === digestOfficialRow("variants", values) ? "none" : "update")
        : "insert";
      ...
      variantPlans.push({
        ...
        operation,
        existing_review_required: existing?.review_required === true,
        apply: buildOfficialApplyOperation({
          table: "variants",
          operation,
          values,
          existing,
    ...
    if (rereleasePlan.event) {
      ...
      sanitizedRereleaseEvent.existing_review_required = existingEvent?.review_required === true;
      restockEventApply = buildOfficialApplyOperation({
        table: "restock_events",
        operation,
        values: buildOfficialRestockWriteValues(rereleasePlan.event, sanitizedRereleaseEvent.evidence),
    ...
    const applyContract = {
      ...
      series: seriesApply,
      variants: variantPlans.map((variant) => variant.apply),
      restock_event: restockEventApply,
      deletes: 0,
      cleanup_operations: 0,
      import_issue_writes: 0,
    };
    applyContract.canonical_digest = officialCanonicalDigest(applyContract);
    ...
    candidates.push({
      ...
      blockers: rereleasePlan.blocker ? [rereleasePlan.blocker] : [],
      variants: variantPlans.map(({ apply, ...variant }) => variant),
...
export function validateOfficialReadOnlyAudit(report) {
  if (report?.schema_version !== OFFICIAL_READ_ONLY_AUDIT_SCHEMA_VERSION || report?.report_type !== "official_read_only_live_audit") {
  ...
  for (const candidate of asArray(report?.plan?.candidates)) validateCandidateApplyContract(candidate);
...
function validateCandidateApplyContract(candidate) {
  const contract = candidate?.apply_contract;
  if (contract?.schema_version !== OFFICIAL_APPLY_CONTRACT_SCHEMA_VERSION
    || contract?.deletes !== 0 || contract?.cleanup_operations !== 0 || contract?.import_issue_writes !== 0) {
    throw new Error("Official candidate apply contract is invalid.");
  }
  validateOfficialApplyOperation(contract.series, "series");
  if (contract.series.id !== candidate.series_id || contract.series.operation !== candidate.operation) {
    throw new Error("Official series apply identity does not match its candidate.");
  }
  const variants = asArray(contract.variants);
  if (variants.length !== candidate.variant_count || duplicateValues(variants.map((entry) => entry.id)).length) {
    throw new Error("Official variant apply contract is incomplete or duplicated.");
  }
  for (const operation of variants) {
    validateOfficialApplyOperation(operation, "variants");
    if (operation.values.series_id !== candidate.series_id) throw new Error("Official variant apply parent does not match its candidate.");
  }
  const summarizedVariants = asArray(candidate.variants)
    .map((entry) => ({ id: entry.id, operation: entry.operation }))
    .sort((left, right) => left.id.localeCompare(right.id, "en"));
  const appliedVariants = variants
    .map((entry) => ({ id: entry.id, operation: entry.operation }))
    .sort((left, right) => left.id.localeCompare(right.id, "en"));
  if (JSON.stringify(summarizedVariants) !== JSON.stringify(appliedVariants)) {
    throw new Error("Official variant apply summary does not match its contract.");
  }
  if (contract.restock_event) {
    validateOfficialApplyOperation(contract.restock_event, "restock_events");
    const values = contract.restock_event.values;
    if (values.series_id !== candidate.series_id || values.variant_id !== null || values.matched_variant_id !== null
      ...
      throw new Error("Official rerelease apply contract is invalid.");
    }
    if (candidate.restock_event?.id !== contract.restock_event.id
      || candidate.restock_event?.operation !== contract.restock_event.operation) {
      throw new Error("Official rerelease apply summary does not match its contract.");
    }
  } else if (candidate.restock_event) {
    throw new Error("Official rerelease apply contract is missing.");
  }
  const expectedDigest = text(contract.canonical_digest);
  const clone = structuredClone(contract);
  delete clone.canonical_digest;
  if (!/^sha256:[0-9a-f]{64}$/.test(expectedDigest) || officialCanonicalDigest(clone) !== expectedDigest) {
    throw new Error("Official candidate apply digest does not match its canonical content.");
  }
}
