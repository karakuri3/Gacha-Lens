...
export function buildOfficialReadOnlyAudit({
  ...
  for (const record of formalRecords) {
    ...
    const seriesOperation = byId ? (seriesChanged(byId, record) ? "update" : "none") : "insert";
    ...
    for (const variant of asArray(record.variants)) {
      ...
      const operation = existing ? (variantChanged(existing, variant, record) ? "update" : "none") : "insert";
  ...
  const report = {
    ...
    plan: {
      ...
      would_insert: { series: newSeriesCount, variants: newVariantCount, import_issues: 0 },
      would_update: { series: seriesUpdates, variants: variantUpdates, import_issues: 0 },
      would_delete: { series: 0, variants: 0, import_issues: 0 },
...
export function validateOfficialReadOnlyAudit(report) {
  if (report?.schema_version !== 1 || report?.report_type !== "official_read_only_live_audit") {
...
function normalizeCountSnapshot(value) {
  ...
  return Object.fromEntries(["series", "variants", "import_issues", "review_required", "provisional_variants"]
    .map((key) => [key, nonnegative(input[key])]));
}

function isCompleteCountSnapshot(value) {
  return value && typeof value === "object"
    && ["series", "variants", "import_issues", "review_required", "provisional_variants"]
