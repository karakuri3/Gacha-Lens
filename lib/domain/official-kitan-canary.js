...
export function buildOfficialKitanReadinessAudit({ provider, catalog = {}, databaseBefore, databaseAfter, workflow = {} } = {}) {
  ...
  for (const record of asArray(provider?.records)) {
    const prepared = prepareKitanRecord(record);
    ...
    candidates.push({ series_id: canonical.id, source_product_id: canonical.source_product_id, series_name: canonical.name, official_url: canonical.official_url, operation: seriesOperation, variant_count: variantPlans.length, variants: variantPlans.map(({ apply, ...item }) => item), source_count_conflict: false, capability: canonical.capability, canonical_release: { release_date: canonical.release_date, release_month: canonical.release_month, release_week: null }, apply_contract: contract });
  }

  const selected = candidates.slice().sort((a, b) => a.series_id.localeCompare(b.series_id, "en")).at(0) || null;
  if (candidates.length > OFFICIAL_KITAN_CANARY_LIMITS.max_series) blockers.push("kitan_series_cap_exceeded");
  ...
  const report = {
    ...
    workflow: { run_id: numericId(workflow.run_id), head_sha: sha(workflow.head_sha), event_name: text(workflow.event_name) || "local" },
    ...
    plan: { candidate_count: candidates.length, selected_candidate: selected, rejected_candidates: rejected, restock_events: 0, deletes: 0, cleanup_operations: 0, import_issue_writes: 0 },
    ...
    manual_canary_ready: reportComplete && blockers.length === 0 && candidates.length === 1,
    blockers: [...new Set(blockers)].sort(),
    final_verdict: reportComplete && blockers.length === 0 && candidates.length === 1 ? "OFFICIAL_KITAN_READINESS_READY" : "OFFICIAL_KITAN_READINESS_BLOCKED",
  };
...
export function authorizeOfficialKitanCanary({ report, auditRunId, auditDigest, approval, headSha, originMainSha } = {}) {
  ...
  if (report.final_verdict !== "OFFICIAL_KITAN_READINESS_READY" || report.manual_canary_ready !== true || report.plan.candidate_count !== 1 || report.provider !== "kitan_club" || report.plan.deletes !== 0 || report.plan.cleanup_operations !== 0 || report.plan.import_issue_writes !== 0) throw kitanError("kitan_canary_audit_not_ready");
...
export function validateOfficialKitanReadinessAudit(report) {
  ...
  if (report.plan?.candidate_count !== asArray(report.plan?.selected_candidate ? [report.plan.selected_candidate] : []).length || report.plan?.candidate_count > 1) throw new Error("Kitan readiness candidate selection is invalid.");
...
export function formatOfficialKitanReadinessMarkdown(report) {
  return ["# Official Kitan readiness audit", "", `- Verdict: ${report.final_verdict}`, `- Provider: ${report.provider}`, `- Manual canary ready: ${report.manual_canary_ready}`, `- Candidate count: ${report.plan.candidate_count}`, `- Selected candidate: ${report.plan.selected_candidate?.series_id || "none"}`, `- Source count conflicts excluded: ${report.source.source_count_conflict_excluded_count}`, `- Database writes: ${report.database.writes}`, "- Deletes: 0", "- Cleanup operations: 0", `- Canonical digest: ${report.canonical_digest}`, ""].join("\n");
}

function prepareKitanRecord(record) {
  const reasons = [];
  ...
  const variants = record.variants.map((variant) => ({ id: buildKitanStableIdentity(record.source_product_id, variant.name), name: text(variant.name), image: validImage(variant.image_candidate) ? variant.image_candidate : null, released: false, price: record.price, brand: "キタンクラブ", release_date: text(record.release_date) || null, release_month: text(record.release_month) || null, official_url: canonicalUrl(record.official_url), variant_type: "normal" }));
  return { ok: true, record: { id, slug: id, source_product_id: text(record.source_product_id), name: text(record.series_name), brand: "キタンクラブ", release_date: text(record.release_date) || null, release_month: text(record.release_month) || null, price: record.price, image_url: validImage(record.series_image_candidate) ? record.series_image_candidate : null, official_url: canonicalUrl(record.official_url), released: false, variants, capability: record.capability } };
}
