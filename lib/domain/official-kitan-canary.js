...
export function buildOfficialKitanReadinessAudit({ provider, catalog = {}, databaseBefore, databaseAfter, workflow = {} } = {}) {
  ...
  const report = {
    ...
    plan: { eligible_candidate_count: candidates.length, selected_candidate_count: selected ? 1 : 0, candidate_count: selected ? 1 : 0, selected_candidate: selected, eligible_candidates: candidates.map(summarizeEligibleCandidate), automatic_candidates: candidates, rejected_candidates: rejected, restock_events: 0, deletes: 0, cleanup_operations: 0, import_issue_writes: 0 },
