...
export function validateManualMarketBoundedArmingGate(input = {}) {
  ...
  const configuredStage = normalized(input.configured_stage);
  let reasonCode = null;
...
export function approvalNonceSha256(nonce) {
  const normalizedNonce = normalized(nonce);
  if (!NONCE.test(normalizedNonce)) throw new Error("Manual bounded approval nonce is invalid.");
  return crypto.createHash("sha256").update(normalizedNonce, "utf8").digest("hex");
}
...
export function validateManualApprovalClaimReuse({ artifacts, approval_nonce_sha256, current_run_id, current_run_attempt, require_current = false } = {}) {
  ...
  const matches = artifacts.filter((artifact) => artifact?.name === claimName && artifact?.expired !== true);
