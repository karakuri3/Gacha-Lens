...
export const MARKET_BOUNDED_PERSISTENCE_POLICIES = Object.freeze({
  p1: Object.freeze({ name: "p1-bounded", max_candidates: MARKET_BOUNDED_PERSISTENCE_HARD_CAP }),
  p3_seed_v1: Object.freeze({ name: "p3-bounded-seed-v1", max_candidates: 5 }),
});
...
export async function persistMarketBounded({
  ...
  const maxCandidates = resolvePersistencePolicy(persistencePolicy);
  assertRowCardinality(listingRows, observationRows, persistencePolicy);
...
function resolvePersistencePolicy(policy) {
  if (policy === MARKET_BOUNDED_PERSISTENCE_POLICIES.p1) return MARKET_BOUNDED_PERSISTENCE_HARD_CAP;
  if (policy === MARKET_BOUNDED_PERSISTENCE_POLICIES.p3_seed_v1) return 5;
