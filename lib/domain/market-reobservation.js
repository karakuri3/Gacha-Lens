...
export function planMarketReobservation({ listing, providerResult, observedAt, observationKey } = {}) {
  ...
  if (previousObservedAt
    && observed.getTime() === previousObservedAt.getTime()
    && (priceChanged || statusChanged)) {
    return noWritePlan("provider_error", listing, {
      reason: "conflicting_equal_observation_time",
      observedAt: observed.toISOString(),
      observationKey: key,
      provider: persistedIdentity.provider,
    });
...
function validDate(value) {
  ...
  if (!(value instanceof Date) && String(value).trim() === "") return null;
