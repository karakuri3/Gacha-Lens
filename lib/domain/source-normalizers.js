import { LISTING_TYPES, MARKET_REVIEW_TYPES, SOURCE_TYPES, SOURCE_WEIGHTS, STOCK_STATUSES } from "./gacha-schema.js";
import { classifyMarketListingDetailed, resolveSeriesFromListing, resolveVariantFromListing } from "./listing-classifier.js";
...
export function normalizeMarketListing(raw = {}, catalog = {}) {
  ...
  return restoreVerifiedPersistedCompleteSetSafety(normalized, raw) ?? normalized;
}

function restoreVerifiedPersistedCompleteSetSafety(normalized = {}, persisted = {}) {
  const safety = persisted.raw?.market_safety;
  const confidence = Number(safety?.confidence);
  if (
    persisted.raw?.market_safety_assessed !== true
    || safety?.accepted !== true
    || safety?.review_required !== false
    || safety?.reason !== "series_complete_set_confirmed"
    || safety?.listing_type !== LISTING_TYPES.COMPLETE_SET
    || persisted.review_required !== false
    || text(persisted.series_id) !== text(safety.series_id)
    || persisted.listing_type !== LISTING_TYPES.COMPLETE_SET
    || safety?.market_review_type !== MARKET_REVIEW_TYPES.FULL_SET
    || persisted.market_review_type !== safety.market_review_type
    || persisted.classification_reason !== safety.reason
    || Number(persisted.classification_confidence) !== confidence
    || !Number.isFinite(confidence)
    || confidence < 0.8
    || hasValue(persisted.variant_id)
    || hasValue(persisted.matched_variant_id)
    || hasValue(safety.variant_id)
    || hasValue(safety.matched_variant_id)
  ) return null;
  ...
  return {
    ...
    series_id: text(safety.series_id),
...
export function normalizeRestockEvent(raw = {}, catalog = {}) {
  ...
  const variant = resolveVariantFromText(raw, catalog);
  const explicitSeriesId = text(raw.series_id || raw.seriesId || variant?.series_id);
  const series = catalog.series?.find((entry) => entry.id === explicitSeriesId || entry.slug === explicitSeriesId) ?? null;
  const sourceType = normalizeSourceType(raw.source_type || raw.sourceType || raw.source);
  const classification = inferRestockEvent(raw);
  const isSeriesLevel = Boolean(series
    && !raw.variant_id && !raw.variantId
    && !raw.matched_variant_id && !raw.matchedVariantId);
  const reviewRequired = (!variant && !isSeriesLevel) || classification.event_type === "unknown" || raw.review_required === true;

  return {
    normalized_entity: "restock_event",
    id: text(raw.id) || stableId("restock", variant?.id || raw.variant_id, raw.reported_at, raw.shop_name),
    variant_id: text(raw.variant_id || raw.variantId || variant?.id),
    matched_variant_id: text(raw.matched_variant_id || raw.matchedVariantId),
    series_id: explicitSeriesId,
    source_type: sourceType,
    source_weight: SOURCE_WEIGHTS[sourceType] ?? SOURCE_WEIGHTS[SOURCE_TYPES.USER_X],
    event_type: classification.event_type,
    event_label: text(raw.event_label || raw.eventLabel) || classification.event_label,
    classification_reason: text(raw.classification_reason) || classification.reason,
    classification_keywords: Array.isArray(raw.classification_keywords) ? raw.classification_keywords.map(text).filter(Boolean) : classification.keywords,
    ...
    review_required: reviewRequired,
    raw: raw.raw && typeof raw.raw === "object" ? raw.raw : raw,
  };
}
