...
export const AFFILIATE_DEMAND_PROVIDER_QUERY_STRATEGY_VERSION = 1;
export const AFFILIATE_DEMAND_PROVIDER_QUERY_PROFILE = "affiliate_demand_exact_variant_v1";

const HEAD_SHA = /^[0-9a-f]{40}$/;
const MAX_QUERY_LENGTH = 120;
const PROVIDERS = Object.freeze({
  rakuten: Object.freeze({
    aliases: new Set(["rakuten", "rakuten_ichiba"]),
  ...
  yahoo: Object.freeze({
    aliases: new Set(["yahoo", "yahoo_shopping"]),
...
export function buildAffiliateDemandProviderReadPlan({
  ...
  const requests = cohort.targets.map((target) => {
    ...
    const evidence = target.listing_ids.map((listingId) => normalizeBoundListing(listingById.get(listingId), target))
    ...
    const searchQuery = buildExactSearchQuery(seriesName, variantName);
    const requestFields = {
      ...
      query_strategy_version: AFFILIATE_DEMAND_PROVIDER_QUERY_STRATEGY_VERSION,
      query_profile: AFFILIATE_DEMAND_PROVIDER_QUERY_PROFILE,
      fallback_queries_allowed: false,
    ...
    return normalizeRequest({
      request_key: buildRequestKey(requestFields),
      ...requestFields,
    });
  });
...
function normalizeRequest(input) {
  ...
  const evidence = Array.isArray(input.listing_evidence)
    ? input.listing_evidence.map((entry) => normalizeEvidence(entry, provider)).sort(compareEvidence)
    : [];
  const searchQuery = clean(input.search_query, MAX_QUERY_LENGTH + 1);
  const expectedQuery = buildExactSearchQuery(seriesName, variantName);
  ...
  const normalizedFields = {
    ...
    fallback_queries_allowed: false,
    request_sequence: ["discovery", "affiliate_enrichment"],
    required_configuration: PROVIDERS[provider] ? [...PROVIDERS[provider].required_configuration] : [],
    max_http_attempts_per_phase: AFFILIATE_DEMAND_PROVIDER_READ_MAX_ATTEMPTS_PER_PHASE,
    persistence_authorized: false,
  };

  if (!variantId || !seriesId || !seriesName || !variantName || !PROVIDERS[provider]
    || !Number.isInteger(clicks) || clicks < 1 || evidence.length < 1
    || searchQuery !== expectedQuery
    || Number(input.query_strategy_version) !== AFFILIATE_DEMAND_PROVIDER_QUERY_STRATEGY_VERSION
    || clean(input.query_profile, 80) !== AFFILIATE_DEMAND_PROVIDER_QUERY_PROFILE
    || input.fallback_queries_allowed !== false
    ...
    || requestKey !== buildRequestKey(normalizedFields)) {
  ...
  return { request_key: requestKey, ...normalizedFields };
}
...
function buildRequestKey(fields) {
  const digest = crypto.createHash("sha256").update(canonicalJson({ version: 1, ...fields }), "utf8").digest("hex").slice(0, 20);
  return `affiliate-read-${digest}`;
}

function buildExactSearchQuery(seriesName, variantName) {
  const query = clean(`${seriesName} ${variantName} ガチャ`, 1000);
  if (!query || query.length > MAX_QUERY_LENGTH) throw new Error("Affiliate provider-read exact search query is invalid.");
  return query;
}
...
function canonicalProvider(value) {
  ...
  if (PROVIDERS.rakuten.aliases.has(normalized)) return "rakuten";
  if (PROVIDERS.yahoo.aliases.has(normalized)) return "yahoo";
  return "";
}
