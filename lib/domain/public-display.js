...
export function isCirculatingItem(item = {}) {
  ...
  return Boolean(
    (item.listing_count ?? market.listing_count ?? 0) > 0 ||
      (item.sold_count ?? market.sold_count ?? 0) > 0 ||
      (item.active_listing_count ?? market.active_listing_count ?? 0) > 0 ||
...
export function releasedPriorityScore(item = {}) {
  const market = item.market_summary ?? {};
  const sold = item.sold_count ?? market.sold_count ?? 0;
  const active = item.active_listing_count ?? market.active_listing_count ?? 0;
  return watchScore(item) * 12 + Math.max(0, item.profit_estimate ?? 0) * 0.7 + sold * 24 + active * 8;
}
...
export function trendPriorityScore(item = {}) {
  ...
  return trend * 10 + xCount * 22 + sold * 26 + active * 9 + stockMoves * 20 + Math.max(0, item.profit_estimate ?? 0) * 0.25;
}
...
export function publicTrendTags(item = {}) {
  ...
  if (xCount > 0 || (item.x_signal_score ?? 0) >= 18) tags.push("X反応強め");
