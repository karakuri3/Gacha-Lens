...
export function finalizeReadOnlyIngestionRunReport(input = {}) {
  ...
  const storedOriginForReport = storedOriginSha;
  ...
  const positiveDeltaWrites = Object.values(deltas)
    .filter((delta) => delta > 0)
    .reduce((sum, delta) => sum + delta, 0);
  const existingDatabaseWrites = nonnegativeInteger(existing?.database_writes);
  const observedDatabaseWrites = Math.max(existingDatabaseWrites, positiveDeltaWrites);
  const contractValid = Boolean(
    ...
    && existing?.result?.completed_ingestion === false
    && existing?.result?.cleanup_started === false
    && observedDatabaseWrites === 0
  );
  ...
  return buildSanitizedIngestionRunReport({
    ...
    workflow: {
      ...existing?.workflow,
      origin_main_sha: storedOriginForReport,
    },
    database: {
      before,
      after,
    ...
    database_writes: observedDatabaseWrites,
  });
}

export function didIngestionCleanupStart(value) {
  return String(value ?? "")
    .split(":")
    .map((status) => status.trim().toLowerCase())
    .some((status) => status !== "" && status !== "skipped");
}
