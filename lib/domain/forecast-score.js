...
function isObservedXReaction(reaction = {}) {
  return Boolean(text(reaction.id ?? reaction.url ?? reaction.text) && Number(reaction.confidence ?? 0) > 0);
}

function isObservedAvailabilitySignal(entry = {}) {
  return Boolean(text(entry.id ?? entry.source_url ?? entry.text) && Number(entry.confidence ?? 0) > 0);
}
