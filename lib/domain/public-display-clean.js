...
export function buildReleasedCustomerMetrics(item = {}) {
  ...
  const isSeries = item.entity_type === "series" || item.variant_type === "series";
  return [
    { label: RELEASED_METRIC_LABELS.price, value: formatYen(item.price) },
    {
      label: evidence?.label || (isSeries ? "セット価格データ不足" : "データ不足"),
      value: formatMarketEvidenceValue(evidence),
      meta: evidence?.explanation,
      tone: "highlight",
    },
...
export function buildUpcomingCustomerMetrics(item = {}) {
  const pending = item.variant_type === "provisional" || !hasEvidenceBackedForecast(item);
  const evidence = item.market_evidence;
  return [
    { label: UPCOMING_METRIC_LABELS.price, value: formatYen(item.price) },
    ...(evidence?.tier === "listing_guide"
      ? [{ label: evidence.label, value: formatMarketEvidenceValue(evidence), meta: evidence.explanation }]
      : []),
    { label: UPCOMING_METRIC_LABELS.forecast, value: pending ? "算出待ち" : formatScore(item.forecast_score), tone: "highlight" },
    { label: UPCOMING_METRIC_LABELS.upside, value: pending ? "算出待ち" : formatScore(priceUpsideScore(item)) },
    { label: UPCOMING_METRIC_LABELS.scarcity, value: pending ? "算出待ち" : formatScore(scarcityScore(item)) },
    { label: UPCOMING_METRIC_LABELS.opportunity, value: pending ? "算出待ち" : formatScore(opportunityScore(item)), tone: "highlight" },
...
export function customerTags(item = {}, isReleased = Boolean(item.is_released)) {
  const tags = [];
  if (isReleased) {
    if (watchScore(item) >= 70) tags.push("注目度上昇");
    if ((item.circulation_score ?? 0) >= 42) tags.push("流通あり");
    const sellThrough = sellThroughLabel(item.market_summary);
    if (["売れ行きあり", "動きあり"].includes(sellThrough)) tags.push("売れ行きあり");
    if (sellThrough === "出品多め") tags.push("出品増加中");
    if (stockStatusLabel(item.stock_summary || item.availability_summary) !== "未取得") tags.push("在庫動きあり");
  } else if (hasEvidenceBackedForecast(item)) {
    if (opportunityScore(item) >= 70) tags.push("注目度高め");
    if (priceUpsideScore(item) >= 68) tags.push("話題化期待");
    if (scarcityScore(item) >= 60) tags.push("入手難度高め");
    if ((item.forecast_score ?? 0) >= 70) tags.push("先行反応強め");
  }
  return [...new Set(tags)].slice(0, 4);
}

export function hasEvidenceBackedForecast(item = {}) {
  const status = item.forecast_status
    ?? item.forecast_breakdown?.evidence_status
    ?? item.forecast_summary?.evidence_status;
  if (status) return status === "ready" && Number.isFinite(item.forecast_score) && item.forecast_score > 0;
  // Compatibility for legacy/sample fixtures that predate evidence_status. Production enrichment now supplies the status.
  return Number.isFinite(item.forecast_score) && item.forecast_score > 0;
}
...
export function opportunityScore(item = {}) {
  if (!hasEvidenceBackedForecast(item)) return null;
...
export function formatPriceRange(range) {
  if (!Number.isFinite(range?.low) || !Number.isFinite(range?.high)) return "データ不足";
  return `${Math.round(range.low).toLocaleString("ja-JP")}〜${Math.round(range.high).toLocaleString("ja-JP")}円`;
}

export function formatMarketEvidenceValue(evidence = {}) {
  if (Number.isFinite(evidence.primaryPrice)) {
    if (Number.isFinite(evidence.minimumPrice) && Number.isFinite(evidence.maximumPrice) && evidence.minimumPrice !== evidence.maximumPrice) {
      return `${Math.round(evidence.minimumPrice).toLocaleString("ja-JP")}〜${Math.round(evidence.maximumPrice).toLocaleString("ja-JP")}円`;
    }
    return formatYen(evidence.primaryPrice);
  }
  const observed = evidence.observedCompletedPrices?.length
    ? evidence.observedCompletedPrices
    : evidence.observedActivePrices ?? [];
  if (observed.length > 0 && observed.length < 3) {
    const unique = [...new Set(observed.map((value) => Math.round(value)))];
    return `${unique.map((value) => `${value.toLocaleString("ja-JP")}円`).join(" / ")}（確認${observed.length}件）`;
  }
  return "データ不足";
}

export function hasPriceRankingEvidence(item = {}) {
  return Boolean((item.market_evidence ?? item.market_summary?.evidence)?.eligibleForPriceRanking);
}
