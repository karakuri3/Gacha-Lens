...
import { fetchPublicSitemapParentRows, fetchPublicSitemapRows, fetchSeriesObserverRows } from "./data/public-sitemap-identifiers";
import {
...
export async function getVariantObserverSitemapEntries() {
  const { variantSlugs } = await getPublicSitemapIdentifiers();
  return variantSlugs.map((slug) => ({ slug, updated_at: null }));
}
