...
export async function getPublicCategoryCatalogPage(name, options = {}) {
  const category = normalizeDiscoveryFacetName(name);
  if (!isMeaningfulCategoryFacetName(category)) return null;
  ...
  return {
    ...
    facet: { name: category, filter_value: category, series_count: seriesCount, variant_count: result.total },
  };
}
...
export async function getPublicDiscoveryFacetSeriesPage(type, name, options = {}) {
  ...
  const facetName = normalizeDiscoveryFacetName(name);
  if (!facetType || !isMeaningfulDiscoveryFacetName(facetName)) return null;
  const pageSize = Math.max(1, Math.min(60, Number(options.pageSize) || 60));
  ...
  if (shouldUseSupabaseRecords()) {
    ...
    return {
      ...result,
      items,
      facet: { name: facetName, series_count: result.total, variant_count: result.variantCount },
    };
  }
