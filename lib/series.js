...
const loadCachedReleasedRanking = unstable_cache(
  async () => createGachaRepository(await fetchSupabaseReleasedSignalCatalog(supabase)).listVariants(),
...
export async function getSeriesCatalogPage(options = {}) {
  ...
  return loadCachedCatalogPage(normalizeCatalogOptions(options));
}
...
export async function getParentSeriesCatalogPage(options = {}) {
  ...
  const result = await loadCachedParentSeriesPage(normalizeCatalogOptions(options));
  return {
...
export async function getSeriesCatalogCounts() {
  ...
  return loadCachedCatalogCounts();
}

export async function getCategoryCatalog() {
  if (shouldUseSupabaseRecords()) return loadCachedCategoryCatalog();
...
export async function getRankingSeries(mode = "released", scope = "variant") {
  ...
  if (scope === "series") {
    const records = mode === "released" ? await loadCachedReleasedSeriesRanking() : await loadCachedUpcomingSeriesRanking();
    return createGachaRepository(records).listParentSeries();
  }
  return mode === "released" ? loadCachedReleasedRanking() : loadCachedUpcomingRanking();
}
...
export async function getParentSeriesBySlug(slug) {
  ...
  if (shouldUseSupabaseRecords()) {
    try {
      const direct = await loadCachedParentSeries(normalizedSlug);
      if (direct) return direct;
    } catch (error) {
      console.warn(`[gacha-repository] Direct parent series fetch failed: ${error.message}`);
    }
  }
...
export async function getSeriesBySlug(slug) {
  if (shouldUseSupabaseRecords()) {
    try {
      const directVariant = await loadCachedVariant(normalizeSlugValue(slug));
...
function createGachaRepository(records = getInitialRecords()) {
  ...
  const visibleVariantRows = normalized.variants.filter((variant) => variant.variant_type !== "provisional");
...
function enrichParentSeries(parent, variants, records, indexes) {
  ...
  const marketListings = dedupeById([
    ...(indexes.marketBySeries.get(parent.id) ?? []),
    ...variants.flatMap((variant) => variant.market_listings ?? []),
  ]);
  ...
  const marketSummary = buildMarketSummary(syntheticVariant, marketListings);
  ...
  const officialAxes = deriveOfficialForecastAxes({ variant: syntheticVariant, parent, siblingCount: rawChildren.length });
  ...
  return {
    ...
    lineup_count: rawChildren.filter((variant) => variant.variant_type !== "provisional").length,
    variants,
    ...
    market_price_median: released ? marketSummary.single : null,
...
function buildRecordIndexes(records) {
  ...
  const marketBySeries = groupBy(records.marketListings, "series_id");
  ...
  const restockBySeries = groupBy(records.restockEvents, "series_id");
  const stockBySeries = groupBy(records.stockReports, "series_id");
  const xBySeries = groupBy(records.xReactions, "series_id");
