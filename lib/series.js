...
import { buildSeriesCompleteSetReference } from "./domain/market-series-complete-set-reference";
...
import { buildVariantImagePresentation } from "./domain/variant-image-presentation";
import {
...
import {
  ...
  fetchSupabaseParentSeriesCategoryCatalog,
  fetchSupabaseCatalogCounts,
  fetchSupabaseCatalogPage,
  fetchSupabaseCatalogSeries,
  fetchSupabaseCatalogVariant,
  fetchSupabasePublicDiscoveryFacetSeriesPage,
  fetchSupabasePublicParentSeriesCount,
  fetchSupabaseRelatedCatalog,
  fetchSupabaseParentSeriesCatalogPage,
  fetchSupabaseScheduleSeriesPage,
  fetchSupabasePublicVariantIdentifiers,
  fetchSupabaseReleasedSignalCatalog,
  fetchSupabaseReleasedSeriesSignalCatalog,
  fetchSupabaseUpcomingMonths,
  fetchSupabaseUpcomingParentSeriesMonths,
} from "./data/supabase-gacha-repository";
import {
  MAX_VARIANT_SITEMAP_SHARDS,
  VARIANT_SITEMAP_SHARD_SIZE,
  fetchPublicDiscoveryParentRows,
  fetchPublicSitemapParentRows,
  fetchPublicSitemapRows,
  fetchPublicVariantSitemapCount,
  fetchPublicVariantSitemapPage,
  fetchSeriesObserverRows,
} from "./data/public-sitemap-identifiers";
import {
...
} from "./domain/discovery-facets";
import { collectPublicCategoryFacets, isMeaningfulCategoryFacetName, paginatePublicCategoryVariants } from "./domain/category-discovery";
import { collectPublicSitemapIdentifiers, collectSeriesObserverEntries } from "./domain/sitemap-publication";
import {
...
const loadCachedCategoryCatalog = unstable_cache(
  ...
  ["gacha-category-catalog-v2"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedUpcomingMonths = unstable_cache(
  async () => fetchSupabaseUpcomingMonths(supabase),
  ["gacha-upcoming-months-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedParentSeriesCategoryCatalog = unstable_cache(
  async () => fetchSupabaseParentSeriesCategoryCatalog(supabase),
  ["gacha-parent-series-category-catalog-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedUpcomingParentSeriesMonths = unstable_cache(
  async () => fetchSupabaseUpcomingParentSeriesMonths(supabase),
  ["gacha-upcoming-parent-series-months-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedScheduleSeriesPage = unstable_cache(
  async (options) => fetchSupabaseScheduleSeriesPage(supabase, options),
  ["gacha-schedule-series-page-v1"],
  { revalidate: PUBLIC_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);
...
const loadCachedUpcomingRanking = unstable_cache(
  async () => {
    const firstResult = await fetchSupabaseCatalogPage(supabase, { release: "upcoming", page: 1, pageSize: 120 });
    const first = {
    ...
    const remainingResults = await Promise.all(
      Array.from({ length: Math.max(0, pageCount - 1) }, (_, index) =>
        fetchSupabaseCatalogPage(supabase, { release: "upcoming", page: index + 2, pageSize: 120 })
      )
    );
...
const loadCachedUpcomingSeriesRanking = unstable_cache(
  async () => {
    const result = await fetchSupabaseParentSeriesCatalogPage(supabase, { release: "upcoming", page: 1, pageSize: 120 });
...
const loadCachedParentSeries = unstable_cache(
  ...
  ["gacha-parent-series-detail-v2"],
  { revalidate: PUBLIC_CACHE_SECONDS, tags: ["gacha-public-series"] }
);

const loadCachedPublicSitemapIdentifiers = unstable_cache(
  async () => buildPublicSitemapData(await fetchPublicSitemapRows(supabase)),
  ["gacha-public-sitemap-identifiers-v2"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedPublicSitemapParents = unstable_cache(
  async () => fetchPublicSitemapParentRows(supabase),
  ["gacha-public-sitemap-parents-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedPublicDiscoveryParents = unstable_cache(
  async () => fetchPublicDiscoveryParentRows(supabase),
  ["gacha-public-discovery-parents-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedPublicVariantSitemapCount = unstable_cache(
  async () => fetchPublicVariantSitemapCount(supabase),
  ["gacha-public-variant-sitemap-count-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);

const loadCachedPublicVariantSitemapPage = unstable_cache(
  async (page) => fetchPublicVariantSitemapPage(supabase, { page, pageSize: VARIANT_SITEMAP_SHARD_SIZE }),
  ["gacha-public-variant-sitemap-page-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);
...
export async function getParentSeriesCatalogPage(options = {}) {
  if (!shouldUseSupabaseRecords()) {
    ...
    const allItems = (await getRepository()).listParentSeries().filter((item) => {
      const released = Boolean(item.is_released ?? item.isReleased);
      const month = String(item.release_date || item.release_month || "").slice(0, 7);
      const haystack = [item.name, item.franchise, item.brand, item.category, item.slug].filter(Boolean).join(" ").toLocaleLowerCase("ja");
      return (!normalized.franchise || item.franchise === normalized.franchise)
        && (!normalized.brand || item.brand === normalized.brand)
        && (!normalized.category || item.category === normalized.category)
        && (!normalized.month || month === normalized.month)
        && (normalized.release !== "released" || released)
        && (normalized.release !== "upcoming" || !released)
        && (!normalized.q || haystack.includes(normalized.q.toLocaleLowerCase("ja")));
    });
    const pageSize = normalized.pageSize;
    const total = allItems.length;
    const lastPage = Math.max(1, Math.ceil(total / pageSize));
    const page = Math.min(normalized.page, lastPage);
    const from = (page - 1) * pageSize;
    return { items: allItems.slice(from, from + pageSize), total, page, pageSize };
  }
...
export async function getUpcomingScheduleMonths() {
  if (!shouldUseSupabaseRecords()) {
    return [...new Set((await getRepository()).listVariants()
      .filter((item) => !item.is_released)
      .map((item) => String(item.release_date || "").slice(0, 7))
      .filter((month) => /^\d{4}-\d{2}$/.test(month)))].sort();
  }
  return runDataSourceOperation("upcoming-months", () => loadCachedUpcomingMonths());
}

export async function getParentSeriesCategoryCatalog() {
  if (shouldUseSupabaseRecords()) {
    return runDataSourceOperation("parent-series-category-catalog", () => loadCachedParentSeriesCategoryCatalog());
  }
  const groups = new Map();
  for (const item of (await getRepository()).listParentSeries()) {
    const filterValue = String(item.category || "").trim();
    const name = normalizeDiscoveryFacetName(filterValue);
    if (!isMeaningfulCategoryFacetName(name)) continue;
    const key = name.toLocaleLowerCase("ja");
    const group = groups.get(key) ?? { name, filter_value: filterValue, series_count: 0, upcoming_count: 0, image_url: "" };
    if (group.filter_value !== filterValue) continue;
    group.series_count += 1;
    if (!item.is_released) group.upcoming_count += 1;
    if (!group.image_url) group.image_url = item.image_url || "";
    groups.set(key, group);
  }
  return [...groups.values()].sort((a, b) => b.series_count - a.series_count || a.name.localeCompare(b.name, "ja"));
}

export async function getUpcomingParentSeriesScheduleMonths() {
  if (!shouldUseSupabaseRecords()) {
    return [...new Set((await getRepository()).listParentSeries()
      .filter((item) => !item.is_released)
      .map((item) => String(item.release_date || item.release_month || "").slice(0, 7))
      .filter((month) => /^\d{4}-\d{2}$/.test(month)))].sort();
  }
  return runDataSourceOperation("upcoming-parent-series-months", () => loadCachedUpcomingParentSeriesMonths());
}

export async function getScheduleParentSeriesPage(options = {}) {
  if (!shouldUseSupabaseRecords()) {
    return getParentSeriesCatalogPage(options);
  }
  return runDataSourceOperation(
    "schedule-series-page",
    () => loadCachedScheduleSeriesPage(normalizeCatalogOptions(options))
  );
}
...
function normalizeCatalogOptions(options = {}) {
  return {
    ...
    release: String(options.release || options.filter || "all"),
    ...
    franchise: String(options.franchise || "").normalize("NFKC").trim().replace(/\s+/g, " ").slice(0, 120),
    brand: String(options.brand || "").normalize("NFKC").trim().replace(/\s+/g, " ").slice(0, 120),
    month: String(options.month || "").trim().slice(0, 7),
  };
}
...
export async function getPublicSitemapIdentifiers() {
  if (shouldUseSupabaseRecords()) {
    return runDataSourceOperation(
      "public-sitemap-identifiers",
      () => loadCachedPublicSitemapIdentifiers()
    );
  }

  const rows = (await getRepository()).listVariants().map((item) => ({
    ...
    parent: {
      ...
      franchise: item.parent_series?.franchise || item.character || "",
      brand: item.parent_series?.brand || item.brand || "",
      category: item.parent_series?.category || item.category || "",
    },
  }));
  return buildPublicSitemapData(rows);
}

export async function getPublicRootSitemapIdentifiers() {
  if (shouldUseSupabaseRecords()) {
    const rows = await runDataSourceOperation(
      "public-root-sitemap-identifiers",
      () => loadCachedPublicSitemapParents()
    );
    return buildPublicParentSitemapData(rows);
  }
  const { franchises, brands, categories, parentSeriesSlugs } = await getPublicSitemapIdentifiers();
  return { franchises, brands, categories, parentSeriesSlugs };
}

export async function getVariantObserverSitemapShardCount() {
  const total = shouldUseSupabaseRecords()
    ? await runDataSourceOperation(
        "public-variant-sitemap-count",
        () => loadCachedPublicVariantSitemapCount()
      )
    : (await getSeriesSlugs()).length;
  const shardCount = Math.ceil(total / VARIANT_SITEMAP_SHARD_SIZE);
  if (shardCount > MAX_VARIANT_SITEMAP_SHARDS) {
    throw new Error(`Variant sitemap requires more than ${MAX_VARIANT_SITEMAP_SHARDS} shards`);
  }
  return shardCount;
}

export async function getVariantObserverSitemapEntries(page = 1) {
  const shard = Math.max(1, Math.floor(Number(page) || 1));
  if (shard > MAX_VARIANT_SITEMAP_SHARDS) {
  ...
  if (shouldUseSupabaseRecords()) {
    const rows = await runDataSourceOperation(
      "public-variant-sitemap-page",
      () => loadCachedPublicVariantSitemapPage(shard)
    );
    slugs = rows.map((row) => String(row?.slug || "").trim()).filter(Boolean);
  } else {
    const all = await getSeriesSlugs();
    const from = (shard - 1) * VARIANT_SITEMAP_SHARD_SIZE;
    slugs = all.slice(from, from + VARIANT_SITEMAP_SHARD_SIZE);
  }
...
export async function getSeriesObserverSitemapEntries({ today = jstDate() } = {}) {
  if (shouldUseSupabaseRecords()) {
    const [parentRows, rows] = await Promise.all([
      runDataSourceOperation(
        "public-series-sitemap-parents",
        () => loadCachedPublicSitemapParents()
      ),
      runDataSourceOperation(
        "series-observer-sitemap",
        () => fetchSeriesObserverRows(supabase, { today })
      ),
    ]);
    const { parentSeriesSlugs } = buildPublicParentSitemapData(parentRows);
    return collectSeriesObserverEntries({ parentSeriesSlugs, seriesRows: rows, today });
  }
  const identifiers = await getPublicSitemapIdentifiers();
  const rows = (await getRepository()).listParentSeries().map((item) => ({
    slug: item.slug,
    name: item.name,
    brand: item.brand,
  ...
  return collectSeriesObserverEntries({ parentSeriesSlugs: identifiers.parentSeriesSlugs, seriesRows: rows, today });
}

export async function getPublicDiscoveryFacets() {
  if (shouldUseSupabaseRecords()) {
    const rows = await runDataSourceOperation(
      "public-discovery-parents",
      () => loadCachedPublicDiscoveryParents()
    );
    return buildPublicDiscoveryFacetsFromParentRows(rows);
  }
  const { franchises = [], brands = [], categories = [] } = await getPublicSitemapIdentifiers();
  return { franchises, brands, categories };
}

export async function getPublicCategoryCatalogPage(name, options = {}) {
  const categories = discoveryFacetLookupCandidates(name).filter(isMeaningfulCategoryFacetName);
  if (!categories.length) return null;
  const pageSize = Math.max(1, Math.min(60, Number(options.pageSize) || 60));
  const candidatePage = Number(options.page);
  const requestedPage = Number.isFinite(candidatePage) ? Math.max(1, Math.floor(candidatePage)) : 1;

  for (const category of categories) {
    let result;
    let seriesCount;
    if (shouldUseSupabaseRecords()) {
      [result, seriesCount] = await Promise.all([
        getSeriesCatalogPage({ category, page: requestedPage, pageSize, sort: "newest" }),
        runDataSourceOperation("public-category-series-count", () => fetchSupabasePublicParentSeriesCount(supabase, "category", category)),
      ]);
    } else {
      const items = (await getRepository()).listVariants().filter(
        (item) => normalizeDiscoveryFacetName(item.parent_series?.category ?? item.category ?? "") === category,
      );
      result = paginatePublicCategoryVariants(items, { page: requestedPage, pageSize });
      seriesCount = new Set(items.map((item) => item.series_id).filter(Boolean)).size;
    }

    // A successful targeted query is authoritative. Never turn cache skew into a 404.
    if (result.total === 0) continue;
    const canonical = normalizeDiscoveryFacetName(
      result.items.find((item) => item.category || item.parent_series?.category)?.category
        ?? result.items.find((item) => item.parent_series?.category)?.parent_series?.category
        ?? category,
    );
    return {
      ...result,
      totalPages: Math.max(1, Math.ceil(result.total / result.pageSize)),
      facet: { name: canonical, filter_value: canonical, series_count: seriesCount, variant_count: result.total },
    };
  }
  return null;
}

export async function getPublicCategorySeriesPage(name, options = {}) {
  const requestedName = normalizeDiscoveryFacetName(name);
  if (!isMeaningfulCategoryFacetName(requestedName)) return null;
  const pageSize = Math.max(1, Math.min(60, Number(options.pageSize) || 60));
  const requestedPage = Math.max(1, Number(options.page) || 1);
  const category = (await getParentSeriesCategoryCatalog()).find((item) => (
    normalizeDiscoveryFacetName(item.name) === requestedName
  ));
...
export async function getPublicDiscoveryFacetSeriesPage(type, name, options = {}) {
  const facetType = type === "brand" ? "brand" : type === "franchise" ? "franchise" : "";
  const facetNames = discoveryFacetLookupCandidates(name).filter(isMeaningfulDiscoveryFacetName);
  if (!facetType || !facetNames.length) return null;
  const pageSize = Math.max(1, Math.min(60, Number(options.pageSize) || 60));
  const requestedPage = Math.max(1, Number(options.page) || 1);
  ...
  for (const facetName of facetNames) {
    if (shouldUseSupabaseRecords()) {
      const result = await runDataSourceOperation(
        "public-discovery-series-page",
        () => fetchSupabasePublicDiscoveryFacetSeriesPage(supabase, {
          type: facetType,
          value: facetName,
          page: requestedPage,
          pageSize,
      ...
      if (result.total === 0) continue;
      const items = createGachaRepository(result.records).listParentSeries();
      if (items.length !== result.records.series.length) {
        throw new Error("Public discovery series page failed publication validation");
      }
      const canonical = normalizeDiscoveryFacetName(items[0]?.[facetType] ?? facetName);
      return {
        ...result,
        items,
        facet: { name: canonical, series_count: result.total, variant_count: result.variantCount },
      };
    }

    const allItems = (await getRepository()).listParentSeries().filter((item) => (
      normalizeDiscoveryFacetName(item[facetType]) === facetName
    ));
    if (!allItems.length) continue;
    const page = paginatePublicDiscoveryFacetSeries(allItems, { page: requestedPage, pageSize });
    const canonical = normalizeDiscoveryFacetName(page.items[0]?.[facetType] ?? facetName);
    return {
      ...page,
      items: page.items,
      facet: {
        name: canonical,
        series_count: allItems.length,
        variant_count: allItems.reduce((count, item) => count + (item.variants?.length ?? 0), 0),
      },
    };
  }
  return null;
...
function buildPublicSitemapData(rows) {
  return {
    ...collectPublicSitemapIdentifiers(rows),
    ...collectPublicDiscoveryFacets(rows),
    discoveryFacetCatalogs: collectPublicDiscoveryFacetCatalogs(rows),
    categories: collectPublicCategoryFacets(rows),
  };
}

function buildPublicDiscoveryFacetsFromParentRows(rows = []) {
  return {
    franchises: collectParentDiscoveryFacets(rows, "franchise", isMeaningfulDiscoveryFacetName),
    brands: collectParentDiscoveryFacets(rows, "brand", isMeaningfulDiscoveryFacetName),
    categories: collectParentCategoryDiscoveryFacets(rows),
  };
}

function collectParentDiscoveryFacets(rows, field, isMeaningful) {
  const groups = new Map();
  for (const row of Array.isArray(rows) ? rows : []) {
    const seriesId = String(row?.id || "").trim();
    const seriesSlug = String(row?.slug || "").trim();
    const name = normalizeDiscoveryFacetName(row?.[field]);
    const variantCount = embeddedVariantCount(row?.variants);
    if (!seriesId || !seriesSlug || !isMeaningful(name) || variantCount <= 0) continue;
    const key = name.toLocaleLowerCase("ja");
    const group = groups.get(key) ?? { name, seriesCounts: new Map() };
  ...
  return [...groups.values()]
    ...
    .filter((facet) => facet.series_count >= DISCOVERY_FACET_MIN_SERIES)
    .sort((a, b) => a.name.localeCompare(b.name, "ja"));
}

function collectParentCategoryDiscoveryFacets(rows) {
  const groups = new Map();
  for (const row of Array.isArray(rows) ? rows : []) {
    const seriesId = String(row?.id || "").trim();
    const seriesSlug = String(row?.slug || "").trim();
    const rawCategory = String(row?.category ?? "");
    const name = normalizeDiscoveryFacetName(rawCategory);
    const variantCount = embeddedVariantCount(row?.variants);
    if (!seriesId || !seriesSlug || !isMeaningfulCategoryFacetName(name) || variantCount <= 0) continue;
    const key = name.toLocaleLowerCase("ja");
    const group = groups.get(key) ?? { name, rawValues: new Set(), seriesCounts: new Map() };
  ...
  return [...groups.values()]
    .filter((group) => group.rawValues.size === 1 && group.seriesCounts.size >= DISCOVERY_FACET_MIN_SERIES)
    .map((group) => ({
      name: group.name,
...
function buildPublicParentSitemapData(rows = []) {
  const sourceRows = Array.isArray(rows) ? rows : [];
  const parentSeriesSlugs = [...new Set(
    sourceRows.map((row) => String(row?.slug || "").trim()).filter(Boolean)
  )].sort((a, b) => a.localeCompare(b, "ja"));

  return {
    parentSeriesSlugs,
    franchises: collectParentSitemapFacets(sourceRows, "franchise", isMeaningfulDiscoveryFacetName),
    brands: collectParentSitemapFacets(sourceRows, "brand", isMeaningfulDiscoveryFacetName),
    categories: collectParentSitemapFacets(sourceRows, "category", isMeaningfulCategoryFacetName, { requireSingleRawValue: true }),
  };
}

function collectParentSitemapFacets(rows, field, isMeaningful, options = {}) {
  const groups = new Map();
  for (const row of rows) {
    const seriesId = String(row?.id || "").trim();
    const rawValue = String(row?.[field] || "");
    const name = normalizeDiscoveryFacetName(rawValue);
    if (!seriesId || !isMeaningful(name)) continue;
    const key = name.toLocaleLowerCase("ja");
    const group = groups.get(key) ?? { name, seriesIds: new Set(), rawValues: new Set() };
    group.seriesIds.add(seriesId);
    group.rawValues.add(rawValue);
    groups.set(key, group);
  }
  ...
  return [...groups.values()]
    .map((group) => ({
      ...
      raw_value_count: group.rawValues.size,
    }))
    .filter((facet) => (
      facet.series_count >= DISCOVERY_FACET_MIN_SERIES
      && (!options.requireSingleRawValue || facet.raw_value_count === 1)
    ))
    .map(({ name, series_count }) => ({ name, series_count }))
    .sort((a, b) => a.name.localeCompare(b.name, "ja"));
}
...
export async function getRelatedSeries(slug, limit = 4) {
  if (shouldUseSupabaseRecords()) {
    const records = await runDataSourceOperation(
      "related-variants",
      () => fetchSupabaseRelatedCatalog(supabase, normalizeSlugValue(slug), { candidateLimit: 24 })
    );
    return records ? createGachaRepository(records).getRelatedVariants(slug, limit) : [];
  }
...
function enrichVariant(variant, records, indexes) {
  ...
  const siblingVariants = siblings.map((entry) => {
    const imagePresentation = buildVariantImagePresentation({ variant: entry, parent, siblingCount: siblings.length });
    return {
      ...entry,
      image: imagePresentation.variant_image_url,
      image_url: imagePresentation.variant_image_url,
      imageUrl: imagePresentation.variant_image_url,
      ...imagePresentation,
    };
  });
  ...
  const imagePresentation = buildVariantImagePresentation({ variant, parent, siblingCount: siblings.length });
...
function enrichParentSeries(parent, variants, records, indexes) {
  ...
  return {
    ...
    complete_set_reference: released ? buildSeriesCompleteSetReference({ series: { ...parent, ...lineupState }, listings: marketListings }) : null,
