...
const repository = createGachaRepository();

export async function getSeriesList() {
  return repository.listVariants();
}
...
export async function getSeriesBySlug(slug) {
  return repository.findVariantBySlug(slug);
}
...
export async function getSeriesDetail(slug) {
  const variant = await repository.findVariantBySlug(slug);
...
export async function getSeriesSlugs() {
  return repository.listVariants().map((item) => item.slug);
}

export async function getRelatedSeries(slug, limit = 4) {
  return repository.getRelatedVariants(slug, limit);
}
...
export function createRepositoryFromDataSource(dataSource = createStaticGachaDataSource(getMockRecords())) {
  const records = typeof dataSource.loadRecords === "function" ? dataSource.loadRecords() : dataSource;
  return createGachaRepository(records);
}

export { createGachaRepository, createRepositoryRecords };

export default repository.listVariants();

function createGachaRepository(records = getMockRecords()) {
...
function enrichVariant(variant, records) {
  ...
  const marketListings = records.marketListings.filter((entry) => entry.variant_id === variant.id);
  ...
  const profit = variant.released && Number.isFinite(singleMarket) ? singleMarket - parent.price : null;
