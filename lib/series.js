...
import {
  ...
  fetchSupabaseParentSeriesByIds,
...
import { collectPublicCategoryFacets, findPublicCategoryFacet, paginatePublicCategoryVariants } from "./domain/category-discovery";
...
export async function getPublicCategoryCatalogPage(name, options = {}) {
  const publicData = await getPublicSitemapIdentifiers();
  const facet = findPublicCategoryFacet(publicData.categories, name);
  ...
  let result;
  if (shouldUseSupabaseRecords()) {
    result = await getSeriesCatalogPage({ category: facet.filter_value, page: requestedPage, pageSize, sort: "newest" });
  } else {
    const items = (await getRepository()).listVariants().filter(
      (item) => String(item.parent_series?.category ?? item.category ?? "") === facet.filter_value,
    );
    result = paginatePublicCategoryVariants(items, { page: requestedPage, pageSize });
  }

  // The sitemap identity cache is the publication source of truth. A mismatch must not publish a mixed page.
  if (result.total !== facet.variant_count || !result.items.length) return null;
  return { ...result, totalPages: Math.max(1, Math.ceil(result.total / result.pageSize)), facet };
}

export async function getPublicDiscoveryFacetSeriesPage(type, name, options = {}) {
  const publicData = await getPublicSitemapIdentifiers();
  const catalogs = publicData.discoveryFacetCatalogs ?? { franchises: [], brands: [] };
  const facets = type === "brand" ? catalogs.brands : catalogs.franchises;
  const facet = findPublicDiscoveryFacet(facets, name);
  if (!facet) return null;

  const page = paginatePublicDiscoveryFacetSeries(facet.parent_series, options);
  const requestedIds = page.items.map((item) => item.id);
  let items;

  if (shouldUseSupabaseRecords()) {
    const records = await runDataSourceOperation(
      "public-discovery-series-page",
      () => fetchSupabaseParentSeriesByIds(supabase, requestedIds)
    );
    const byId = new Map(createGachaRepository(records).listParentSeries().map((item) => [String(item.series_id || item.id), item]));
    items = requestedIds.map((id) => byId.get(id)).filter(Boolean);
  } else {
    const byId = new Map((await getRepository()).listParentSeries().map((item) => [String(item.series_id || item.id), item]));
    items = requestedIds.map((id) => byId.get(id)).filter(Boolean);
  }

  // The identity cache owns the public population. A stale catalog must not fill a page with other series.
  if (items.length !== requestedIds.length) return null;
  return { ...page, items, facet: { name: facet.name, series_count: facet.series_count, variant_count: facet.variant_count } };
}
