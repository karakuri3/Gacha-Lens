...
async function createPreferredRepository() {
  ...
  try {
    ...
    return createGachaRepository(dbRecords);
  } catch (error) {
...
function enrichVariant(variant, records) {
  ...
  return {
    ...
    summary: variant.released
      ? `${variant.name}の単品相場、利益目安、在庫の動きを確認できます。`
      : `${variant.name}の発売前期待値を、コンプ需要・当たり枠・互換性・限定性から確認できます。`,
    description: variant.released
      ? `${parent.name}の中の個別種です。単品、レア単品、コンプセットを分けて判断します。`
      : `${parent.name}の中の個別種です。発売前のため相場や利益は出さず、狙い目度だけを表示します。`,
  };
}
