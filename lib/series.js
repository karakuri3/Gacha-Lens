...
export async function getPublicCategoryCatalogPage(name, options = {}) {
  ...
  let result;
  if (shouldUseSupabaseRecords()) {
    result = await getSeriesCatalogPage({ category: facet.name, page: requestedPage, pageSize, sort: "newest" });
  } else {
    const items = (await getRepository()).listVariants().filter((item) => item.category === facet.name);
