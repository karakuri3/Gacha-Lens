...
const loadCachedPublicSitemapIdentifiers = unstable_cache(
  async () => collectPublicSitemapIdentifiers(await fetchPublicSitemapRows(supabase)),
  ["gacha-public-sitemap-identifiers-v1"],
  { revalidate: CATALOG_CACHE_SECONDS, tags: ["gacha-public-catalog"] }
);
...
export async function getPublicSitemapIdentifiers() {
  ...
  const rows = (await getRepository()).listVariants().map((item) => ({
    ...
    parent: { id: item.series_id, slug: item.series_slug },
  }));
  return collectPublicSitemapIdentifiers(rows);
}
