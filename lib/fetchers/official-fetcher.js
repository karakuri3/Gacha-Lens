...
export async function fetchOfficialRaw(options = {}) {
  ...
  const allDetailUrls = [...new Set([...priorityDetailUrls, ...detailQueue, ...previousDetailUrls])];
  ...
  const nextDetailCursor = allDetailUrls.length
    ...
    : 0;
  const currentTakaratomyPage = Math.max(2, number(options.sourceCursors?.takaratomyPage) ?? 2);
...
function expandOfficialSourceUrls(urls, options = {}) {
  ...
  const expanded = [];
  const takaratomyCursor = Math.max(2, number(options.sourceCursors?.takaratomyPage) ?? 2);
  const takaratomyPagesPerRun = Math.max(1, number(options.takaratomyPagesPerRun ?? process.env.OFFICIAL_TARTS_PAGES_PER_RUN) ?? 4);
