const DEFAULT_OFFICIAL_SOURCE_URLS = ["https://gashapon.jp/schedule/", "https://gashapon.jp/products/"];

export async function fetchOfficialRaw(options = {}) {
  const urls = parseList(options.urls ?? process.env.OFFICIAL_SOURCE_URLS);
  const sourceUrls = urls.length ? urls : DEFAULT_OFFICIAL_SOURCE_URLS;
  const detailFetchLimit = number(options.detailFetchLimit ?? process.env.OFFICIAL_DETAIL_FETCH_LIMIT) ?? 20;
  const detailFetchDelayMs = number(options.detailFetchDelayMs ?? process.env.OFFICIAL_DETAIL_FETCH_DELAY_MS) ?? 150;
  const results = [];
  const issues = [];
  const detailQueue = [];
  ...
  const detailUrls = [...new Set(detailQueue)].slice(0, detailFetchLimit);
  for (const detailUrl of detailUrls) {
    try {
      ...
      const detail = parseOfficialDetailHtml(await response.text(), detailUrl);
...
function parseScheduleCards(body, url) {
  ...
  return weekBlocks.flatMap((weekMatch) => {
    ...
    const schedule = parseWeekLabel(weekHtml);
...
function parseScheduleCard(href, cardHtml, schedule, sourceUrl) {
  ...
  return {
    ...
    image_url: imageUrl,
    released: isProbablyReleased(schedule.release_month),
    variants: [],
    source_type: "official_site",
    review_required: true,
    raw: {
...
function parseOfficialDetailHtml(body, url) {
  ...
  return {
    record: {
      ...
      official_url: url,
      image_url: imageUrl,
      released: isProbablyReleased(parseReleaseMonth(releaseText)),
      variants,
      source_type: "official_site",
...
function parseWeekLabel(html) {
  ...
  return {
    release_month: dates[0] ? `${dates[0]}月` : "",
...
function isProbablyReleased(monthLabel) {
