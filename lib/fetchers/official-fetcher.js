import {
  applyOfficialDetailRereleaseEvidence,
  mergeOfficialRecordEvidence,
  parseOfficialRereleaseEvidence,
} from "../domain/official-rerelease.js";
...
export async function fetchOfficialRaw(options = {}) {
  ...
  const delayImpl = options.sleep ?? sleep;
  const fetchTimeoutMs = clampOfficialRequestInteger(
    options.fetchTimeoutMs ?? process.env.OFFICIAL_FETCH_TIMEOUT_MS,
    0,
    30000,
    0,
  );
  const fetchMaxAttempts = clampOfficialRequestInteger(
    options.fetchMaxAttempts ?? process.env.OFFICIAL_FETCH_MAX_ATTEMPTS,
    1,
    3,
    1,
  );
  const fetchRetryBaseDelayMs = clampOfficialRequestInteger(
    options.fetchRetryBaseDelayMs ?? process.env.OFFICIAL_FETCH_RETRY_BASE_DELAY_MS,
    0,
    5000,
    250,
  );
  const fetchRetryMaxDelayMs = clampOfficialRequestInteger(
    options.fetchRetryMaxDelayMs ?? process.env.OFFICIAL_FETCH_RETRY_MAX_DELAY_MS,
  ...
  const requestPolicy = {
    ...
    baseDelayMs: fetchRetryBaseDelayMs,
  ...
  for (const url of sourceUrls) {
    ...
    if (sourceFetchDelayMs > 0) await delayImpl(sourceFetchDelayMs);
  }
  ...
  for (const detailUrl of detailUrls) {
    try {
      const response = await fetchOfficialResponse(detailUrl, {
        ...requestPolicy,
        headers: {
    ...
    if (detailFetchDelayMs > 0) await delayImpl(detailFetchDelayMs);
  }
...
const OFFICIAL_RETRYABLE_HTTP_STATUSES = new Set([408, 425, 429, 500, 502, 503, 504]);

async function fetchOfficialResponse(url, options = {}) {
  const fetchImpl = options.fetchImpl ?? globalThis.fetch;
  const delayImpl = options.sleep ?? sleep;
  const timeoutMs = clampOfficialRequestInteger(options.timeoutMs, 0, 30000, 0);
  const maxAttempts = clampOfficialRequestInteger(options.maxAttempts, 1, 3, 1);
  const baseDelayMs = clampOfficialRequestInteger(options.baseDelayMs, 0, 5000, 250);
  const maxDelayMs = clampOfficialRequestInteger(options.maxDelayMs, baseDelayMs, 10000, 1500);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (response?.ok || !OFFICIAL_RETRYABLE_HTTP_STATUSES.has(Number(response?.status)) || attempt >= maxAttempts) {
        return response;
      }
      try {
        await response.body?.cancel?.();
      } catch {
        // Best-effort release before retry; the retry decision must not depend on body cancellation.
      }
    } catch (error) {
      if (attempt >= maxAttempts) {
        throw new Error(isOfficialTimeoutError(error)
          ? `official request timeout after ${timeoutMs}ms`
          : "official network request failed");
      }
    }

    const retryDelayMs = officialRetryDelayMs(response, attempt, baseDelayMs, maxDelayMs);
    if (retryDelayMs > 0) await delayImpl(retryDelayMs);
  }

  throw new Error("official request failed");
}

function officialRetryDelayMs(response, attempt, baseDelayMs, maxDelayMs) {
  const status = Number(response?.status);
  const retryAfter = status === 429 || status === 503
    ? parseOfficialRetryAfter(response?.headers?.get?.("retry-after"))
    : null;
  if (retryAfter != null) return Math.min(maxDelayMs, retryAfter);
  return Math.min(maxDelayMs, baseDelayMs * (2 ** Math.max(0, attempt - 1)));
}

function parseOfficialRetryAfter(value) {
...
function isOfficialTimeoutError(error) {
  return error?.name === "AbortError" || error?.name === "TimeoutError" || error?.code === "ETIMEDOUT";
}

function clampOfficialRequestInteger(value, min, max, fallback) {
...
export function parseOfficialSourceDocument(body, url, options = {}) {
  const contentType = text(options.contentType).toLowerCase();
  if (contentType.includes("json")) return parseJsonPayload(body, url);
  return parseHtmlPayload(body, url);
}

export function parseOfficialDetailDocument(body, url) {
  return isTakaratomyDetailUrl(url)
    ? parseTakaratomyDetailHtml(body, url)
    : parseOfficialDetailHtml(body, url);
}
...
function parseTakaratomyDetailHtml(body, url) {
  ...
  const parsedVariants = variantNames.map((variantName, index) => createTakaratomyVariant(id, variantName, imageUrl, index));
  const variantCountMismatch = !expectedVariantCount || parsedVariants.length !== expectedVariantCount;
  const variants = variantCountMismatch ? [] : parsedVariants;
  ...
  } else if (!expectedVariantCount) {
    issues.push(createIssue(url, "Takara Tomy Arts expected variant count was not found"));
  } else if (variantCountMismatch) {
    issues.push(createIssue(url, `Takara Tomy Arts variant count mismatch: expected ${expectedVariantCount}, parsed ${parsedVariants.length}`));
  }
...
function parseOfficialDetailHtml(body, url) {
  ...
  const imageUrl = matchMeta(body, "og:image") || matchDetailImages(body, url)[0]?.image_url || "";
  ...
  const rereleaseEvidence = parseOfficialRereleaseEvidence(stripTags(body));
  ...
  const parsedVariants = matchDetailImages(body, url)
  ...
  if (!parsedVariants.length) {
    ...
    issues.push(createIssue(url, "Detail page expected variant count was not found"));
  } else if (variantCountMismatch) {
    issues.push(createIssue(url, `Detail page variant count mismatch: expected ${expectedVariantCount}, parsed ${parsedVariants.length}`));
  }
  ...
  const record = {
      ...
      brand: "バンダイ",
      category: "ガシャポン",
      release_month: currentSchedule.release_month,
      release_week: currentSchedule.release_week,
      release_date: releaseDate,
      price,
      official_url: url,
      image_url: imageUrl,
  ...
  return {
    record: applyOfficialDetailRereleaseEvidence(record, rereleaseEvidence, currentSchedule),
    issues,
  };
}
...
function matchDetailImages(body, baseUrl) {
  return [...body.matchAll(/<li[^>]+class=["'][^"']*swiper-slide[^"']*["'][^>]*>([\s\S]*?)<\/li>/gi)]
    .map((match) => match[1].match(/<img\b[^>]*>/i)?.[0] || "")
    .filter(Boolean)
    .map((imgTag) => ({
      image_url: absoluteUrl(attributeValue(imgTag, "src"), baseUrl),
...
export function mergeOfficialRecords(previousRecords, incomingRecords) {
  const records = new Map();
  for (const incoming of [...previousRecords, ...incomingRecords]) {
    if (!incoming?.id) continue;
    records.set(incoming.id, mergeOfficialRecordEvidence(records.get(incoming.id) || {}, incoming));
  }
