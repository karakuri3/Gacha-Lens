...
export async function fetchMarketListingsRaw(options = {}) {
  ...
  const [rakutenResult, yahooResult] = await Promise.all([
    fetchRakutenMarketListingsRaw({ ...(options.rakuten ?? {}), queries }),
    fetchYahooShoppingListingsRaw({ ...(options.yahoo ?? {}), queries }),
  ]);
  ...
  return {
    ...
    configuredSources: sources.length + (rakutenResult.configuredSources ?? 0) + (yahooResult.configuredSources ?? 0),
