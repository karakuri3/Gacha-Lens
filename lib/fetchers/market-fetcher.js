...
export function describeMarketSourceConfiguration(options = {}) {
  ...
  const rakutenRootLimit = positiveInteger(options.rakuten?.queryLimit ?? process.env.RAKUTEN_MARKET_QUERY_LIMIT, 8);
  const yahooRootLimit = positiveInteger(options.yahoo?.queryLimit ?? process.env.YAHOO_SHOPPING_QUERY_LIMIT, 24);
  ...
  return {
    ...
    plannedSourceRequests: {
      ...
      rakuten_ichiba: rakutenConfigured ? boundedAttemptCount(queryCount, queryAttemptCount, rakutenRootLimit) : 0,
      yahoo_shopping: yahooConfigured ? boundedAttemptCount(queryCount, queryAttemptCount, yahooRootLimit) : 0,
    },
    ...readiness,
  };
}
...
function boundedAttemptCount(rootCount, attemptCount, rootLimit) {
  if (!rootCount) return 0;
  if (rootCount <= rootLimit) return attemptCount;
  return Math.min(attemptCount, rootLimit * 3);
}
