...
export const PRIORITY_TWO_DISTINCT_EVIDENCE_QUERY_PROFILE = "priority_2_distinct_exact_diagnostic";

export function planPriorityTwoDistinctEvidenceQueries(catalog = {}, coverageRows = [], options = {}) {
  const eligibleRows = coverageRows.filter((row) => {
    ...
    && row?.released === true
    && Number(row?.activeCount) === 1
    && Number(row?.eligibleListingCount) === 1
  ...
  for (const coverage of selection.selected) {
    ...
    const plan = buildPriorityTwoDistinctEvidenceQueriesForVariant(variant, parentSeries);
  ...
  return {
    ...
    summary: {
      ...
      eligible_priority_two_single_listing_variants: eligibleRows.length,
      skipped_unsafe_query: selection.selected.length - selected.length,
    },
  };
}

export function buildPriorityTwoDistinctEvidenceQueriesForVariant(variant, parentSeries) {
