...
export async function fetchStockRaw(options = {}) {
  ...
  const xSearchEnabled = parseBoolean(options.xSearchEnabled ?? process.env.STOCK_X_SEARCH_ENABLED ?? "false");
  const configuredXQueries = parseList(options.xQueries ?? process.env.STOCK_X_SEARCH_QUERIES);
  const xQueries = xSearchEnabled ? configuredXQueries : [];
  const xAccounts = xSearchEnabled ? parseList(options.xAccounts ?? process.env.STOCK_X_MONITOR_ACCOUNTS) : [];
...
export function describeStockFetcherSafety() {
  return {
    enabled: true,
    mode: "approved_feed_export_primary_x_optional",
    reason: "Stock and restock collection uses approved JSON/CSV feeds or exports first. X API monitoring is optional and disabled by default. Ambiguous sightings still go to import_issues.",
    acceptedInputs: ["restockEventsRaw JSON", "stockReportsRaw JSON", "stock/restock CSV export", "approved shop/official feed", "reviewed X export converted to JSON", "optional X API search results"],
    env: "STOCK_RAW_FEED_URLS or STOCK_RAW_FEED_SOURCES_JSON. Optional: STOCK_X_SEARCH_ENABLED=true with STOCK_X_SEARCH_QUERIES/STOCK_X_MONITOR_ACCOUNTS.",
