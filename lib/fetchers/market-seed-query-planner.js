...
export function normalizeRecallV5SeriesAnchor(value) {
  // V5 alone removes presentation marks before V4's NFKC pass can turn ™ into TM.
  const compact = normalizeRecallV4SeriesAlias(String(value ?? "")
  ...
  const minimal = cleanText(compact
    ...
    .replace(/\s+/g, " "));
  if ((!hasMeaningfulCompactAnchor(minimal) && minimal !== "MLB") || /^(?:japan|日本)$/i.test(minimal)) return "";
  return minimal;
}
