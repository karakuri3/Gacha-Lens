...
export async function fetchXReactionsRaw(options = {}) {
  ...
  const rawFeedUrls = parseList(options.rawFeedUrls ?? process.env.X_RAW_FEED_URLS);
  ...
  for (const url of rawFeedUrls) {
    try {
      ...
      if (!response.ok) {
        issues.push(createIssue("raw_feed", url, `HTTP ${response.status}`));
        continue;
      }
      const data = await response.json();
      records.push(...normalizeContainer(data, { source: "raw_feed", url }));
    } catch (error) {
      issues.push(createIssue("raw_feed", url, error.message));
    }
  }
