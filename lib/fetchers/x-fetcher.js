...
export async function fetchXReactionsRaw(options = {}) {
  ...
  const xApiEnabled = parseBoolean(options.apiEnabled ?? process.env.X_FETCH_ENABLED ?? "false");
  const configuredQueries = parseList(options.queries ?? process.env.X_SEARCH_QUERIES);
  const useDefaultQueries = parseBoolean(options.useDefaultQueries ?? process.env.X_USE_DEFAULT_QUERIES ?? "false");
  const queries = xApiEnabled ? (configuredQueries.length ? configuredQueries : (useDefaultQueries ? DEFAULT_X_SEARCH_QUERIES : [])) : [];
  const accounts = xApiEnabled ? parseList(options.accounts ?? process.env.X_MONITOR_ACCOUNTS) : [];
  ...
  } else if (xApiEnabled && (queries.length || accounts.length) && !bearerToken) {
    issues.push(createIssue("x_api", "", "X_BEARER_TOKEN is required when X_FETCH_ENABLED=true"));
  }
  ...
  return {
    ...
    xApiEnabled,
    configuredXSearchQueries: queries.length,
    configuredXMonitorAccounts: accounts.length,
