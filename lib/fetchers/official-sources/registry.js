import { buildOfficialProviderCapabilityStatus, buildOfficialProviderIntegrationPreview, getOfficialProviderCapabilityProfile } from "../../domain/official-provider-capabilities.js";
...
export async function fetchOfficialSourceExpansionDiagnostic(options = {}) {
  ...
  const providers = selectOfficialSourceExpansionProviders(options.providers);
  const sourceResults = [];
...
export async function fetchOfficialProviderSourceExpansionDiagnostic(source, options = {}) {
  const snapshot = await fetchOfficialSourceExpansionDiagnostic({ ...options, providers: [source] });
...
async function fetchProvider(provider, { mode, fetchImpl, limits, cursor }) {
  ...
  return {
    ...
    capability_profile: buildOfficialProviderIntegrationPreview(provider.source, { records: safeRecords, metadataRecords: safeMetadataRecords }),
  };
}
...
export function parseProviderDetail(source, body, officialUrl, discovery = {}) {
  ...
  if (!detail.variant_names.length) return { ok: false, issue_code: provider.source === "kitan_club" && Number.isInteger(detail.expected_variant_count) ? "official_detail_variant_count_mismatch" : "official_detail_zero_lineup", metadata };
  ...
  return {
    ...
    record: {
      ...
      source_count_evidence: detail.source_count_evidence || null,
      source_count_conflict: detail.source_count_conflict === true,
      capability: buildOfficialProviderCapabilityStatus(provider.source, { metadata: recordMetadata, variants, sourceCountConflict: detail.source_count_conflict === true }),
      diagnostic_identity: {
...
function emptyMetrics(provider) {
  return { source: provider.source, manufacturer: provider.manufacturer, list_url: provider.list_url, mode: null, capability_profile: getOfficialProviderCapabilityProfile(provider.source), list_pages_fetched: 0, products_discovered: 0, detail_attempted: 0, detail_success: 0, detail_failures: 0, parse_failures: 0, successful_records: 0, metadata_only_records: 0, metadata_only_reasons: {}, rejected_records: 0, rejection_reasons: {}, request_count: 0, request_failures: 0, lineup_discovered: 0, lineup_attempted: 0, lineup_success: 0, lineup_failures: 0, lineup_fetch_limit: 0, lineup_archive_pages_fetched: 0, lineup_archive_page_limit: 0 };
}
...
function sanitizeDiscoveryMetadata(provider, entry, metadata) {
  ...
  return { ...record, capability: buildOfficialProviderCapabilityStatus(provider.source, { metadata: { ...record, manufacturer: provider.manufacturer }, variants: [] }) };
}
...
function buildQualiaLinkedRecord(provider, entry, metadata, linked) {
  ...
  return {
    ...
    source_count_evidence: null,
    source_count_conflict: false,
    capability: buildOfficialProviderCapabilityStatus(provider.source, { metadata: recordMetadata, variants }),
    diagnostic_identity: {
...
function parseKitanDetail(html, baseUrl) {
  ...
  const detailFieldCount = expectedCount(fields.get("価格"));
  const prose = parseKitanLineupProse(html);
  const formalPickupVariants = prose.names.length
    ? pickupVariants.filter((variant) => prose.names.some((name) => normalizeName(name) === normalizeName(variant.name)))
    : pickupVariants;
  const namedVariantCount = uniqueNames(formalPickupVariants.map((variant) => variant.name)).size;
  const sourceCountEvidence = {
    detail_field_count: detailFieldCount,
    formal_lineup_prose_count: prose.declared_count,
    concrete_named_variant_count: namedVariantCount || null,
  };
  const declaredCounts = Object.values(sourceCountEvidence).filter(Number.isInteger);
  const sourceCountConflict = new Set(declaredCounts).size > 1;
  const proseIsSafe = prose.names.length > 0 && prose.names.length === prose.declared_count && prose.names.every((name) => pickupVariants.some((variant) => normalizeName(variant.name) === normalizeName(name)));
  const pickupIsSafe = namedVariantCount > 0 && detailFieldCount === namedVariantCount;
  const variants = proseIsSafe
    ? prose.names.map((name) => ({ name, image: pickupVariants.find((variant) => normalizeName(variant.name) === normalizeName(name))?.image || null }))
    : pickupIsSafe
      ? pickupVariants
      : [];
  const expectedVariantCount = proseIsSafe ? prose.declared_count : detailFieldCount;
  return { series_name: title, variant_names: variants.map((variant) => variant.name), variant_images: variants.map((variant) => variant.image), expected_variant_count: expectedVariantCount, source_count_evidence: sourceCountEvidence, source_count_conflict: sourceCountConflict, release_date: parseReleaseDate(fields.get("発売日")), release_month: parseReleaseMonth(fields.get("発売日")), price: parsePrice(fields.get("価格")), series_image_candidate: safeUrl(html.match(/<div[^>]*class=["'][^"']*c-productDetail__thum[^"']*["'][^>]*>[\s\S]*?<img[^>]+src=["']([^"']+)["']/i)?.[1], baseUrl), copyright_text: cleanText(html.match(/(?:©|\(c\))[^<\n]+/i)?.[0]) || null };
}

function parseKitanLineupProse(html) {
  const match = String(html || "").match(/ラインナップは[「『]\s*([\s\S]*?)\s*[」』]\s*の全\s*(\d+)\s*種/i);
  if (!match) return { declared_count: null, names: [] };
  const names = cleanText(match[1]).split(/\s*、\s*/).map(cleanText).filter(Boolean);
  const declaredCount = Number(match[2]);
  return { declared_count: declaredCount, names: names.length === declaredCount && new Set(names.map(normalizeName)).size === declaredCount ? names : [] };
}
...
function selectOfficialSourceExpansionProviders(value) {
  if (value == null) return listOfficialSourceExpansionProviders();
  const sources = Array.isArray(value) ? value : [value];
  const normalized = [...new Set(sources.map(text).filter(Boolean))];
  if (!normalized.length) throw new Error("Official diagnostic provider selection is empty.");
  return normalized.map((source) => {
    const provider = assertProvider(source);
    const { detail_path: _detailPath, ...publicProvider } = provider;
    return { ...publicProvider, detail_path: provider.detail_path };
  });
}
...
function cleanText(value) { return decodeHtmlEntities(String(value || "").replace(/<[^>]+>/g, " ")).replace(/\s+/g, " ").trim(); }
function decodeHtmlEntities(value) { return String(value || "").replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/&#(x[\da-f]+|\d+);/gi, (_match, entity) => { const codePoint = entity[0].toLowerCase() === "x" ? Number.parseInt(entity.slice(1), 16) : Number.parseInt(entity, 10); try { return Number.isInteger(codePoint) && codePoint >= 0 && codePoint <= 0x10ffff ? String.fromCodePoint(codePoint) : _match; } catch { return _match; } }); }
function uniqueNames(names) { return new Set(names.map(normalizeName).filter(Boolean)); }
