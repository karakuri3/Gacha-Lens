...
async function fetchProvider(provider, { mode, fetchImpl, limits, cursor }) {
  ...
  if (mode === "BACKFILL_SAMPLE" && provider.source === "qualia") {
  ...
  const qualiaLineupUrls = provider.source === "qualia" ? new Set(parseQualiaLineupLinks(list.body, archive.url)) : new Set();
  const issues = [];
  for (const entry of selected) {
    ...
    if (provider.source === "qualia") {
      for (const lineupUrl of parseQualiaLineupLinks(detail.body, entry.official_url)) qualiaLineupUrls.add(lineupUrl);
    }
  ...
  if (provider.source === "qualia") {
    ...
    for (const pending of pendingQualiaProducts) {
      const linked = lineups.map((lineup) => linkQualiaProductToLineup(pending.metadata, lineup)).find((result) => result.linked);
      if (linked) {
      ...
      metrics.detail_failures += 1;
      metrics.parse_failures += 1;
      issues.push("official_detail_zero_lineup");
...
function emptyMetrics(provider) {
  return { source: provider.source, manufacturer: provider.manufacturer, list_url: provider.list_url, mode: null, list_pages_fetched: 0, products_discovered: 0, detail_attempted: 0, detail_success: 0, detail_failures: 0, parse_failures: 0, request_count: 0, request_failures: 0, lineup_discovered: 0, lineup_attempted: 0, lineup_success: 0, lineup_failures: 0, lineup_fetch_limit: 0 };
}
...
function canonicalQualiaLineupUrl(value, baseUrl) { try { const url = new URL(value, baseUrl); url.hash = ""; if (url.protocol !== "https:" || !["qualia-45.jp", "www.qualia-45.jp"].includes(url.hostname) || url.search || !/^\/distinations\/[^/?#]+\/$/.test(url.pathname)) return null; url.hostname = "www.qualia-45.jp"; return url.toString(); } catch { return null; } }
