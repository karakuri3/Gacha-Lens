...
async function fetchProvider(provider, { mode, fetchImpl, limits, cursor }) {
  ...
  return {
    ...
    successful_records: safeRecords.length,
    metadata_only_records: safeMetadataRecords.length,
    rejected_records: metrics.detail_failures,
    rejection_reasons: countIssueCodes(issues),
    parser_success: safeRecords.length > 0,
    parser_complete: issues.length === 0,
