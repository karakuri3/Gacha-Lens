...
async function fetchProvider(provider, { mode, fetchImpl, limits, cursor }) {
  ...
  const issues = [];
  const qualiaLineupUrls = provider.source === "qualia"
    ? await discoverQualiaLineupUrls(provider, { fetchImpl, limits, metrics, issues })
    : [];
...
async function discoverQualiaLineupUrls(provider, { fetchImpl, limits, metrics, issues }) {
  ...
  for (let index = 0; index < archiveUrls.length && index < limits.qualia_lineup_archive_page_limit; index += 1) {
    ...
    for (const lineupUrl of parseQualiaLineupLinks(response.body, url)) discovered.add(lineupUrl);
