...
export function buildMarketSearchQueries(catalog = {}, options = {}) {
  ...
  for (const entry of rotatingWindow(recent, recentLimit, rotationCursor(now, recent.length))) {
    queries.push(seriesQuery(entry));
    const children = variantsBySeries.get(entry.id) ?? [];
    const child = children[rotationCursor(now, children.length, hash(entry.id))];
  ...
  const archivePool = interleave(
    rotatingWindow(archive, archive.length, rotationCursor(now, archive.length, 17)),
    rotatingWindow(deepArchive, deepArchive.length, rotationCursor(now, deepArchive.length, 31))
  );
  for (const entry of archivePool) {
    const children = variantsBySeries.get(entry.id) ?? [];
    const child = children[rotationCursor(now, children.length, hash(entry.id) + 7)];
  ...
  if (queries.length < limit) {
    for (const entry of rotatingWindow(candidates, limit - queries.length, rotationCursor(now, candidates.length, 31))) {
      queries.push(seriesQuery(entry));
    }
  }
...
function rotationCursor(now, length, salt = 0) {
  if (!length) return 0;
  const intervalMinutes = Math.max(5, number(process.env.MARKET_QUERY_CURSOR_MINUTES) ?? 30);
  const interval = Math.floor(now.getTime() / (intervalMinutes * 60 * 1000));
  return Math.abs(interval + salt) % length;
}
