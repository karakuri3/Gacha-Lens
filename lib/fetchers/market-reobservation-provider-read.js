...
import { resolveMarketRetryOptions, retryableJsonRequest } from "./retryable-json-request.js";
...
export async function fetchRakutenExactReobservation(identity, options = {}) {
  ...
  const retry = resolveMarketRetryOptions(options, timeoutMs);
  ...
  const exact = items.find((item) => clean(item?.itemCode) === identity.sourceListingId) ?? items[0];
  return {
    result: normalizeRakutenReobservationResponse(exact, {
...
export async function fetchYahooExactReobservation(identity, options = {}) {
  ...
  const response = await retryableTextRequest(url, {
    timeoutMs,
    maxAttempts,
    fetchImpl: options.fetchImpl,
    sleep: options.sleep,
  ...
  let payload;
  try {
    payload = parseYahooItemLookupJsonp(response.text, YAHOO_CALLBACK);
  } catch {
  ...
  return {
    result: normalizeYahooReobservationResponse(parsed.item, {
      source_listing_id: identity.sourceListingId,
      public_url: identity.publicUrl,
    }),
    diagnostics,
...
export function buildRakutenExactItemUrl({ endpoint = RAKUTEN_ENDPOINT, applicationId, itemCode } = {}) {
  ...
  const url = secureEndpoint(endpoint, RAKUTEN_ENDPOINT, "Rakuten");
...
export function buildYahooExactItemLookupUrl({ endpoint = YAHOO_ENDPOINT, appId, itemCode } = {}) {
  ...
  const url = secureEndpoint(endpoint, YAHOO_ENDPOINT, "Yahoo");
...
export function parseYahooItemLookupJsonp(value, callback = YAHOO_CALLBACK) {
  const body = String(value ?? "").trim();
  const prefix = `${callback}(`;
  if (!body.startsWith(prefix)) throw new Error("Yahoo JSONP callback mismatch.");
  const suffixLength = body.endsWith(");") ? 2 : body.endsWith(")") ? 1 : 0;
  if (!suffixLength) throw new Error("Yahoo JSONP wrapper is incomplete.");
  const payload = body.slice(prefix.length, -suffixLength).trim();
...
async function retryableTextRequest(url, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (response?.ok) {
        ...
        attempts.push({ attempt, status, failure_category: null, rate_limited: false, timed_out: false });
        return { ok: true, status, text, diagnostics: textDiagnostics(attempts, null, status) };
      }
    ...
    if (!retryable || attempt >= maxAttempts) {
      return { ok: false, status, text: "", diagnostics: textDiagnostics(attempts, category, status) };
    }
    await sleep(Math.min(4000, 1000 * (2 ** (attempt - 1))));
  }

  return { ok: false, status: null, text: "", diagnostics: textDiagnostics(attempts, "unknown", null) };
}
...
function textDiagnostics(attempts, failureCategory, finalStatus) {
  return {
...
function secureEndpoint(value, fallback, label) {
  const url = new URL(clean(value) || fallback);
  if (url.protocol !== "https:" || url.username || url.password) {
    throw new Error(`${label} exact item endpoint must use credential-free HTTPS.`);
  }
  return url;
}
