...
export const OFFICIAL_AUDIT_SELECTION_MODES = Object.freeze({
  priority: "priority",
...
export async function fetchOfficialLiveSnapshot(options = {}) {
  ...
  const selectionMode = normalizeSelectionMode(options.selectionMode);
  const limits = {
  ...
  const selectedDetails = selectOfficialAuditDetails(discoveries, {
    now,
    selectionMode,
    knownOfficialUrls: options.knownOfficialUrls,
    knownOfficialIds: options.knownOfficialIds,
  ...
  return {
    ...
    selection: {
      mode: selectionMode,
      selected_detail_count: selectedDetails.length,
    },
    sources,
...
export function selectOfficialAuditDetails(discoveries, options = {}) {
  ...
  const knownUrls = new Set(asArray(options.knownOfficialUrls).map(canonicalOfficialDetailUrl).filter(Boolean));
  const knownIds = new Set(asArray(options.knownOfficialIds).map(text).filter(Boolean));
  const limits = {
  ...
  return unique
    .map((entry) => ({
      ...entry,
      progressive_rank: selectionMode === OFFICIAL_AUDIT_SELECTION_MODES.progressive
        && !isKnownOfficialDiscovery(entry, knownUrls, knownIds) ? 0 : 1,
      priority: detailPriority(entry.record, entry.url, interest, now),
    }))
    ...
    .map(({ priority: _priority, progressive_rank: _progressiveRank, ...entry }) => entry);
}

export function buildKnownOfficialCatalogIdentity(catalog = {}) {
  const series = asArray(catalog.series);
  const variants = asArray(catalog.variants);
  return {
    urls: [...new Set([...series, ...variants].map((row) => text(row?.official_url)).filter(Boolean))].sort(),
    ids: [...new Set([
      ...series.map((row) => text(row?.id)),
      ...variants.map((row) => text(row?.series_id)),
    ].filter(Boolean))].sort(),
  };
}

function isKnownOfficialDiscovery(entry, knownUrls, knownIds) {
  return knownUrls.has(canonicalOfficialDetailUrl(entry?.url))
    || knownIds.has(text(entry?.record?.id));
}

function normalizeSelectionMode(value) {
  const mode = text(value) || OFFICIAL_AUDIT_SELECTION_MODES.priority;
  if (!Object.hasOwn(OFFICIAL_AUDIT_SELECTION_MODES, mode)) {
    throw new Error("Unsupported official audit selection mode.");
  }
  return mode;
}

function canonicalOfficialDetailUrl(value) {
  try {
    const url = new URL(value);
    const provider = url.hostname === "gashapon.jp" ? "gashapon"
      : url.hostname === "www.takaratomy-arts.co.jp" ? "takaratomy_arts" : "";
    if (!provider || !isAllowedOfficialDetailUrl(url.toString(), provider)) return "";
    url.hash = "";
    return url.toString();
  } catch {
    return "";
...
function comparePriority(left, right) {
  return left.progressive_rank - right.progressive_rank
    || left.priority.bucket - right.priority.bucket
...
async function fetchDetail(detail, options) {
  ...
  try {
    ...
    const parsedRecord = parsed.record ?? null;
    const record = parsedRecord
      ? mergeOfficialRecords(detail.record ? [detail.record] : [], [parsedRecord])[0] ?? parsedRecord
      : null;
