...
export async function fetchRakutenMarketListingsRaw(options = {}) {
  ...
  for (let index = 0; index < searchKeywords.length; index += 1) {
    ...
    const result = await fetchRakutenKeyword(url, { keyword, requestOrigin });
    ...
    records.push(...result.items.filter(isRelevantRakutenItem).map((item) => normalizeRakutenItem(item, {
      keyword,
...
async function fetchRakutenKeyword(url, { keyword, requestOrigin }) {
...
function isRelevantRakutenItem(item) {
  const title = text(item.itemName);
  ...
  if (rejected.some((pattern) => pattern.test(normalized))) return false;
  return /(ガチャガチャ|ガシャポン|カプセルトイ|カプセル[\s・]?(トイ|フィギュア|コレクション)|全\d+種|コンプ(リート)?セット)/i.test(normalized);
}
