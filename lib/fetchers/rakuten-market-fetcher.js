import { createFetchIssue, parseList, stableId, text } from "./feed-source-utils.js";
import { assessMarketItemRelevance } from "./market-item-relevance.js";
import { dedupeMarketQueries } from "./market-query-dedupe.js";
import { expandMarketQueryAttempts } from "./market-query-planner.js";
import { resolveMarketProviderRootLimit } from "./market-request-budget.js";
import {
  configurationRequestDiagnostics,
  resolveMarketRetryOptions,
  retryableJsonRequest,
} from "./retryable-json-request.js";

const DEFAULT_KEYWORDS = ["ガチャ", "ガチャガチャ", "カプセルトイ", "ガシャポン"];
const DEFAULT_ENDPOINT = "https://openapi.rakuten.co.jp/ichibams/api/IchibaItem/Search/20260701";
const DEFAULT_REQUEST_ORIGIN = "https://gachalens.com";
const DISCOVERY_ELEMENTS = [
  "itemName",
  "itemPrice",
  "itemUrl",
  "itemCode",
  "availability",
  "shopName",
  "shopCode",
  "shopUrl",
  "mediumImageUrls",
  "smallImageUrls",
  "reviewCount",
  "reviewAverage",
  "genreId",
].join(",");
const AFFILIATE_ENRICHMENT_ELEMENTS = ["itemCode", "itemUrl", "affiliateUrl"].join(",");

export async function fetchRakutenMarketListingsRaw(options = {}) {
  const enabled = parseBoolean(options.enabled ?? process.env.RAKUTEN_MARKET_FETCH_ENABLED ?? Boolean(process.env.RAKUTEN_APPLICATION_ID));
  if (!enabled) {
    return emptyResult({ enabled: false });
  }

  const applicationId = text(options.applicationId ?? process.env.RAKUTEN_APPLICATION_ID);
  const accessKey = text(options.accessKey ?? process.env.RAKUTEN_ACCESS_KEY);
  const affiliateId = text(options.affiliateId ?? process.env.RAKUTEN_AFFILIATE_ID);
  const endpoint = text(options.endpoint ?? process.env.RAKUTEN_ICHIBA_ITEM_SEARCH_URL) || DEFAULT_ENDPOINT;
  const requestOrigin = resolveRequestOrigin(options);
  const keywords = parseList(options.keywords ?? process.env.RAKUTEN_MARKET_KEYWORDS);
  const queryLimit = resolveMarketProviderRootLimit(
    "rakuten_ichiba",
    options.queryLimit ?? process.env.RAKUTEN_MARKET_QUERY_LIMIT,
  );
  const normalizedQueries = prioritizeVariantQueries(normalizeQueries(options.queries, keywords.length ? keywords : DEFAULT_KEYWORDS));
  const dedupedQueries = dedupeMarketQueries(normalizedQueries);
  const rootQueries = dedupedQueries.queries.slice(0, queryLimit);
  const queries = expandMarketQueryAttempts(rootQueries);
  const hits = clamp(number(options.hits ?? process.env.RAKUTEN_MARKET_HITS) ?? 20, 1, 30);
  const delayMs = Math.max(0, number(options.delayMs ?? process.env.RAKUTEN_REQUEST_DELAY_MS) ?? 1200);
  const requestDelay = options.requestDelay ?? options.sleep ?? delay;
  const timeoutMs = clamp(number(options.timeoutMs ?? process.env.RAKUTEN_REQUEST_TIMEOUT_MS) ?? 12000, 2000, 30000);
  const retryOptions = resolveMarketRetryOptions(options, timeoutMs);
  const availability = text(options.availability ?? process.env.RAKUTEN_MARKET_AVAILABILITY) || "1";
  const sort = text(options.sort ?? process.env.RAKUTEN_MARKET_SORT) || "-updateTimestamp";
  const issues = [];
  const records = [];
  const feedResults = [];
  const enrichedRoots = new Set();
  let requestCount = 0;

  const source = {
    name: "rakuten-ichiba-item-search",
    source: "rakuten_ichiba",
    url: "https://webservice.rakuten.co.jp/documentation/ichiba-item-search",
  };

  if (!applicationId) {
    return issueResult(source, "RAKUTEN_APPLICATION_ID is not configured.", rootQueries.length);
  }

  if (!accessKey) {
    return issueResult(source, "RAKUTEN_ACCESS_KEY is not configured. Current Rakuten Ichiba Item Search API requires accessKey with applicationId.", rootQueries.length);
  }

  for (let index = 0; index < queries.length; index += 1) {
    const query = queries[index];
    const keyword = normalizeRakutenKeywordForApi(query.query);
    if (!keyword) {
      issues.push(createFetchIssue("market_fetch", source, "Rakuten keyword could not be safely normalized.", "market_listings"));
      continue;
    }
    if (requestCount > 0 && delayMs) await requestDelay(delayMs);

    const url = buildRakutenUrl(endpoint, {
      applicationId,
      accessKey,
      keyword,
      hits,
      availability,
      sort,
      elements: DISCOVERY_ELEMENTS,
    });

    const result = await fetchRakutenKeyword(url, {
      keyword,
      queryIndex: index,
      requestKind: "discovery",
      requestOrigin,
      retryOptions,
      fetchImpl: options.fetchImpl,
      sleep: options.sleep,
      clock: options.clock,
      random: options.random,
    });
    requestCount += 1;
    if (!result.ok) {
      feedResults.push(result.feedResult);
      issues.push(createFetchIssue("market_fetch", source, result.message, "market_listings"));
      continue;
    }

    const relevance = assessItems(result.items, (item) => item.itemName, query);
    const relevantItems = relevance.accepted;
    Object.assign(result.feedResult, relevance.diagnostics);
    feedResults.push(result.feedResult);
    let affiliateDestinations = new Map();
    const rootKey = `${query.variant_id || "keyword"}:${query.root_query || query.query}`;
    if (affiliateId && relevantItems.length > 0 && !enrichedRoots.has(rootKey)) {
      enrichedRoots.add(rootKey);
      if (requestCount > 0 && delayMs) await requestDelay(delayMs);
      const enrichmentUrl = buildRakutenUrl(endpoint, {
        applicationId,
        accessKey,
        affiliateId,
        keyword,
        hits,
        availability,
        sort,
        elements: AFFILIATE_ENRICHMENT_ELEMENTS,
      });
      const enrichment = await fetchRakutenKeyword(enrichmentUrl, {
        keyword,
        queryIndex: index,
        requestKind: "affiliate_enrichment",
        requestOrigin,
        retryOptions,
        fetchImpl: options.fetchImpl,
        sleep: options.sleep,
        clock: options.clock,
        random: options.random,
      });
      requestCount += 1;
      feedResults.push(enrichment.feedResult);
      if (enrichment.ok) affiliateDestinations = buildAffiliateDestinationsByItemCode(enrichment.items);
    }

    records.push(...relevantItems.map((item) => normalizeRakutenItem(item, {
      keyword,
      query: rootQueryContext(query),
      executedQuery: keyword,
      sourceUrl: source.url,
      fetchedAt: result.fetchedAt,
      affiliateUrl: affiliateDestinations.get(text(item.itemCode)) || "",
    })));
  }

  return {
    ok: true,
    enabled: true,
    source: "rakuten_ichiba",
    configuredSources: rootQueries.length,
    count: records.length,
    records: dedupeById(records),
    issues,
    feedResults,
    duplicateQueriesSkipped: dedupedQueries.duplicateQueriesSkipped,
  };
}

function emptyResult(extra = {}) {
  return {
    ok: true,
    source: "rakuten_ichiba",
    configuredSources: 0,
    count: 0,
    records: [],
    issues: [],
    feedResults: [],
    ...extra,
  };
}

function issueResult(source, message, configuredSources) {
  return {
    ...emptyResult({ enabled: true, ok: true, configuredSources }),
    issues: [createFetchIssue("market_fetch", source, message, "market_listings")],
    feedResults: [{
      name: source.name,
      source: source.source,
      url: source.url,
      format: "api",
      ok: false,
      status: null,
      message,
      ...configurationRequestDiagnostics(),
    }],
  };
}

async function fetchRakutenKeyword(url, { keyword, queryIndex, requestKind, requestOrigin, retryOptions, fetchImpl, sleep, clock, random }) {
  const fetchedAt = new Date().toISOString();
  const response = await retryableJsonRequest(url, {
    ...retryOptions,
    fetchImpl,
    sleep,
    clock,
    random,
    request: {
      headers: {
        accept: "application/json",
        origin: requestOrigin,
        referer: `${requestOrigin}/`,
        "user-agent": "GachaLensBot/0.3 (+rakuten-ichiba-api)",
      },
    },
  });
  const items = response.ok ? extractItems(response.data) : [];

  return {
    ok: response.ok,
    message: response.message,
    items,
    fetchedAt,
    feedResult: {
      name: requestKind === "affiliate_enrichment" ? `rakuten-affiliate:${keyword}` : `rakuten:${keyword}`,
      source: "rakuten_ichiba",
      url: "https://webservice.rakuten.co.jp/documentation/ichiba-item-search",
      format: "api",
      ok: response.ok,
      status: response.status,
      message: response.message,
      query: keyword,
      query_index: queryIndex,
      request_kind: requestKind,
      ...response.diagnostics,
    },
  };
}

function buildRakutenUrl(endpoint, params) {
  const url = new URL(endpoint);
  url.searchParams.set("applicationId", params.applicationId);
  url.searchParams.set("accessKey", params.accessKey);
  url.searchParams.set("format", "json");
  url.searchParams.set("formatVersion", "2");
  url.searchParams.set("keyword", params.keyword);
  url.searchParams.set("hits", String(params.hits));
  url.searchParams.set("availability", params.availability);
  url.searchParams.set("imageFlag", "1");
  url.searchParams.set("sort", params.sort);
  url.searchParams.set("elements", params.elements || DISCOVERY_ELEMENTS);
  if (params.affiliateId) url.searchParams.set("affiliateId", params.affiliateId);
  return url;
}

function extractItems(body) {
  const items = Array.isArray(body.items)
    ? body.items
    : Array.isArray(body.Items)
      ? body.Items
      : [];
  return items.map((entry) => entry.Item || entry.item || entry).filter(Boolean);
}

function normalizeRakutenItem(item, context) {
  const title = text(item.itemName);
  const itemCode = text(item.itemCode);
  const affiliateUrl = text(context.affiliateUrl);
  const publicItemUrl = text(item.itemUrl);
  const sourceUrl = publicItemUrl;
  const imageUrls = [
    ...asImageUrls(item.mediumImageUrls),
    ...asImageUrls(item.smallImageUrls),
  ];

  return {
    id: stableId("rakuten", itemCode || sourceUrl || title),
    title,
    price: number(item.itemPrice),
    status: text(item.availability) === "0" ? "sold_out" : "active",
    source: "rakuten",
    source_type: "marketplace",
    source_url: sourceUrl,
    listed_at: context.fetchedAt,
    sold_at: "",
    raw: {
      provider: "rakuten_ichiba",
      keyword: context.keyword,
      query: context.query,
      executed_query: context.executedQuery,
      itemCode,
      public_item_url: publicItemUrl,
      affiliate_url: affiliateUrl,
      affiliate_url_source: affiliateUrl ? "rakuten_api" : "",
      affiliate_url_contract: affiliateUrl ? "item_search_20260701_item_code_join" : "",
      shopName: text(item.shopName),
      shopCode: text(item.shopCode),
      shopUrl: text(item.shopUrl),
      reviewCount: number(item.reviewCount),
      reviewAverage: number(item.reviewAverage),
      genreId: text(item.genreId),
      imageUrls,
      fetchedAt: context.fetchedAt,
      source_documentation: context.sourceUrl,
    },
  };
}

function normalizeQueries(queries, keywords) {
  if (Array.isArray(queries)) {
    return queries.map((entry) => typeof entry === "string" ? { query: entry } : entry).filter((entry) => text(entry.query));
  }
  return keywords.map((query) => ({ query }));
}

export function normalizeRakutenKeywordForApi(value) {
  const tokens = text(value).split(/\s+/).filter(Boolean);
  const normalized = [];
  for (let index = 0; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (!/^[A-Za-z0-9]$/.test(token)) {
      normalized.push(token);
      continue;
    }
    if (normalized.length > 0) {
      normalized[normalized.length - 1] += token;
      continue;
    }
    const next = tokens[index + 1];
    if (!next) return "";
    normalized.push(`${token}${next}`);
    index += 1;
  }
  return normalized.join(" ");
}

function rootQueryContext(query) {
  const context = { ...query };
  const rootQuery = context.root_query;
  delete context.root_query;
  delete context.root_query_index;
  delete context.query_attempt_index;
  return {
    ...context,
    query: rootQuery || query.query,
    query_strategy_version: query.query_strategy_version || 1,
  };
}

function assessItems(items, titleSelector, query) {
  const accepted = [];
  const rejectionReasonCounts = {};
  for (const item of items) {
    const assessment = assessMarketItemRelevance(titleSelector(item), query);
    if (assessment.accepted) accepted.push(item);
    else rejectionReasonCounts[assessment.reason] = (rejectionReasonCounts[assessment.reason] ?? 0) + 1;
  }
  return {
    accepted,
    diagnostics: {
      results_returned: items.length,
      normalized_records: accepted.length,
      records_rejected: items.length - accepted.length,
      rejection_reason_counts: rejectionReasonCounts,
    },
  };
}

function prioritizeVariantQueries(queries) {
  return [...queries].sort((a, b) => Number(b.kind === "variant") - Number(a.kind === "variant"));
}

function asImageUrls(value) {
  if (!Array.isArray(value)) return [];
  return value.map((entry) => text(entry.imageUrl || entry)).filter(Boolean);
}

function normalizeOrigin(value) {
  const raw = text(value);
  if (!raw) return "";
  try {
    const url = new URL(raw.startsWith("http://") || raw.startsWith("https://") ? raw : `https://${raw}`);
    return url.origin;
  } catch {
    return "";
  }
}

function buildAffiliateDestinationsByItemCode(items) {
  const destinations = new Map();
  const conflicted = new Set();
  for (const item of items) {
    const itemCode = text(item.itemCode);
    const affiliateUrl = text(item.affiliateUrl);
    const responseItemUrl = text(item.itemUrl);
    if (!itemCode || !affiliateUrl || responseItemUrl !== affiliateUrl || conflicted.has(itemCode)) continue;
    const existing = destinations.get(itemCode);
    if (existing && existing !== affiliateUrl) {
      destinations.delete(itemCode);
      conflicted.add(itemCode);
      continue;
    }
    destinations.set(itemCode, affiliateUrl);
  }
  return destinations;
}

function resolveRequestOrigin(options = {}) {
  const explicit = options.requestOrigin ?? process.env.RAKUTEN_REQUEST_ORIGIN;
  const canonical = options.siteUrl ?? process.env.NEXT_PUBLIC_SITE_URL;
  return normalizeOrigin(explicit) || normalizeOrigin(canonical) || DEFAULT_REQUEST_ORIGIN;
}

function dedupeById(records) {
  return [...new Map(records.map((record) => [record.id, record])).values()];
}

function delay(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function parseBoolean(value) {
  const normalized = String(value ?? "").trim().toLowerCase();
  if (!normalized) return false;
  return !["0", "false", "no", "off"].includes(normalized);
}

function clamp(value, min, max) {
  return Math.min(max, Math.max(min, value));
}

function number(value) {
  if (value == null || value === "") return null;
  const parsed = Number(String(value).replace(/[^\d.-]/g, ""));
  return Number.isFinite(parsed) ? parsed : null;
}
