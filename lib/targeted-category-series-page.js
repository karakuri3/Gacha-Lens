...
async function readCategoryPage(category, page, pageSize) {
  ...
  if (source === "supabase") {
    // The public first page already has a bounded Cloudflare HTML edge-cache policy,
    // while the route-level React cache deduplicates metadata/page reads per request.
    // Avoid routing this targeted read through vinext's KV-backed data cache.
