...
export async function getTargetedPublicCategorySeriesPage(name, options = {}) {
  ...
  const direct = await readCategoryPage(requestedName, requestedPage, pageSize, Boolean(options.bypassDataCache));
  ...
  const fallback = await readCategoryPage(facet.filter_value, requestedPage, pageSize, Boolean(options.bypassDataCache));
  if (fallback.total === 0) return null;
  return buildResult(fallback, facet.name, facet.filter_value);
}

async function readCategoryPage(category, page, pageSize, bypassDataCache = false) {
