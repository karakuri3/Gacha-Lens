import {
  ...
  OFFICIAL_PHASE_A3_VARIANT_COLUMNS,
} from "../domain/official-phase-a3.js";

const VARIANT_COLUMNS = OFFICIAL_PHASE_A3_VARIANT_COLUMNS;
...
export async function executeOfficialPhaseA3VariantTransaction({ client, plan } = {}) {
  ...
  const variantSlugs = plan.variant_rows.map((row) => row.slug);
  ...
  try {
    ...
    const idCollisions = await client.query(
      "SELECT id FROM public.variants WHERE id = ANY($1::text[]) LIMIT 1",
      [variantIds],
    );
    if (idCollisions.rows?.length) throw phaseA3Error("phase_a3_variant_identity_collision");

    const slugCollisions = await client.query(
      "SELECT id, slug FROM public.variants WHERE slug = ANY($1::text[]) LIMIT 1",
      [variantSlugs],
    );
    if (slugCollisions.rows?.length) throw phaseA3Error("phase_a3_variant_slug_collision");
