...
test("bulk runner keeps each write round bounded while allowing a large one-time campaign", () => {
  assert.equal(BULK_RECOVERY_ROUND_LIMIT, 25);
  assert.equal(BULK_RECOVERY_MAX_ROUNDS, 50);
  assert.equal(BULK_RECOVERY_MAX_TARGETS, 1250);
  assert.equal(BULK_RECOVERY_LOOKBACK_DAYS, 90);
...
test("bulk target selection covers the full recent-release priority-3 window and sorts newest first", () => {
  const now = new Date("2026-09-16T12:00:00.000Z");
  const selected = selectBulkRecoveryTargets([
    row({ variantId: "recent", releaseDate: "2026-09-10T00:00:00.000Z" }),
    row({ variantId: "older-recent", releaseDate: "2026-07-01T00:00:00.000Z" }),
    ...
    row({ variantId: "too-old", releaseDate: "2026-05-01T00:00:00.000Z" }),
    row({ variantId: "backlog", releaseDate: "2026-05-01T00:00:00.000Z", priority: 5 }),
  ], { now });
  assert.deepEqual(selected.map((entry) => entry.variantId), ["recent", "older-recent"]);
});

test("round calculation accounts for both total capacity and repeated variants in one series", () => {
  assert.equal(calculateBulkRecoveryRounds(0, 0), 0);
  assert.equal(calculateBulkRecoveryRounds(100, 1), 4);
  assert.equal(calculateBulkRecoveryRounds(100, 7), 7);
  assert.equal(calculateBulkRecoveryRounds(1250, 1), 50);
  assert.throws(() => calculateBulkRecoveryRounds(-1, 0));
});

test("series depth reports the repeated-series traversal requirement", () => {
  const rows = [
