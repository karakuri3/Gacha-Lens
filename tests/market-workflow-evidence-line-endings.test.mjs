...
const PHASE_6_D_DIGEST = "f09197b1c6599fa7a624ce31260d2345a376da66e2af1ff54a6a6a072133d9b8";
...
test("the committed Production workflow resolves to the reviewed Phase 6-D digest", async () => {
  const workflow = await readFile(new URL("../.github/workflows/gacha-ingestion.yml", import.meta.url));
  assert.equal(productionWorkflowDigest(workflow), PHASE_6_D_DIGEST);
  assert.equal(isReviewedProductionWorkflow(workflow), true);
});

test("the allowlist contains only the corrected Phase 6-D digest", () => {
  assert.equal(REVIEWED_PRODUCTION_WORKFLOW_DIGESTS.filter((digest) => digest === PHASE_6_D_DIGEST).length, 1);
